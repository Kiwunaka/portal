      ...
      <RouteBoundary loading={resource.loading} refreshing={resource.refreshing} error={resource.error} hasData={resource.data !== null} retryLabel="Повторить governance" onRetry={resource.reload}>
        ...
        {tab === "audit" ? (
          <Card>
            ...
            <div className="overflow-x-auto"><table className="w-full min-w-[70rem] border-collapse text-left text-xs"><thead><tr className="border-b border-[color:var(--atlas-border)] text-[color:var(--atlas-text-muted)]"><th className="p-2">Время</th><th className="p-2">Оператор</th><th className="p-2">Роль</th><th className="p-2">Действие</th><th className="p-2">Ресурс</th><th className="p-2">Результат</th><th className="p-2">Command</th></tr></thead><tbody>{(resource.data?.audit.items || []).map((row) => <tr key={row.id} className="border-b border-[color:var(--atlas-border)] align-top"><td className="p-2 whitespace-nowrap">{dateTime(row.created_at)}</td><td className="p-2"><strong>{row.operator_name || shortId(row.operator_id)}</strong><span className="mt-0.5 block font-mono text-[10px] text-[color:var(--atlas-text-muted)]">session {shortId(row.session_id)}</span></td><td className="p-2">{row.roles.join(", ") || "—"}</td><td className="p-2 font-mono">{row.action}</td><td className="p-2 font-mono">{row.resource ? `${row.resource.type}:${shortId(row.resource.id)}` : "—"}</td><td className="p-2"><Badge tone={statusTone(row.result)}>{row.result}</Badge>{row.reason_code ? <span className="mt-1 block text-[color:var(--atlas-text-muted)]">{row.reason_code}</span> : null}</td><td className="p-2 font-mono">{shortId(row.command_intent_id)}</td></tr>)}</tbody></table></div>
