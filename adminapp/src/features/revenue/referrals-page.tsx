...
export function ReferralsPage({ onShellStatus }: { onShellStatus?: (status: OpsShellStatus) => void }) {
  ...
  const queueRows = useMemo(() => [...(queue.data || [])].filter((row) => !urlState.q || `${row.order_id} ${row.referrer_tg_id} ${row.referred_tg_id}`.toLowerCase().includes(urlState.q.toLowerCase())).sort((left, right) => (BASIS_PRIORITY[left.basis] ?? 99) - (BASIS_PRIORITY[right.basis] ?? 99) || String(left.ready_at || "").localeCompare(String(right.ready_at || "")) || left.id - right.id), [queue.data, urlState.q]);
  ...
  const columns = useMemo<ColumnDef<ReferralRow>[]>(() => [
    ...
    { header: "Основание", cell: ({ row }) => <Badge tone={tone(row.original.basis)}>{basisLabel(row.original.basis)}</Badge> },
    { header: "Готово с", cell: ({ row }) => row.original.ready_at ? dateText(row.original.ready_at) : <MissingData /> },
    { header: "Статус", cell: ({ row }) => <Badge tone={tone(row.original.status)}>{basisLabel(row.original.status)}</Badge> },
  ], []);
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(340px,0.65fr)]">
        ...
        <aside aria-label="Карточка реферального решения"><Card className="xl:sticky xl:top-4"><SectionTitle title={selected ? `Запись #${selected.id}` : "Карточка решения"} description="Только безопасные идентификаторы, статус и основание; raw meta не возвращается." />{selected ? <dl className="grid grid-cols-2 gap-3 text-xs"><dt className="text-[color:var(--atlas-text-soft)]">Заказ</dt><dd className="font-semibold">{selected.order_id}</dd><dt className="text-[color:var(--atlas-text-soft)]">Реферер</dt><dd className="tabular-nums">{selected.referrer_tg_id}</dd><dt className="text-[color:var(--atlas-text-soft)]">Приглашённый</dt><dd className="tabular-nums">{selected.referred_tg_id}</dd><dt className="text-[color:var(--atlas-text-soft)]">Основание</dt><dd>{basisLabel(selected.basis)}</dd><dt className="text-[color:var(--atlas-text-soft)]">Поставлено</dt><dd>{selected.queued_at ? dateText(selected.queued_at) : "— · Нет данных"}</dd><dt className="text-[color:var(--atlas-text-soft)]">Обработано</dt><dd>{selected.processed_at ? dateText(selected.processed_at) : "— · Нет данных"}</dd></dl> : <EmptyState title="Выберите запись" description="Откройте строку очереди или истории." />}</Card></aside>
      </section>
