...
export function PromosPage({ onShellStatus }: { onShellStatus?: (status: OpsShellStatus) => void }) {
  ...
  function remove() {
    if (!selected) return;
    setRequest({ action: "promo.delete", target: { type: "promo", id: selected.code }, payload: {}, endpoint: `/api/admin/promos/${encodeURIComponent(selected.code)}`, method: "DELETE" });
    setDialogOpen(true);
  }

  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex gap-2 text-xs"><Badge tone={resource.error ? "warning" : resource.data ? "success" : "neutral"}>{resource.error ? "Источник промо отвечает с ошибкой" : resource.data ? "Промокоды получены" : "Промокоды ещё не получены"}</Badge><span className="text-[color:var(--atlas-text-soft)]">{resource.data ? `${resource.data.length} записей` : "— · Нет данных"}</span></div><div className="flex gap-2"><Button tone="primary" onClick={() => replaceUrlState<PromoUrlState>({ selected: "new" }, PROMO_URL_CODECS)}><Plus size={15} /> Создать</Button><Button tone="secondary" disabled={resource.loading || resource.refreshing} onClick={resource.reload}><RefreshCw size={15} className={resource.refreshing ? "animate-spin" : ""} /> Обновить</Button></div></div>
