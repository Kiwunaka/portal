...
export function TicketDetail({ ticket, permissions, onRefresh }: { ticket: AdminTicket; permissions: string[]; onRefresh: () => void }) {
  ...
  const knownIssues = useRouteResource(`support:known-issues:${ticket.id}:${issueErrorCode || "none"}`, loadKnownIssues, { enabled: Boolean(issueErrorCode) });
  const canReadSensitiveBundles = permissions.includes("support.sensitive.read");
  const selectedAttempt = attempts.data?.selected || attempts.data?.attempts[0] || null;
    ...
    <>
      <div className="space-y-3">
        ...
        <Card>
          ...
          {selectedAttempt ? <div className="mb-3 rounded-[var(--pokrov-radius-card)] border border-[color:var(--atlas-border)] bg-[color:var(--atlas-canvas)] p-3"><div className="flex flex-wrap items-center justify-between gap-2 text-xs"><div><strong>Сводка выбранной попытки</strong><p className="mt-1 text-[color:var(--atlas-text-muted)]">{[issueScope?.platform, issueScope?.appVersion, issueScope?.buildNumber].filter(Boolean).join(" · ") || "Версия не указана"}</p></div><code className="text-[11px]">{selectedAttempt.attemptRef}</code></div><dl className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-5">{diagnosticFacts.map((fact) => <div key={fact.label} className="rounded-[var(--pokrov-radius-control)] border border-[color:var(--atlas-border)] p-2 text-xs"><dt className="text-[color:var(--atlas-text-muted)]">{fact.label}</dt><dd className="mt-1 flex flex-wrap items-center gap-2"><Badge tone={diagnosticTone(fact.event?.result || null)}>{fact.event?.result || "Нет сигнала"}</Badge>{fact.event?.errorCode ? <code>{fact.event.errorCode}</code> : null}</dd></div>)}</dl></div> : null}
