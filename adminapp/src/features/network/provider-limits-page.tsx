...
function stateLabel(value: string): string {
  ...
  }[value.toLowerCase()] || value || "Нет данных";
}
...
export function ProviderLimitsPage({ onShellStatus }: { onShellStatus?: (status: OpsShellStatus) => void }) {
  ...
  useEffect(() => {
    ...
    setFormError("");
  }, [selectedCode, selectedConfig, selectedStatus]);
  ...
  const columns = useMemo<ColumnDef<ProviderQuotaStatus>[]>(() => [
    {
      header: "Нода",
      cell: ({ row }) => <button aria-label={`Открыть лимит ${row.original.node_code.toUpperCase()}`} className="rounded-sm text-left font-semibold uppercase outline-none hover:underline focus-visible:ring-2 focus-visible:ring-[color:var(--atlas-focus)]" onClick={() => replaceUrlState<ProviderUrlState>({ selected: row.original.node_code }, PROVIDER_URL_CODECS)}>{row.original.node_code}<span className="block font-normal normal-case text-[11px] text-[color:var(--atlas-text-muted)]">{row.original.node_name || "Название: —"}</span></button>,
    },
    { header: "Состояние", cell: ({ row }) => <Badge tone={tone(row.original.state)}>{stateLabel(row.original.state)}</Badge> },
    { header: "Использовано", cell: ({ row }) => row.original.configured ? <span className="tabular-nums">{numberText(row.original.used_gb, " ГиБ")} / {numberText(row.original.included_gb, " ГиБ")}</span> : <span>—<span className="block text-[11px] text-[color:var(--atlas-text-muted)]">Нет данных</span></span> },
    { header: "Осталось", cell: ({ row }) => row.original.configured ? <span className="tabular-nums">{numberText(row.original.remaining_gb, " ГиБ")}</span> : "—" },
    { header: "Заполнение", cell: ({ row }) => row.original.configured && finite(row.original.used_pct) !== null ? <div className="min-w-28"><div className="mb-1 tabular-nums">{numberText(row.original.used_pct, "%")}</div><Progress value={row.original.used_pct as number} tone={tone(row.original.state)} /></div> : <span>—<span className="block text-[11px] text-[color:var(--atlas-text-muted)]">Нет данных</span></span> },
    { header: "Сброс", cell: ({ row }) => <span>{dateText(row.original.cycle_end)}{row.original.cycle_end ? null : <span className="block text-[11px] text-[color:var(--atlas-text-muted)]">Нет данных</span>}</span> },
  ], []);

  function openSave(event: FormEvent) {
    event.preventDefault();
    if (!draft) return;
    const includedGb = Number(draft.includedGb);
    const resetDay = Number(draft.resetDay);
    const warningRatio = Number(draft.warningRatio);
    const criticalRatio = Number(draft.criticalRatio);
    ...
    const basePayload = { included_gb: includedGb, reset_day: resetDay, timezone: draft.timezone.trim(), warning_ratio: warningRatio, critical_ratio: criticalRatio, enabled: draft.enabled, notes: draft.notes.trim() || null };
    setFormError("");
    setRequest({
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2 text-xs text-[color:var(--atlas-text-soft)]"><Badge tone={resource.error ? "warning" : "success"}>{resource.error ? "Есть сбой источника" : "Лимиты перечитаны"}</Badge><span>{resource.data ? `${resource.data.configs.length} настроенных квот` : "Данные ещё не получены"}</span></div>
          ...
          <Card className="xl:sticky xl:top-4">
            ...
            {!draft ? <EmptyState title="Выберите ноду" description="Откройте строку в таблице, чтобы настроить или проверить квоту." /> : (
              <form className="space-y-3" onSubmit={openSave}>
                ...
                <label className="block text-xs font-semibold">Лимит, ГиБ<input aria-label="Лимит квоты в ГиБ" inputMode="decimal" value={draft.includedGb} onChange={(event) => setDraft({ ...draft, includedGb: event.target.value })} className="mt-1 min-h-10 w-full rounded-[var(--pokrov-radius-control)] border border-[color:var(--atlas-border)] bg-[color:var(--atlas-canvas)] px-3 tabular-nums outline-none focus:border-[color:var(--atlas-focus)]" /></label>
                <div className="grid grid-cols-2 gap-2">
                  <label className="block text-xs font-semibold">День сброса<input aria-label="День сброса квоты" inputMode="numeric" value={draft.resetDay} onChange={(event) => setDraft({ ...draft, resetDay: event.target.value })} className="mt-1 min-h-10 w-full rounded-[var(--pokrov-radius-control)] border border-[color:var(--atlas-border)] bg-[color:var(--atlas-canvas)] px-3 tabular-nums outline-none focus:border-[color:var(--atlas-focus)]" /></label>
                  <label className="block text-xs font-semibold">Часовой пояс<input aria-label="Часовой пояс квоты" value={draft.timezone} onChange={(event) => setDraft({ ...draft, timezone: event.target.value })} className="mt-1 min-h-10 w-full rounded-[var(--pokrov-radius-control)] border border-[color:var(--atlas-border)] bg-[color:var(--atlas-canvas)] px-3 outline-none focus:border-[color:var(--atlas-focus)]" /></label>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <label className="block text-xs font-semibold">Порог предупреждения<input aria-label="Порог предупреждения квоты" inputMode="decimal" value={draft.warningRatio} onChange={(event) => setDraft({ ...draft, warningRatio: event.target.value })} className="mt-1 min-h-10 w-full rounded-[var(--pokrov-radius-control)] border border-[color:var(--atlas-border)] bg-[color:var(--atlas-canvas)] px-3 tabular-nums outline-none focus:border-[color:var(--atlas-focus)]" /></label>
                  <label className="block text-xs font-semibold">Критический порог<input aria-label="Критический порог квоты" inputMode="decimal" value={draft.criticalRatio} onChange={(event) => setDraft({ ...draft, criticalRatio: event.target.value })} className="mt-1 min-h-10 w-full rounded-[var(--pokrov-radius-control)] border border-[color:var(--atlas-border)] bg-[color:var(--atlas-canvas)] px-3 tabular-nums outline-none focus:border-[color:var(--atlas-focus)]" /></label>
                </div>
                <label className="flex min-h-10 items-center gap-2 text-xs font-semibold"><input type="checkbox" checked={draft.enabled} onChange={(event) => setDraft({ ...draft, enabled: event.target.checked })} /> Контроль лимита включён</label>
                <label className="block text-xs font-semibold">Примечание оператора<textarea aria-label="Примечание к квоте" value={draft.notes} onChange={(event) => setDraft({ ...draft, notes: event.target.value })} className="mt-1 min-h-20 w-full rounded-[var(--pokrov-radius-card)] border border-[color:var(--atlas-border)] bg-[color:var(--atlas-canvas)] p-3 outline-none focus:border-[color:var(--atlas-focus)]" /></label>
