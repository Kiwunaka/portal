...
Вкладка `События приложения` в карточке пользователя показывает до 100
последних Event Envelope V1 для этого пользователя или его account identity:
время события и приёма, платформу и версию, этап, результат, нормализованный
код ошибки, повторяемость и длительность. Произвольный `meta_json`, session и
device identifiers, trace, адреса сайтов, конфиги, ключи и токены в браузер не
возвращаются.
