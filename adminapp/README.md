...
В карточке обращения каждый пакет поддержки можно связать с отдельной
попыткой подключения. Выбор проходит через подтверждение действия; связь
помечена как указанная оператором и сохраняется независимо от попытки тикета.
Выбрать можно только попытку из доступной телеметрии владельца обращения.
Отсутствие связи показано явно; существующий доступ к шифротексту не меняется.
...
User 360 и выбранная попытка обращения показывают четыре состояния профиля:
назначен сервером при отчёте, скачан, подготовлен и применён по данным клиента.
Ревизии представлены непрозрачными ссылками, протоколы — фиксированными именами.
Расхождение AWG2/AWG3.1, этап защиты, возраст наблюдения и следующий безопасный
шаг видны в одной карточке. Неизвестное время не считается свежим; отчёт клиента
не заменяет серверное подтверждение трафика. Поздний отчёт с меньшим номером
внутри попытки не перезаписывает её более новое состояние.
...
- Общая кнопка обновления и polling не повторяют запрос ресурса, пока он
  выполняется. Перечитывание после команды принудительное: оно отменяет старый
  запрос, чтобы ответ, начатый до изменения, не подменил результат действия.
- Таблицы платежей, пользователей, онлайн-снимка, аудита и карточки пользователя
  при числе строк больше 40 показывают строки по мере прокрутки. Нативная
  таблица сохраняет общее число строк, их индексы и строку с фокусом; Home/End
  в области прокрутки переходят к началу/концу. «Режим чтения» открывает все
  строки для чтения целиком и поиска браузером.
...
- Графики трафика и воронки загружаются при открытии соответствующего раздела;
  остальные маршруты не загружают Recharts. Пока модуль загружается, показано
...
- карточка заказа показывает серверные причины проверки, отдельные состояния
  claim/grant/outbox, срок доступа, попытки и безопасный код ошибки доставки,
  следующий retry и последние команды с opaque intent reference. Отсутствующая
  запись явно отличается от недоступного поля; выбор другого заказа заменяет
  эти данные. Карточка не восстанавливает статус доступа из статуса оплаты;
- сохранённый расчёт показывает allowlisted условия `order_intent` при создании
  заказа: тариф, сумму, commercial/campaign/terms revisions и срок резерва.
  Это исторический snapshot, а не свежий checkout quote. При отсутствии
  snapshot интерфейс показывает недоступность; owner/subject/token скрыты;
...
Панель отдельно показывает юридический статус профиля, terms revision,
активную и предельную ёмкость, текущую зону и разрешение привлечения по ёмкости.
...
Ограничитель роста отдельно показывает активные права, соединения по данным
узлов, CPU/Мбит/с/потери и нагрузку поддержки. Одновременные устройства без
измерения так и помечаются. Причины остановки и ранний сигнал расширения
берутся из `capacity_automation`; здоровая сеть не отменяет юридические и
...
## Таймаут и отмена загрузки

JSON и file requests сохраняют свой deadline, даже если экран передал
`AbortSignal`. По таймауту API client возвращает `request_timeout`, чтобы
route resource вышел из загрузки и предложил повтор; отмена при уходе с
экрана остаётся отменой и не показывает ошибку на следующем маршруте.
Неопределённый результат отправленной команды требует readback, как и раньше.
