...
from scripts.verify_awg2_contract_sync import verify_awg2_contract_sync
...
def _write_fixture_roots(tmp_path: Path) -> tuple[Path, Path, Path, str]:
    ...
    contract_path = core_root / "config" / "awg2-capability.json"
    platform_path = platform_root / "portal_bot" / "awg2_lab_service.py"
    ...
    platform_path.write_text(
        f'AWG2_CONTRACT_ID = "{CONTRACT_ID}"\n'
        f'AWG2_CONTRACT_SHA256 = (\n    "{digest}"\n)\n',
        encoding="utf-8",
    )
    client_path.write_text(
        f"const _pokrovAwg2ContractId = '{CONTRACT_ID}';\n"
        f"const _pokrovAwg2ContractSha256 =\n    '{digest}';\n",
        encoding="utf-8",
    )
...
def test_rejects_a_stale_consumer_digest(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="contract drift in client"):
        verify_awg2_contract_sync(
            platform_root=platform_root,
            core_root=core_root,
            client_root=client_root,
        )
