...
def test_repository_fixture_has_closed_exact_12_49_12_10_schema(bundle) -> None:
    assert len(bundle.smoke) == 12
    assert len(bundle.normal) == 49
...
def test_deterministic_corpus_runs_boundaries_and_returns_aggregate_only() -> None:
    ...
    assert report["fixture_counts"] == {"smoke": 12, "normal": 49, "adversarial": 12, "sessions": 10}
...
def test_cli_deterministic_prints_one_aggregate_json_document(capsys) -> None:
    ...
    assert report["fixture_counts"]["normal"] == 49
