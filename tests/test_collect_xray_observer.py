...
class CollectXrayObserverTests(unittest.TestCase):
    ...
    def test_parse_xray_access_log_line_extracts_identity_and_source_ip(self) -> None:
        ...
        self.assertEqual(row["occurred_at"], "2023-11-22T17:01:32")
    ...
    def test_collect_observations_uses_cursor_and_aggregates_by_minute_identity_and_ip(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            first = self.collector.collect_observations(log_path=log_path, cursor_path=cursor_path)
            ...
            self.assertEqual(first["observations"][0]["occurred_at"], "2023-11-22T17:01:00")
            self.assertEqual(first["observations"][1]["client_email"], "panel-bob")
            self.assertTrue(cursor_path.exists())

            second = self.collector.collect_observations(log_path=log_path, cursor_path=cursor_path)
            ...
            third = self.collector.collect_observations(log_path=log_path, cursor_path=cursor_path)
            self.assertEqual(len(third["observations"]), 1)
            self.assertEqual(third["parse_error_count"], 0)
            self.assertEqual(third["observations"][0]["occurred_at"], "2023-11-22T17:03:00")
