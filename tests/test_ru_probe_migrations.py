...
def test_sqlite_ru_probe_ddl_follows_admin_ops_economy_and_payment_domains(migrated_engine) -> None:
    statements: list[str] = []
    ...
    economy_first = next(
        index
        for index, sql in enumerate(statements)
        if "CREATE TABLE IF NOT EXISTS connection_evidence" in sql
    )
    payment_last = next(
        index
        for index, sql in enumerate(statements)
        if "ix_payment_entitlement_claim_grant" in sql
    )
    ru_first = next(
        index
        for index, sql in enumerate(statements)
    ...
    assert admin_last < economy_first < payment_last
    assert ru_first == payment_last + 1
...
def test_admin_action_intent_migration_contract(tmp_path: Path) -> None:
    ...
    with engine.begin() as conn:
        existing_node_id = int(
            conn.execute(
                text(
                    "INSERT INTO nodes(code, name, enabled, accepting_new_clients, access_role) "
                    "VALUES ('legacy-map', 'Legacy map', 0, 0, 'paid')"
                )
            ).lastrowid
        )
