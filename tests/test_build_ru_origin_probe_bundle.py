...
def test_signed_candidate9_source_revision_is_explicitly_allowlisted() -> None:
    assert (
        MODULE._validate_source_revision(MODULE.CANDIDATE9_SOURCE_REVISION)
        == MODULE.CANDIDATE9_SOURCE_REVISION
    )
    assert MODULE._candidate_id(MODULE.CANDIDATE9_SOURCE_REVISION) == (
        "pokrov-1.2.0-candidate.9"
    )


def test_signed_candidate10_source_revision_is_explicitly_allowlisted() -> None:
    assert (
        MODULE._validate_source_revision(MODULE.CANDIDATE10_SOURCE_REVISION)
        == MODULE.CANDIDATE10_SOURCE_REVISION
    )
    assert MODULE._candidate_id(MODULE.CANDIDATE10_SOURCE_REVISION) == (
        "pokrov-1.2.0-candidate.10"
    )


def test_signed_candidate16_source_revision_is_explicitly_allowlisted() -> None:
    assert (
        MODULE._validate_source_revision(MODULE.CANDIDATE16_SOURCE_REVISION)
        == MODULE.CANDIDATE16_SOURCE_REVISION
    )
    assert MODULE._candidate_id(MODULE.CANDIDATE16_SOURCE_REVISION) == (
