...
def main() -> int:
    ...
    try:
        print("== status ==")
        print(_run(ssh, "systemctl status x-ui --no-pager | head -120 || true", timeout=60))
        print("== journal ==")
        print(_run(ssh, "journalctl -u x-ui -n 120 --no-pager || true", timeout=60))
        print("== ports ==")
        print(_run(ssh, "ss -tlnp | head -200", timeout=60))
        return 0
    finally:
        ssh.close()
