...
def main() -> int:
    ap = argparse.ArgumentParser(description="Check PostgreSQL availability on brain node.")
    ...
    try:
        checks = [
            "command -v psql >/dev/null 2>&1 && psql --version || echo psql:not_found",
            "systemctl is-active postgresql || true",
            "systemctl is-enabled postgresql 2>/dev/null || true",
            "ss -tlnp | grep -E ':5432\\b' || true",
        ]
        for cmd in checks:
            _, out, err = _run(ssh, cmd, timeout=60)
