...
EXPECTED_PAYLOAD_MAPPING_SHA256 = (
    "114f324acfb06e49f6101ea41474f40c21a79d90cee5a4095c4215b9d21544e8"
)
...
def _payload_mapping_sha256(mappings: list[tuple[str, str]]) -> str:
    encoded = json.dumps(
        mappings,
        separators=(",", ":"),
        ensure_ascii=True,
...
def _revision_paths(repo_root: Path, revision: str) -> set[str]:
    ...
    if result.returncode != 0:
        raise ValueError("reviewed deploy payload revision is unavailable")
    return {
        value.decode("utf-8")
...
def _reviewed_upload_mappings(repo_root: Path) -> list[tuple[str, str]]:
    mappings: list[tuple[str, str]] = []
    live_paths = _revision_paths(repo_root, EXPECTED_LIVE_REVISION)
    candidate_paths = _revision_paths(repo_root, EXPECTED_CANDIDATE_REVISION)
    for source_path, remote_target in iter_upload_mappings(repo_root):
        relative_path = source_path.relative_to(repo_root).as_posix()
        live_has_path = relative_path in live_paths
        candidate_has_path = relative_path in candidate_paths
        if live_has_path and candidate_has_path:
            mappings.append((relative_path, remote_target))
            continue
        if not live_has_path and not candidate_has_path:
            # Later aggregate payload entries are outside this frozen one-file window.
            continue
        raise ValueError(
            f"reviewed live/candidate deploy payload shapes differ at: {relative_path}"
        )
    if len(mappings) != EXPECTED_PAYLOAD_COUNT:
        raise ValueError(f"expected exactly {EXPECTED_PAYLOAD_COUNT} reviewed mappings")
    if _payload_mapping_sha256(mappings) != EXPECTED_PAYLOAD_MAPPING_SHA256:
        raise ValueError("reviewed deploy payload mapping digest changed")
    return mappings


def _source_payload(repo_root: Path, revision: str) -> list[tuple[str, str, bytes]]:
    payload: list[tuple[str, str, bytes]] = []
    for relative_path, remote_target in _reviewed_upload_mappings(repo_root):
        payload.append(
