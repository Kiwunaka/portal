...
def main() -> int:
    ap = argparse.ArgumentParser(description="Copy an env var value inside /root/portal_bot/.env without printing it.")
    ...
    try:
        ...
        try:
            ...
            env = _parse_env(raw)
            if args.src not in env or not env[args.src]:
                raise SystemExit(f"Source env var missing/empty: {args.src}")
            updated = _set_kv(raw, args.dst, env[args.src])
