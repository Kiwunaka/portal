...
def main() -> int:
    ...
    try:
        ...
        _run(ssh, "DEBIAN_FRONTEND=noninteractive apt-get install -y binutils >/dev/null 2>&1 || true", timeout=600)
        print("== strings grep 2096 ==")
        print(_run(ssh, "strings /usr/local/x-ui/x-ui | grep -n '2096' | head -40 || true", timeout=60))
        print("== strings grep XUI_/ENV ==")
        print(
            _run(
                ssh,
                "strings /usr/local/x-ui/x-ui | grep -E 'XUI_|SUB_PORT|SUBPORT|SUBPORT|SUB_.*PORT|subPort' | head -200 || true",
                timeout=60,
            )
        )
        print("== strings grep SUB ==")
        print(_run(ssh, "strings /usr/local/x-ui/x-ui | grep -E 'SUB|sub' | head -80 || true", timeout=60))
        return 0
    finally:
        ssh.close()
