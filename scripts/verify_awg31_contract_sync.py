...
CONTRACT_FILENAME = "config/awg31-capability.json"
PLATFORM_FILENAME = "portal_bot/awg31_lab_service.py"
...
def verify_awg31_contract_sync(
    *, platform_root: Path, core_root: Path, client_root: Path
    ...
    if not contract_id:
        raise ValueError("Core AWG 3.1 contract_id is missing")

    platform_source = _read_text(platform_path)
    platform_id = _extract(
        r'^AWG31_CONTRACT_ID\s*=\s*"([^"]+)"$',
        platform_source,
        label="platform AWG31_CONTRACT_ID",
    )
    platform_sha = _extract(
        r'^AWG31_CONTRACT_SHA256\s*=\s*\(\s*"([0-9a-f]{64})"\s*\)',
        platform_source,
        label="platform AWG31_CONTRACT_SHA256",
    )

    client_source = _read_text(client_path)
    client_id = _extract(
        r"^const _pokrovAwg31ContractId\s*=\s*'([^']+)';$",
        client_source,
        label="client _pokrovAwg31ContractId",
    )
    client_sha = _extract(
        r"^const _pokrovAwg31ContractSha256\s*=\s*\n\s*'([0-9a-f]{64})';$",
        client_source,
        label="client _pokrovAwg31ContractSha256",
    )
    ...
    if mismatches:
        ...
        raise ValueError(
            f"AWG 3.1 contract drift in {joined}: expected {contract_id} "
            f"sha256={contract_sha256}"
        )

    return {
        "schema": "pokrov-awg31-contract-sync/v1",
        "status": "PASS",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Verify the pinned AWG 3.1 contract across Core, platform and client."
        )
    )
