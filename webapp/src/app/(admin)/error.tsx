...
export default function AdminError({ error, reset }: ErrorProps) {
  ...
  return (
    <ShellBoundary
      ...
      badgeLabel={error.digest ? `digest ${error.digest.slice(0, 8)}` : "admin error"}
      badgeTone="warning"
      primaryAction={{ label: "Повторить", onClick: reset }}
      secondaryAction={{ label: "К сводке", href: "/admin/dashboard" }}
      metrics={[
        { label: "Область", value: "admin", hint: "Проблема возникла внутри операторского интерфейса." },
        { label: "Действие", value: "retry", hint: "Повторный запрос часто возвращает экран без полного выхода." },
      ]}
      steps={[
