...
function Simulation({ visual }: { visual: GuideVisualSpec }) {
  ...
  return (
    <div
      className={`mx-auto overflow-hidden border border-line bg-canvas shadow-soft ${
        narrow
          ? "w-full max-w-[260px] rounded-[26px]"
          : "w-full rounded-[16px]"
      }`}
    >
      ...
      <div className="bg-[radial-gradient(circle_at_top_right,rgba(50,116,229,0.16),transparent_42%)] p-3.5">
        ...
        <div className="mt-3 grid gap-2">
          {visual.path.map((item, index) => {
            ...
            return (
              <div
                key={item}
                className={`flex min-h-10 items-center gap-2.5 rounded-[11px] border px-3 py-2 text-xs ${
                  target
              ...
              >
                ...
                {target ? (
                  <span className="ml-auto shrink-0 rounded-full bg-brand px-2 py-1 text-[9px] font-bold text-white uppercase">
                    Нажать
                  </span>
                ) : null}
              </div>
    ...
    <div className={`grid gap-3 ${columns}`}>
      {screenshots.map((screenshot, index) => (
        <figure
          ...
          className="min-w-0 rounded-control border border-line bg-surface p-3"
        >
          ...
          <div className="mt-3 flex justify-center overflow-hidden rounded-[11px] bg-[#111217]">
...
export function GuideVisual({ guideId }: { guideId: string }) {
  ...
  return (
    <section
      className="rounded-control border border-line bg-canvas-alt p-3"
      aria-label={`Визуальная инструкция: ${visual.client}, ${visual.screen}`}
      data-testid={`guide-visual-${guideId}`}
    >
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <span className="rounded-full bg-ink px-2.5 py-1 text-[9px] font-bold tracking-[0.08em] text-white uppercase">
          {visual.client}
        </span>
        <span className="rounded-full border border-line bg-surface px-2.5 py-1 text-[9px] font-semibold text-ink-soft">
          {visual.platform}
        </span>
