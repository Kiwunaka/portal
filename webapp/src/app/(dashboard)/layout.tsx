    ...
    return (
      <main className="mx-auto grid min-h-[70vh] w-[min(96vw,720px)] place-items-center py-8">
        <section className="glass-card w-full p-7">
          <p className="font-mono text-xs uppercase tracking-[0.16em] text-emerald-600 dark:text-amber-200">pokrov webapp</p>
  ...
  if (webLoginRequired) {
    return (
      <main className="mx-auto grid min-h-[70vh] w-[min(96vw,680px)] place-items-center py-8">
        <section className="glass-card w-full p-7">
          <p className="font-mono text-xs uppercase tracking-[0.16em] text-emerald-600 dark:text-amber-200">[web login]</p>
  ...
  return (
    <div className="relative min-h-screen overflow-x-clip" style={{ minHeight: "var(--tg-viewport-height, 100dvh)" }}>
      <div className="mx-auto flex min-h-screen min-w-0 max-w-[1500px]">
        {!isAdminRoute ? (
          <aside className="hidden w-72 flex-col justify-between border-r border-white/40 px-5 py-7 dark:border-white/8 lg:flex">
          <div>
            <div className="mb-9 flex items-center gap-3 px-3">
              <span className="material-symbols-rounded rounded-2xl bg-emerald-500/12 p-2.5 text-2xl text-emerald-700 dark:bg-amber-400/10 dark:text-amber-200">shield</span>
              <div>
                <p className="font-display text-xl font-bold tracking-[0.12em]">POKROV</p>
                <p className="font-mono text-[11px] uppercase tracking-[0.16em] text-slate-500">cabinet</p>
              </div>
            </div>
                ...
                return (
                  <AppRouteLink
                    ...
                    className={`haptic-tap flex items-center gap-3 rounded-xl px-4 py-3 text-sm transition ${
                      selected
                        ? "glass-card border border-emerald-200/70 text-emerald-800 shadow-[0_16px_30px_-22px_rgba(23,107,77,0.7)] dark:border-amber-400/16 dark:text-amber-100"
                        : "text-slate-600 hover:bg-white/70 hover:text-emerald-700 dark:text-slate-300 dark:hover:bg-white/5 dark:hover:text-amber-100"
                    }`}
        ...
        >
          <header className="glass-card mb-5 flex items-center justify-between px-4 py-3">
            <div className="flex items-center gap-3">
              ...
              <p className="font-mono text-[11px] uppercase tracking-[0.15em] text-slate-500 md:text-xs">кабинет / pokrov</p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 rounded-full bg-white/80 px-2 py-1.5 text-sm shadow-sm dark:bg-white/10">
                <span className="grid h-8 w-8 place-items-center rounded-full bg-gradient-to-br from-emerald-700 to-amber-500 text-white">{String(user.tg_id).slice(-1)}</span>
        ...
        {navItems.map((item) => {
          ...
          return (
            <AppRouteLink
              key={item.href}
              href={item.href}
              className={`haptic-tap flex min-w-12 flex-col items-center text-[10px] ${selected ? "text-emerald-700 dark:text-amber-100" : "text-slate-500"}`}
              aria-current={selected ? "page" : undefined}
            >
          ...
          >
            <div className="mb-6 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="material-symbols-rounded rounded-2xl bg-emerald-500/12 p-2 text-emerald-700 dark:bg-amber-400/10 dark:text-amber-200">shield</span>
                <div>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <AppRouteLink
                    ...
                    href={item.href}
                    onClick={() => setMobileMenuPath(null)}
                    className={`haptic-tap flex items-center gap-3 rounded-xl px-3 py-3 text-sm ${
                      selected
