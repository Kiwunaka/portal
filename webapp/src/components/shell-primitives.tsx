  ...
  return (
    <div className={cn("flex flex-wrap items-start justify-between gap-4", className)}>
      <div className="min-w-0">
        {eyebrow ? <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">{eyebrow}</p> : null}
...
const METRIC_TONE_CLASSES: Record<NonNullable<MetricCardProps["tone"]>, string> = {
  emerald: "border-emerald-200/60 bg-emerald-50/80 text-emerald-700 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-200",
  amber: "border-amber-200/60 bg-amber-50/80 text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200",
  rose: "border-rose-200/60 bg-rose-50/80 text-rose-700 dark:border-rose-500/30 dark:bg-rose-500/10 dark:text-rose-200",
  slate: "border-white/70 bg-white/72 text-slate-700 dark:border-white/10 dark:bg-white/[0.04] dark:text-slate-200",
};
...
export function MetricCard({ label, value, hint, tone = "slate" }: MetricCardProps) {
  return (
    <article className={cn("stat-card overflow-hidden rounded-[1.2rem] p-4", METRIC_TONE_CLASSES[tone])}>
      ...
      <p className="mt-2 text-2xl font-semibold leading-none text-slate-900 dark:text-slate-50">{value}</p>
      {hint ? <p className="mt-2 text-[11px] leading-5 text-slate-500 dark:text-slate-400">{hint}</p> : null}
    </article>
  );
}
...
export function DialogShell({ title, description, eyebrow, actions, children, className }: DialogShellProps) {
  return (
    <Surface className={cn("w-full overflow-hidden border border-white/70 p-6 dark:border-[#243129]/80 sm:p-8", className)}>
      {(title || description) ? (
        <div className="mb-6">
...
export function FormField({ label, hint, error, required, className, children }: FormFieldProps) {
  return (
    <label className={cn("block space-y-2", className)}>
      ...
      {children}
      {hint ? <span className="block text-xs leading-5 text-slate-500 dark:text-slate-400">{hint}</span> : null}
      {error ? <span className="block text-xs leading-5 text-rose-500">{error}</span> : null}
    </label>
  );
}
...
export function EmptyState({ icon, title, description, actions, className }: EmptyStateProps) {
  return (
    <div className={cn("empty-state rounded-[1.75rem] border border-dashed border-white/55 bg-white/40 dark:border-white/10 dark:bg-white/[0.03]", className)}>
      <div className="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-emerald-900/10 text-emerald-800 dark:bg-emerald-200/10 dark:text-emerald-200">
        {icon}
      </div>
      <h2 className="mt-4 font-display text-2xl font-semibold text-slate-900 dark:text-slate-50">{title}</h2>
      {description ? <p className="mt-2 max-w-xl text-sm leading-7 text-slate-600 dark:text-slate-300">{description}</p> : null}
  ...
  return (
    <ol className={cn("space-y-3", className)}>
      {items.map((item, index) => (
        <li key={index} className="flex gap-3 rounded-[1.15rem] border border-white/60 bg-white/65 p-4 dark:border-white/10 dark:bg-white/[0.04]">
