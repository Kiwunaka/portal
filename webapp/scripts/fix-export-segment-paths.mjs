// Workaround for a Next 16 static-export mismatch with route groups:
// the client segment cache requests dot-joined RSC paths like
//   /devices/__next.!KGRhc2hib2FyZCk.devices.__PAGE__.txt
// while `output: "export"` writes them as nested directories:
//   /devices/__next.!KGRhc2hib2FyZCk/devices/__PAGE__.txt
// Routes without groups export dotted files directly (marketing is unaffected).
// This script adds dot-joined copies next to each `__next.*` directory so any
// static file server (serve_export.py locally, Caddy in production) serves them.
