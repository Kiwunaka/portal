...
from admin_action_intent_runtime import (
    ACTION_INTENT_TTL as ACTION_INTENT_TTL,
    EXTERNAL_EXECUTION_STALE_AFTER as EXTERNAL_EXECUTION_STALE_AFTER,
    EXTERNAL_EXECUTION_TIMEOUT_SECONDS as EXTERNAL_EXECUTION_TIMEOUT_SECONDS,
    NODE_MAPPING_LOCK_NAMESPACE as NODE_MAPPING_LOCK_NAMESPACE,
    ActionIntentError as ActionIntentError,
    ActionPolicy as ActionPolicy,
    AuditActionBuilder as AuditActionBuilder,
    AuditMetaBuilder as AuditMetaBuilder,
    AuditTargetBuilder as AuditTargetBuilder,
    ChallengeBuilder as ChallengeBuilder,
    DbExecutor as DbExecutor,
    EntityState as EntityState,
    EntityStateBuilder as EntityStateBuilder,
    ExecutionStateBuilder as ExecutionStateBuilder,
    ExecutorKindBuilder as ExecutorKindBuilder,
    ExternalContextBuilder as ExternalContextBuilder,
    ExternalOutcome as ExternalOutcome,
    PayloadNormalizer as PayloadNormalizer,
    PostCommitContextBuilder as PostCommitContextBuilder,
    PreviewBuilder as PreviewBuilder,
    ReplayResultBuilder as ReplayResultBuilder,
    ResultSanitizer as ResultSanitizer,
    RuntimeDbExecutor as RuntimeDbExecutor,
    _INTERNAL_ISSUED_CARD_IDS_KEY,
    ...
    _semantic_hash,
    _utcnow,
    action_intent_error_identifiers as action_intent_error_identifiers,
    confirmation_sha256 as confirmation_sha256,
    execute_action_intent as _execute_action_intent_runtime,
    get_action_intent_status as _get_action_intent_status_runtime,
    node_resync_recipient_fingerprint,
    prepare_action_intent as _prepare_action_intent_runtime,
)
...
def _normalize_ticket_expected_version(value: object) -> int:
    try:
        parsed = int(value)
    except (TypeError, ValueError):
        raise ActionIntentError("invalid_payload", status_code=422, message="Версия обращения должна быть положительным числом.")
    if parsed < 1:
        raise ActionIntentError("invalid_payload", status_code=422, message="Версия обращения должна быть положительным числом.")
    return parsed
...
def _normalize_ticket_reply_runtime(payload: Mapping[str, Any]) -> dict[str, Any]:
    allowed = {"body", "attachment_id", "media_type", "media_file_id", "media_payload", "expected_version", "macro_code"}
    _reject_extra_payload_fields(payload, allowed)
    ...
    if expected_version is not None:
        out["expected_version"] = _normalize_ticket_expected_version(expected_version)
    macro_code = payload.get("macro_code")
    out["macro_code"] = (
        _normalize_identifier(macro_code, field="macro_code", minimum=2, maximum=48)
        if macro_code is not None and str(macro_code).strip()
        else None
    )
    return out
...
def _normalize_ticket_reply_payload(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if runtime.get("expected_version") is not None:
        result["expected_version"] = int(runtime["expected_version"])
    return result


def _normalize_ticket_status_payload(payload: Mapping[str, Any]) -> dict[str, Any]:
    _reject_extra_payload_fields(payload, {"status", "expected_version"})
    ...
    if payload.get("expected_version") is not None:
        result["expected_version"] = _normalize_ticket_expected_version(payload.get("expected_version"))
    return result
...
def _ticket_entity_state(
    ...
    current_version = max(1, int(getattr(ticket, "version", 1) or 1))
    if payload.get("expected_version") is not None and current_version != int(payload["expected_version"]):
        raise ActionIntentError(
            "stale_version",
            status_code=409,
...
def _ticket_external_context(state: EntityState, payload: Mapping[str, Any]) -> dict[str, Any]:
    result = {
        "ticket_id": int(state.context["ticket_id"]),
        "user_tg_id": int(state.context["user_tg_id"]),
    }
    if payload.get("expected_version") is not None:
        result["expected_version"] = int(payload["expected_version"])
    return result
...
def _ticket_audit_meta(state: EntityState, payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if payload.get("expected_version") is not None:
        meta["expected_version"] = int(payload["expected_version"])
    if payload.get("macro_code"):
        meta["macro_code"] = str(payload["macro_code"])
...
from operator_work_actions import build_operator_action_policies
from operator_money_actions import build_money_action_policies
from operator_governance_actions import build_governance_action_policies
from operator_release_actions import build_release_action_policies
from support_work_actions import build_support_action_policies

ACTION_POLICIES.update(
    build_operator_action_policies(
        ActionPolicy=ActionPolicy,
        EntityState=EntityState,
        ActionIntentError=ActionIntentError,
    )
)
ACTION_POLICIES.update(
    build_money_action_policies(
        ActionPolicy=ActionPolicy,
        EntityState=EntityState,
        ActionIntentError=ActionIntentError,
    )
...
ACTION_POLICIES.update(
    build_governance_action_policies(
        ActionPolicy=ActionPolicy,
        EntityState=EntityState,
        ActionIntentError=ActionIntentError,
    )
...
def _normalize_campaign_runtime(payload: Mapping[str, Any], *, partial: bool) -> dict[str, Any]:
    ...
    if "expected_revision" in payload:
        out["expected_revision"] = _normalize_promo_integer(
    ...
    for field, choices, default in (
        ("objective", CAMPAIGN_OBJECTIVES, "retention"),
        ("lifecycle_status", CAMPAIGN_LIFECYCLE_STATUSES, "draft"),
        ("legal_profile_status", CAMPAIGN_LEGAL_PROFILE_STATUSES, "missing"),
    ):
        if field in payload or not partial:
            value = str(payload.get(field, default) or "").strip().lower()
            if value not in choices:
                raise ActionIntentError(
                    "invalid_payload",
                    status_code=422,
                    message=f"{field} не входит в закрытый серверный allowlist.",
                )
            out[field] = value
    if "commercial_revision" in payload or not partial:
        out["commercial_revision"] = _bounded_text(
            payload.get("commercial_revision"),
            field="commercial_revision",
            minimum=1,
            maximum=32,
        )
    if "channels" in payload or not partial:
        raw_channels = payload.get("channels") or []
        if not isinstance(raw_channels, list) or len(raw_channels) > 16:
            raise ActionIntentError(
                "invalid_payload",
                status_code=422,
                message="channels должен быть ограниченным списком каналов.",
            )
        channels = normalize_campaign_channels(raw_channels)
        if len(channels) != len({str(item or "").strip().lower() for item in raw_channels}) or set(channels) - CAMPAIGN_CHANNELS:
            raise ActionIntentError(
                "invalid_payload",
                status_code=422,
                message="channels содержит неизвестный или пустой канал.",
            )
        out["channels"] = channels
    for field in ("seller_profile_id", "terms_revision"):
        if field in payload or not partial:
            value = payload.get(field)
            out[field] = (
                _bounded_text(value, field=field, minimum=1, maximum=64)
                if value is not None
                else None
            )
    if "paid_cap" in payload or not partial:
        out["paid_cap"] = _normalize_promo_integer(
            payload.get("paid_cap", 0),
            field="paid_cap",
            minimum=0,
            maximum=1_000_000,
        )
    if "capacity_guard_enabled" in payload or not partial:
        value = payload.get("capacity_guard_enabled", True)
        if type(value) is not bool:
            raise ActionIntentError(
                "invalid_payload",
                status_code=422,
                message="capacity_guard_enabled должен быть bool.",
            )
        out["capacity_guard_enabled"] = bool(value)
    if "state_reason" in payload:
        value = payload.get("state_reason")
        if value is not None:
            normalized_reason = str(value or "").strip().lower()
            if normalized_reason not in CAMPAIGN_STATE_REASONS:
                raise ActionIntentError(
                    "invalid_payload",
                    status_code=422,
                    message="state_reason не входит в закрытый серверный allowlist.",
                )
            out["state_reason"] = normalized_reason
    ...
    if "auto_disable" in payload or not partial:
        value = payload.get("auto_disable", True)
        if type(value) is not bool:
            raise ActionIntentError("invalid_payload", status_code=422, message="auto_disable должен быть bool.")
        out["auto_disable"] = bool(value)
    if "is_active" in payload and payload.get("is_active") is not None:
        value = payload.get("is_active")
        if type(value) is not bool:
            raise ActionIntentError("invalid_payload", status_code=422, message="is_active должен быть bool.")
        implied_status = "live" if value else "paused"
        explicit_status = out.get("lifecycle_status")
        if explicit_status is not None and explicit_status != implied_status:
            raise ActionIntentError(
                "invalid_payload",
                status_code=422,
                message="is_active противоречит lifecycle_status.",
            )
        out["lifecycle_status"] = implied_status
...
def _campaign_model_snapshot(row: IncentiveCampaign | None) -> dict[str, Any] | None:
    snapshot = _safe_model_snapshot(row, _CAMPAIGN_FIELDS)
    if snapshot is None:
        return None
    snapshot["channels"] = normalize_campaign_channels(snapshot.pop("channels_json", None))
    if isinstance(snapshot.get("target_value"), str):
        snapshot["target_value"] = _redacted_text(str(snapshot["target_value"]))
    return snapshot


def _campaign_runtime_record(
    *,
    row: IncentiveCampaign | None,
    payload: Mapping[str, Any],
    public_id: str,
    revision: int,
) -> dict[str, Any]:
    ...
    record.update({key: value for key, value in payload.items() if key != "expected_revision"})
    return record
...
def _campaign_state(
    ...
    *,
    ...
    if mode == "create":
        ...
        challenge = "new"
        public_id = "cmp_" + _semantic_hash(
            "campaign-public-id",
            {"payload": dict(payload), "count": int(count or 0), "max_id": int(max_id or 0)},
        )[:32]
        next_revision = 1
    else:
        ...
        if mode == "update" and str(getattr(row, "lifecycle_status", "") or "").strip().lower() == "killed":
            raise ActionIntentError(
                "campaign_killed",
                status_code=409,
                message="Убитая кампания неизменяема; создайте новую кампанию.",
            )
        expected_revision = payload.get("expected_revision")
        current_revision = max(1, int(getattr(row, "revision", 1) or 1))
        if expected_revision is not None and int(expected_revision) != current_revision:
            raise ActionIntentError(
                "stale_campaign_revision",
                status_code=409,
                message="Ревизия кампании изменилась; подготовьте новый preview.",
            )
        before = _campaign_model_snapshot(row)
        version = before
    ...
    if mode == "delete":
        runtime_record["lifecycle_status"] = "killed"
        runtime_record["state_reason"] = "owner_killed"
    resuming_from_capacity_pause = bool(
        row is not None
        and str(getattr(row, "lifecycle_status", "") or "").strip().lower() == "paused"
        and str(getattr(row, "state_reason", "") or "").strip().lower()
        in {"capacity_forbidden", "capacity_resume_hysteresis"}
        and str(runtime_record.get("lifecycle_status") or "").strip().lower() == "live"
    )
    ...
    safe_runtime["policy"] = decision
    after = safe_runtime
    return EntityState(
        entity=row,
        version_snapshot={"mode": mode, "row": version, "authority": authority},
        public_snapshot=before or {"exists": False},
        context={
            "mode": mode,
            "after_snapshot": after,
            "after_runtime": runtime_record,
            "policy": decision,
            "challenge": challenge,
        },
    )
...
def _campaign_change_preview(state: EntityState, _payload: Mapping[str, Any]) -> dict[str, Any]:
    decision = dict(state.context.get("policy") or {})
    warnings = [
        "Ревизия кампании, commercial contract и entitlement-ёмкость зафиксированы сервером до confirm.",
        "Live разрешается только при пустом blocking_reasons; repository preview не означает юридическое одобрение или запуск.",
    ]
    if decision.get("blocking_reasons"):
        warnings.append(
            "Активация сейчас заблокирована: "
            + ", ".join(str(item) for item in decision["blocking_reasons"])
        )
    return _simple_preview(
        f"Изменение {state.context['mode']} для кампании {state.context.get('challenge')}",
        state.public_snapshot,
        state.context.get("after_snapshot"),
        warnings,
    )
...
def _normalize_awg2_lab_material_runtime(payload: Mapping[str, Any]) -> dict[str, Any]:
    _reject_extra_payload_fields(
        payload,
        {
            "tg_id",
    ...
    try:
        endpoint = validate_awg2_endpoint(endpoint)
    except Awg2LabError:
        raise ActionIntentError(
...
def _normalize_awg2_lab_material_payload(payload: Mapping[str, Any]) -> dict[str, Any]:
    runtime = _normalize_awg2_lab_material_runtime(payload)
    return {
        "tg_id": int(runtime["tg_id"]),
        ...
        "endpoint": _json_fingerprint("admin-awg2-lab-endpoint", runtime["endpoint"]),
    }


def _normalize_awg31_lab_material_runtime(payload: Mapping[str, Any]) -> dict[str, Any]:
    _reject_extra_payload_fields(
        payload,
        {
            "tg_id",
    ...
    endpoint = _normalize_json_object(payload.get("endpoint") or {}, field="endpoint", maximum_bytes=100_000)
    try:
        endpoint = validate_awg31_endpoint(endpoint)
...
def _normalize_awg31_lab_material_payload(payload: Mapping[str, Any]) -> dict[str, Any]:
    runtime = _normalize_awg31_lab_material_runtime(payload)
    return {
        "tg_id": int(runtime["tg_id"]),
        "install_id": _redacted_text(str(runtime["install_id"])),
        "generation": str(runtime["generation"]),
        ...
        "server_record_id": str(runtime["server_record_id"]),
        "node_code": str(runtime["node_code"]),
        "endpoint": _json_fingerprint("admin-awg31-lab-endpoint", runtime["endpoint"]),
    }
...
def _awg2_lab_material_state(session, target_id: str, payload: Mapping[str, Any], for_update: bool) -> EntityState:
    ...
    query = session.query(Awg2LabMaterial).filter(
        Awg2LabMaterial.tg_id == tg_id,
        Awg2LabMaterial.install_id == install_id,
        Awg2LabMaterial.is_active.is_(True),
    )
    if for_update and str(session.get_bind().dialect.name) == "postgresql":
        query = query.with_for_update()
    materials = query.order_by(Awg2LabMaterial.id.asc()).all()
    before = {
        "tg_id": tg_id,
        "install_id_sha256": _semantic_hash("admin-awg2-lab-install", install_id),
        "active_materials": [
            {
                "id": int(row.id),
    ...
    after = {
        ...
        "endpoint": _json_fingerprint("admin-awg2-lab-endpoint", payload["endpoint"]),
    }
    return EntityState(
        entity=user,
        version_snapshot={"user_active": bool(user.is_active), **before},
...
def _awg31_lab_material_state(session, target_id: str, payload: Mapping[str, Any], for_update: bool) -> EntityState:
    tg_id = _integer_target(target_id, positive=True)
    if int(payload.get("tg_id") or 0) != tg_id:
        raise ActionIntentError("invalid_target", status_code=422, message="Цель должна совпадать с tg_id материала.")
    user = _row_for_update(session.query(User).filter(User.tg_id == tg_id), session, for_update)
    ...
    query = session.query(Awg31LabMaterial).filter(
        Awg31LabMaterial.tg_id == tg_id,
        Awg31LabMaterial.install_id == install_id,
        Awg31LabMaterial.is_active.is_(True),
    )
    if for_update and str(session.get_bind().dialect.name) == "postgresql":
        query = query.with_for_update()
    materials = query.order_by(Awg31LabMaterial.id.asc()).all()
    before = {
        "tg_id": tg_id,
        "install_id_sha256": _semantic_hash("admin-awg31-lab-install", install_id),
        "active_materials": [
            {
                "id": int(row.id),
                "material_hash": str(row.material_hash),
...
def _sanitize_awg2_lab_material_result(result: Mapping[str, Any]) -> dict[str, Any]:
    safe = _result_envelope(result)
    material = result.get("material")
    if not isinstance(material, Mapping):
        raise ActionIntentError(
            "invalid_executor_result",
            status_code=500,
            message="Исполнитель AWG2 lab вернул некорректный результат.",
        )
    safe["material"] = {
        key: material[key]
        for key in (
    ...
    if "install_id_sha256" not in safe["material"]:
        safe["material"]["install_id_sha256"] = (
            _semantic_hash("admin-awg2-lab-install", str(install_id))
            if isinstance(install_id, str) and install_id
            else None
        )
    return safe
...
def _sanitize_awg31_lab_material_result(result: Mapping[str, Any]) -> dict[str, Any]:
    safe = _result_envelope(result)
    material = result.get("material")
    if not isinstance(material, Mapping):
        raise ActionIntentError(
            "invalid_executor_result",
            status_code=500,
            message="Исполнитель AWG 3.1 lab вернул некорректный результат.",
        )
    safe["material"] = {
        key: material[key]
        for key in (
...
ACTION_POLICIES.update(
    {
        ...
        "awg2_lab_material.replace": ActionPolicy(
            action="awg2_lab_material.replace",
            target_type="awg2_lab_material",
            risk_level="L3",
            payload_normalizer=_normalize_awg2_lab_material_payload,
            runtime_payload_normalizer=_normalize_awg2_lab_material_runtime,
            entity_state_builder=_awg2_lab_material_state,
            preview_builder=_model_change_preview,
            challenge_kind="exact_tg_id",
            challenge_builder=_context_target_challenge,
            executor_kind="db",
            audit_action="admin_awg2_lab_material_put",
            audit_target_builder=_audit_warp_target,
            result_sanitizer=_sanitize_awg2_lab_material_result,
        ),
        "awg31_lab_material.replace": ActionPolicy(
            action="awg31_lab_material.replace",
            target_type="awg31_lab_material",
            risk_level="L3",
            payload_normalizer=_normalize_awg31_lab_material_payload,
            runtime_payload_normalizer=_normalize_awg31_lab_material_runtime,
            entity_state_builder=_awg31_lab_material_state,
            preview_builder=_model_change_preview,
            challenge_kind="exact_tg_id",
            challenge_builder=_context_target_challenge,
            executor_kind="db",
            audit_action="admin_awg31_lab_material_put",
            audit_target_builder=_audit_warp_target,
        ...
        "campaign.create": ActionPolicy(
            ...
            preview_builder=_campaign_change_preview,
            challenge_kind="exact_phrase",
            challenge_builder=_l2_challenge,
            executor_kind="db",
...
ACTION_POLICY_ROUTES: dict[str, tuple[tuple[str, str], ...]] = {
    ...
    "program_application.review": (("POST", "/api/admin/program-applications/{application_id}/review"),),
    ...
    "ticket.note": (("POST", "/api/admin/tickets/{ticket_id}/note"),),
    ...
    "incident.create": (("POST", "/api/admin/service-incidents"),),
    "incident.update": (("POST", "/api/admin/service-incidents/{incident_id}/resolve"),),
    "incident.compensate": (("POST", "/api/admin/service-incidents/{incident_id}/compensate"),),
    "alert.ack": (("POST", "/api/admin/alerts/{alert_id}/ack"),),
}
...
def prepare_action_intent(
    *,
    ...
    return _prepare_action_intent_runtime(
...
def get_action_intent_status(
    *,
    ...
    return _get_action_intent_status_runtime(
        session=session,
        actor_tg_id=actor_tg_id,
        intent_id=intent_id,
    )
...
async def execute_action_intent(
    *,
    ...
    return await _execute_action_intent_runtime(
        policies=ACTION_POLICIES,
        ...
        post_commit_executor=post_commit_executor,
