...
RETENTION_DEFAULT_COPY: dict[str, dict[str, str]] = {
    ...
    "start99_offer": {
        ...
        "b": (
            ...
            "Скидка {discount_pct}% уже ждёт на следующей оплате."
        ),
    },
}
...
RETENTION_DEFAULT_COPY = {
    "welcome": {
        "a": (
            "🚀 5 дней уже активированы.\n\n"
            "Чтобы всё заработало с первого раза:\n"
            "1) Откройте приложение POKROV;\n"
            "2) откройте раздел «Подключение»;\n"
            "3) нажмите «Подключить».\n\n"
            "Если где-то застряли — напишите в поддержку. Мы рядом и обычно помогаем очень быстро."
        ),
        "b": (
            "Подключение обычно занимает 1–2 минуты.\n"
            "Если на старте что-то тормозит:\n"
            "• выберите приложение для своего устройства;\n"
            "• откройте раздел подключения;\n"
            "• если приложение не видит профиль — смело пишите нам.\n"
            "Ответим быстро и по делу."
        ),
    },
    "t3": {
        "a": (
            "⌛ До окончания доступа осталось около 3 дней.\n\n"
            "Если сервис нужен каждый день, лучше продлить заранее и не оставлять это на последний вечер."
        ),
        "b": (
            "📅 Напоминание T-3.\n\n"
            "Продление заранее помогает сохранить привычное подключение без спешки и лишних касаний."
        ),
    },
    "t1": {
        "a": (
            "⏱ До завершения теста остались примерно сутки.\n\n"
            "Если POKROV вам подошёл, лучше открыть продление сейчас, чтобы подключение продолжало работать без пауз."
        ),
        "b": (
            "⚡ T-1: срок доступа заканчивается в ближайшие сутки.\n\n"
    ...
    "t0": {
        ...
        "b": (
            "🔔 Подписка почти завершена.\n\n"
            "Пара минут на продление сейчас сохранит доступ активным."
        ),
    },
    "reactivation": {
        "a": (
        ...
        "b": (
            "🧭 Срок доступа истёк, но его можно восстановить за пару кликов.\n\n"
            "Откройте продление и вернитесь в рабочий режим за пару минут."
        ),
    },
    "start99_offer": {
        "a": (
            "🎁 Специально для вас сохранили персональную скидку {discount_pct}%. Приятный бонус, чтобы купить ключ доступа без лишних раздумий."
        ),
        "b": (
            "⚡ 5 дней уже помогли проверить подключение, можно переходить на полный режим.\n\n"
            "Скидка {discount_pct}% уже ждёт вас на ключ доступа."
        ),
    },
}
...
RETENTION_BUTTONS = {
    ...
    "reactivation": {"a": "🟦 Вернуться в POKROV", "b": "🟦 Проверить подключение"},
    "start99_offer": {"a": "🟦 Купить ключ доступа", "b": "🟦 Забрать предложение"},
}
...
def _load_admin_template_value(*, key: str) -> str | None:
    ...
    try:
        ...
        if not value:
            return None
        try:
            from migrations import RETENTION_TEMPLATE_PRESETS

            seeded_value = str(RETENTION_TEMPLATE_PRESETS.get(cache_key) or "").strip()
            if seeded_value and value == seeded_value:
                return None
        except Exception:
            pass
...
def _retention_text(*, flow: str, variant: str, context: dict[str, str] | None = None) -> str:
    ...
    fallback = (
        ...
        or (get_copy_text(catalog_key, "") if catalog_key else "")
...
async def oto_free_job() -> None:
    while True:
        ...
        for u in users:
            ...
            text = "🎁 Персональное предложение на 2 часа: 1 месяц за 149 ₽.\nМы закрепили его за вами."
            buttons = [[{"text": "🟦 Купить ключ доступа", "url": _bot_pay_url()}]]
