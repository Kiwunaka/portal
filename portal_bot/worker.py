...
RETENTION_DEFAULT_COPY = {
    ...
    "t3": {
        ...
        "b": (
            "📅 Напоминание T-3.\n\n"
            "Продление заранее помогает сохранить привычный сценарий без спешки и лишних касаний."
        ),
    },
    "t1": {
        "a": (
    ...
    "reactivation": {
        ...
        "b": (
            "🧭 Доступ завершился, но восстановить его можно быстро.\n\n"
            "Откройте продление и вернитесь в рабочий режим за пару минут."
        ),
    },
    "start99_offer": {
        "a": (
            "🎁 Специально для вас сохранили персональную скидку {discount_pct}%. Приятный бонус, чтобы перейти на полный доступ без лишних раздумий."
        ),
        "b": (
            "⚡ Тест уже отработал своё, можно переходить на полный режим.\n\n"
            "Скидка {discount_pct}% уже ждёт вас в следующем шаге продления."
        ),
    },
}
...
RETENTION_BUTTONS = {
    ...
    "reactivation": {"a": "🟦 Вернуться в PORTAL", "b": "🟦 Проверить подключение"},
    "start99_offer": {"a": "🟦 Открыть апгрейд", "b": "🟦 Забрать предложение"},
}
