...
def _welcome_text(is_admin: bool) -> str:
    headline = get_copy_text(
        "bot.support.welcome",
        "Поддержка POKROV на связи. Здесь можно открыть новое обращение или продолжить уже начатый диалог.",
    )
    text = (
        f"👨‍💻 *{headline}*\n\n"
        "Опишите, что хотели сделать: открыть приложение, подключиться, активировать ключ или открыть кабинет.\n"
        "Одного сообщения достаточно — так мы быстрее поймём ситуацию.\n"
