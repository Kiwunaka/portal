...
def public_connect_base_url() -> str:
    ...
    if configured:
        return configured
    domain = str(getattr(Settings, "PUBLIC_CONNECT_DOMAIN", "") or "connect.pokrov.space").strip()
    if domain:
        return f"https://{domain}".rstrip("/")
    return "https://connect.pokrov.space"
...
def public_connect_host() -> str:
    ...
    return str(getattr(Settings, "PUBLIC_CONNECT_DOMAIN", "") or "connect.pokrov.space").strip().lower()
