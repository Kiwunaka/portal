...
from hy2_lab_service import (
    ...
    hy2_lab_material_ready,
    hy2_lab_rollout_access,
    normalize_hy2_lab_config,
)
from shared_surface_facts import get_product_facts
from transport_catalog import AWG2_LAB, AWG31_LAB, GRPC_443_PRIMARY, HY2_LAB, LEGACY_REALITY_FALLBACK, OPERATOR_LAB, RESERVE_XHTTP_CDN, RU_BRIDGE_RELAY
...
def default_network_rollout_config() -> dict[str, Any]:
    ...
    return {
        ...
        HY2_LAB: default_hy2_lab_config(),
        "reserve_xhttp_cdn": {
            "enabled": False,
            "allowlist_node_codes": [],
...
def _transport_metadata(transport_profile: str, *, version: str) -> dict[str, str]:
    ...
    if profile == GRPC_443_PRIMARY:
        ...
        engine_hint = "singbox"
    elif profile == HY2_LAB:
        transport_kind = "hysteria2"
        engine_hint = "singbox"
    else:
...
def normalized_network_rollout_config(payload: Any) -> dict[str, Any]:
    ...
    hy2_lab = normalize_hy2_lab_config(src.get(HY2_LAB))
...
def resolved_client_policy(
    *,
    ...
    if transport_profile == AWG2_LAB:
        lab_config = dict(config.get(AWG2_LAB) or {})
        lab_access = awg2_lab_rollout_access(
            lab_config,
    ...
    if transport_profile == AWG31_LAB:
        ...
        if not material_ready:
            transport_profile = LEGACY_REALITY_FALLBACK
    if transport_profile == HY2_LAB:
        lab_config = dict(config.get(HY2_LAB) or {})
        lab_access = hy2_lab_rollout_access(
            lab_config,
            install_id=install_value,
            tg_ids=tg_ids,
            platform=platform,
        ...
        if not material_ready:
            transport_profile = LEGACY_REALITY_FALLBACK
    if transport_profile not in {LEGACY_REALITY_FALLBACK, GRPC_443_PRIMARY, RESERVE_XHTTP_CDN, RU_BRIDGE_RELAY, OPERATOR_LAB, AWG2_LAB, AWG31_LAB, HY2_LAB}:
        transport_profile = LEGACY_REALITY_FALLBACK
    ...
    if transport_profile == HY2_LAB:
        generation = _clean_text((config.get(HY2_LAB) or {}).get("generation"), fallback="unknown")
        transport_meta["profile_revision"] = f"{config['version']}:{HY2_LAB}:{generation}"
...
def transport_node_allowlist(config: dict[str, Any], transport_profile: str) -> list[str]:
    ...
    if _clean_text(transport_profile) == AWG31_LAB:
        ...
        return _normalize_string_list(lab.get("allowlist_node_codes"), lower=True)
    if _clean_text(transport_profile) == HY2_LAB:
        lab = dict(config.get(HY2_LAB) or {})
        return _normalize_string_list(lab.get("allowlist_node_codes"), lower=True)
