...
- smart-connect eligibility is authorized only by a fresh `authenticated_egress_ok=true`. `false`, missing, expired, or unavailable authenticated probe material is a hard rejection even when `edge_reachability_ok=true`.
