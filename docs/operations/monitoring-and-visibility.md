...
- latest per-node health samples use the `ix_node_health_samples_node_sampled_id` B-tree on `(node_code, sampled_at, id)`, which supports the existing descending timestamp/ID order through a backward scan. For a populated PostgreSQL database, create and validate this index with `CREATE INDEX CONCURRENTLY` before source rollout; startup migrations then reuse the existing index. Fresh databases and SQLite/maintenance migrations create the same declared index.
