...
- generated strict-v2 `pokrov-1.2.0-candidate.14` is now the current signed
  exact candidate: six immutable build-4049 artifacts, refreshed platform
  dependency locks, regenerated SBOM/provenance, and the hosted release-index
  signature are retained, while its output remains
  `ACTIONS_ARTIFACT_ONLY` with `promotion_authorized=false`
- candidate.14 is not the distributed public update: no tag, GitHub Release,
  public asset, store object or stable pointer exists; exact LDPlayer install
  and default tunnel/DNS/egress pass, while AWG 3.1 and AWG2 requested profiles
  remain cached default and are non-PASS. Gate F was not generated because the
  required exact ARM64 physical-install binding is absent
...
- `scripts/release_1_2_candidate_rollback_rehearsal.py` executes the client generator and pointer only from private sparse snapshots materialized from the signed client/platform/Core Git objects. It retains the separate harness revision and tool/source hashes, rechecks every snapshot after child execution, and fails before writing evidence if mutable worktree bytes or a changed snapshot reach the rehearsal. Use `--temporary-root` with an existing private directory on the intended evidence disk when the exact pinned Core artifacts should not be staged under the system temp directory.
- Exact-source helpers materialize regular Git blobs through bounded `ls-tree`/`cat-file` reads. They do not use `git archive`, checkout filters, export attributes, symlinks, or gitlinks, so repository-local archive configuration cannot change the bytes represented as the confirmed commit.
...
- Exact candidate.14 retains the owner exception for the EXE bytes
...
- the recorded signing state matches the exact candidate metadata; candidate.14
  Windows EXE SHA-256
