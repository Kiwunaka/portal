...
- Use `scripts/remote_audit_owned_awg_alignment.py` for v2 key, peer, address, S/H, header-protection, content-padding and randomized-trailer alignment; `scripts/remote_probe_owned_awg_udp_path.py` for fixed-size non-secret one-way probes; `scripts/remote_count_owned_awg_packets.py` for address-free direction and tunnel counts; and `scripts/remote_run_owned_awg_core_interop.py` for exact-Core handshakes. The interop script can additionally cross-build the exact clean Core source as a Linux ARM64 test binary and execute it on an explicitly confirmed owned Raspberry Pi SSH alias. That path requires strict host-key input, exact Core revision confirmation, a caller-supplied non-system temp root, Pi 4/aarch64 plus direct-default-route preflight, digest-verified binary transfer, endpoint material over SSH stdin only, and exact `/tmp/pokrov-awg-ru-pi-<uuid>` cleanup. It returns only the Core/binary/material digests and closed outcome fields; no key, endpoint, raw host, test output or temporary path is retained. The counter and Core interop entrypoints accept optional `--json-out` paths so secret-free evidence can be retained without shell redirection. AWG3.1 randomized trailers make fixed-size handshake classification inapplicable, so direction counts and live handshake state are authoritative for that variant.
...
- After the client connects, run `scripts/inspect_android_runtime_profile.py` with the exact ADB serial and `--expected-profile`. It reads the app-private staged config through root ADB, validates that its path stays below that package's `files/pokrov-runtime/` directory and returns only a normalized profile class, type counts and secret-presence booleans. A non-matching cached default/AWG profile exits nonzero. This readback is required before counting a lab connection as AWG2 or AWG3.1; it still does not replace TUN, egress and server-handshake evidence.
...
- retained distributed release: `v1.1.6`; public client package/build line
  `1.1.6+29`
- working source target: `1.2.0+4048`, `PRE_CANDIDATE_LOCAL`,
