...
- `scripts/build_owned_hy2_server_bundle.py` builds the reviewed `infra/owned-hy2/` source twice from clean Core `e8eb772...` with Go `1.25.13`, requires byte-identical Linux/amd64 binaries, runs a same-source synthetic `sing-box check`, retains the GPL notice and emits a deterministic ZIP with no runtime material. `verify` checks the complete member/digest contract; `plan --operation install|rollback` emits sanitized, non-mutating ordered actions. The current local artifact is `pokrov-hy2-server-e8eb772.zip`, size `15229012`, SHA-256 `6877d3f1d73cd7d493d8e4a178aeb78e6476715e95bee6791c2210ce05089332`; its embedded ELF SHA-256 is `026066be82775af0fef6869f38442775b8737695f48f3bdf26d826a2638fcf5e`.
...
- `scripts/check_smart_dns_policy_parity.py` requires the canonical platform policy and active client copy to be byte-identical after explicit UTF-8 CRLF-to-LF canonicalization; BOM, invalid UTF-8 and lone CR fail closed. `scripts/build_owned_smart_dns_server_bundle.py` applies the same canonical form, requires clean exact platform source plus Go `1.25.13`, runs host tests/vet, builds Linux/amd64 CGO-free bytes twice and emits a deterministic inactive bundle outside the repository. Runtime address, hostname, certificate, private key and upstream resolver coordinates are excluded. The current exact-source `8bdc21f9...` bundle is `2912132` bytes with SHA-256 `dbd39d758b16ba433d6557e163cb3a538e19d40ad5145b1e4871a6a659f7afef`; an independent repeat is byte-identical and both copies pass self-verification. The earlier `2d18fd7...` artifact remains historical evidence only.
- `scripts/remote_install_owned_smart_dns_lab.py` owns guarded server PLAN/APPLY/ROLLBACK. PLAN performs strict-host-key Brain/node resolution and returns only sanitized booleans. APPLY additionally requires exact bundle/source/node confirmations, a unique public IPv4 matching the root-only runtime config, an explicit dedicated-node confirmation, an explicit client-selection-disabled attestation and receipt-bound runtime material. It rejects any existing target, TCP `443` or firewall collision, inactive UFW, non-exact config limits/paths or placeholders; it creates a root-only pre-mutation receipt, starts and probes local TLS/DoH before opening UFW, enables last and automatically restores the receipt on failure. Rollback binds the exact receipt/current release, requires client selection disabled, proves the listener absent, restores only receipt-owned state and retains the failed immutable release. Neither PLAN nor this source authorizes APPLY.
- The 2026-08-29 no-mutation PLAN checked all seven active nodes (`de`, `us`, `ru`, `ru_spb`, `pl`, `it`, `nl`) against the exact current bundle. Every node resolved one owned public IPv4 and had active UFW with no Smart DNS targets, but all seven had TCP/443 already owned; runtime material was absent and every report returned `mutation_performed=false`. Smart DNS must not be overlaid onto those frontends. Under the no-purchase policy, an already owned address is eligible only after a separate guarded migration/rollback proves it can be deliberately freed; otherwise the lab stays undeployed. Explicit owner authorization remains mandatory before APPLY. Source, bundle, PLAN, package/device-state or future DNS-answer evidence is not by itself a live service-access, leak, gameplay, RU-origin or release proof.
