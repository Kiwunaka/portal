...
- `scripts/check_smart_dns_policy_parity.py` requires the canonical platform policy and active client copy to be byte-identical. `scripts/build_owned_smart_dns_server_bundle.py` requires clean exact platform source plus Go `1.25.13`, runs host tests/vet, builds Linux/amd64 CGO-free bytes twice and emits a deterministic inactive bundle outside the repository. Runtime address, hostname, certificate, private key and upstream resolver coordinates are excluded.
- Every active delivery node currently has TCP/443 ownership, so Smart DNS must not be overlaid onto those frontends. A separate or deliberately freed owned public IPv4, guarded PLAN/APPLY/ROLLBACK readback and explicit owner authorization are prerequisites. Source, bundle, DNS-answer or Android-install evidence is not a live service-access, leak, gameplay, RU-origin or release proof.
