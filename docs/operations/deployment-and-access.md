...
- legacy cabinet admin GET/HEAD paths `/admin` and `/admin/*` move to the canonical operator origin only after exact frontend/API fingerprint, authenticated parity and rollback evidence. Root/dashboard map to the operator root; other paths and queries retain their suffix. The redirect is 302/no-store. API routes and unsafe methods are outside this matcher. Preserve the prior Caddy config and legacy static/source bytes; revert the scoped matcher to roll routing back. Patch the observed live config when it contains unrelated operator changes instead of replacing it with an older repository snapshot.
...
- `ru_bridge_relay` stays opt-in unless an incident commander explicitly promotes it; app-managed sing-box manifests expose countries at the top level and nested `Обычный` / `Белые списки` endpoint choices under foreign non-US nodes. US and Russian delivery nodes remain direct-only; RU-to-RU detours are not offered. Russian bridge endpoints still serve eligible foreign destinations.
