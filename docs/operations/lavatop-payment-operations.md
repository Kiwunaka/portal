...
When evidence is missing, checkout must show unavailable/degraded state rather than live purchase copy. The active public beta configuration is Lava.top-only; older provider-specific code paths are retained for legacy/reconciliation tests, not for public provider selection.
