...
3. Upload Android and Windows artifacts only after the release handoff is `GO`, or under the narrower artifact-staging authorization described in the unblock packet. Artifact staging creates a public prerelease for smoke only: do not sync `APP_*`, rebuild static download surfaces, or announce the release until URL-only smoke has passed against the client-owned handoff and runtime app-download smoke has passed after sync.
