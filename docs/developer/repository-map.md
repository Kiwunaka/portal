...
| --- | --- |
| Deploy and release | release_orchestrator.py, remote_deploy_brain_portal_code.py, remote_deploy_brain_awg_route_fix.py, remote_brain_runtime_source_probe.py, remote_deploy_brain_static_sites.py |
...
| Owned transport-lab operations | build_owned_hy2_server_bundle.py, build_owned_smart_dns_server_bundle.py, check_smart_dns_policy_parity.py, remote_install_owned_hy2_lab.py, remote_install_owned_smart_dns_lab.py, remote_deploy_brain_awg_route_fix.py, remote_activate_owned_awg_labs.py, remote_rebind_owned_awg_lab_ports.py, remote_ensure_owned_awg_firewall.py, remote_ensure_owned_awg_reply_source.py, remote_bind_owned_awg_lab_device.py, remote_select_owned_awg_lab.py, remote_set_owned_awg_mtu.py, remote_set_owned_awg31_variant.py, remote_audit_owned_awg_alignment.py, remote_probe_owned_awg_udp_path.py, remote_probe_owned_awg_udp_roundtrip.py, remote_count_owned_awg_packets.py, remote_run_owned_awg_core_interop.py |
...
- The latest signed public-index snapshot is private
  ...
  `pokrov-1.2.0-candidate.21` at `1.2.0+4050`. Six artifacts, bounded Windows
  11 upgrade/default runtime, strict-v2 handoff, refreshed SBOM/provenance and
  signed public-index evidence exist. Its exact production service can
  ...
  candidate. Public tag/assets, Store object, stable pointer and promotion
  remain absent.
