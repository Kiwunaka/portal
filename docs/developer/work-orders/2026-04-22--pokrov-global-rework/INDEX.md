...
| Wave objective | `Execute the full POKROV global rework from canon freeze through shared truth extraction, backend/access redesign, app-next alignment, marketing/webapp/admin cleanup, and cutover/deprecation alignment.` |
| Status | `complete` |
| Orchestrator | `Codex in-session execution` |
| Execution model | `dependency-first` |
| Canonical repo lanes in scope | `portal/master` / `POKROV-app/main` / `legacy bridge main when required` |
| Primary scope roots | `AGENTS.md`, `docs/`, `shared/`, `copy/`, `portal_bot/`, `marketing/`, `webapp/`, `app-next/`, `external/client-fork/app/docs/` |
...
| `4` | `WO-004` | `Wave 3 app-next Android+Windows alignment` | `mixed` | `app-next/`, root docs if contracts move | `WO-002`, `WO-003` | `orchestrator` | `complete` | `App-next shell, IA, hidden transport handling, and local Android/Windows verification were aligned to the new canon.` |
...
| `8` | `WO-008` | `Wave 7 cutover and deprecation alignment` | `mixed` | `docs/`, `app-next/`, `external/client-fork/app/docs/` | `WO-004`, `WO-005`, `WO-006`, `WO-007` | `orchestrator` | `complete` | `Canon/docs/work-order alignment now freezes legacy fork as bridge/compatibility and records the remaining POKROV-app bootstrap dependency.` |
...
| `Phase 2` | `Unify access contracts and client shell` | `WO-003, WO-004` | `Shared truth extraction merged` | `Backend and app-next both speak the new access/account model` | `complete` | `Backend contracts and app-next shell now align on app-first/key-first canon.` |
...
- `app-next/` is the temporary bootstrap-source workspace until the new repo exists locally.
- `external/client-fork/app/` is bridge/hotfix and current public Android+Windows release truth until formal cutover.
...
- `app-next` is a viable successor program but not release-ready and not at feature parity with the bridge client yet.
