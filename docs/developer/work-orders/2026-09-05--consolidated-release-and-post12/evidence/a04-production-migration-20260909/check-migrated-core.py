...
data=plan.remote('pokrov-brain',plan.brain,'/root/portal_bot/venv/bin/python')
migrated=json.loads((out/'migration-applied-v2.json').read_text())['material_changes']
assert len(data['records'])==4
for row in data['records']:
 expected=next(r for r in migrated if r['profile']==row['profile'] and r['install_sha256']==row['install_sha256'])
 assert hashlib.sha256(base64.b64decode(plan.pub(row['endpoint']['private_key']))).hexdigest()==expected['new_public_key_sha256']
...
target=out/'migrated-core-interop.json';assert not target.exists()
report={'started_at':datetime.now(timezone.utc).isoformat(),'origin':'RU-origin','execution':'owned RU Pi, synthetic use of exact device material; not installed Android/Windows proof','build':build,'checks':[],'raw_material_retained':False,'status':'RUNNING'}
...
try:
 ...
 for row in data['records']:
  ...
  raw=json.dumps(row['endpoint'],separators=(',',':')).encode()
  result=op._run_process(base+[command],input_text=base64.b64encode(raw).decode()+'\n',timeout=80)
  combined=result.stdout+'\n'+result.stderr;observations=op._interop_mtu_observations(combined)
  passed=result.returncode==0 and '--- PASS: TestOwnedAWGLabAuthenticatedEgress/mtu_1280 ' in combined and set(observations)=={'1280'}
  check={'profile':row['profile'],'install_sha256':row['install_sha256'],'returncode':result.returncode,'status':'PASS' if passed else 'FAIL','classification':op._classify(combined,result.returncode),'output_sha256':hashlib.sha256(combined.encode()).hexdigest(),'mtu_observations':observations}
  report['checks'].append(check);save();print(json.dumps(check),flush=True)
  assert passed
 report['status']='PASS_BOUNDED_FOUR_MIGRATED_KEYS'
