...
from awg2_lab_service import awg2_lab_rollout_access
from awg31_lab_service import awg31_lab_rollout_access
...
with SessionLocal() as session:
    ...
    result['ordinary_synthetic_identity_denied']={name: not gate(config[name],install_id='r12-a01-synthetic-unassigned',tg_ids=[-999001],platform='android') for name,gate in [('awg2_lab',awg2_lab_rollout_access),('awg31_lab',awg31_lab_rollout_access)]}
    result['normalized_source_hashes']={name:hashlib.sha256(open('/root/portal_bot/'+name,'rb').read().replace(bytes([13,10]),bytes([10]))).hexdigest() for name in ['network_rollout.py','awg31_lab_service.py','awg2_lab_service.py']}
    result['status']='PASS_PUBLIC_ACCESS_DENIED' if result['shared_rules_select_lab']==0 and result['lab_cohort_count']==0 and all(x['install_allowlist_count']==0 and x['account_allowlist_count']==0 for x in result['gates'].values()) and all(result['ordinary_synthetic_identity_denied'].values()) else 'REVIEW_REQUIRED'
