...
sys.path.insert(0,str(root/'test-libs'));sys.path.insert(0,str(source/'portal_bot'));os.chdir(source)
initial=json.loads(sys.stdin.readline())
profile=initial['profile'];assert profile in ('awg2','awg31')
DATABASE='portal_r12_a04_http_keys_'+profile+'_20260908_v3'
result_path=root/('http-'+profile+'-result-v3.json')
...
def main():
 ...
 libraries=json.loads((root/'test-libs-manifest.json').read_text())
 for item in libraries['files']:
  assert hashlib.sha256((root/'test-libs'/item['path']).read_bytes()).hexdigest()==item['sha256']
 report['test_library_versions']=libraries['versions']
 ...
 for line in sys.stdin:
  ...
  if phase=='revoke_a':
   ...
   with api.SessionLocal() as s:
    sessions['a']=auth.issue_authenticated_device_session(s,account_id=accounts['a'],install_id='a04-http-a',device_name='Recovered fixture',platform='windows',now=datetime.datetime.now(datetime.timezone.utc).replace(tzinfo=None))
    s.commit()
...
try:main()
 ...
 report['error_frames']=[{'file':Path(f.filename).name,'line':f.lineno,'name':f.name} for f in traceback.extract_tb(exc.__traceback__)[-8:]]
