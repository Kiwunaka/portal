...
output=root/('http-peer-'+profile+'.json');assert not output.exists(),'Preserve prior evidence'
...
with vm.open_sftp() as f:f.put(str(root/'guest-managed-keys.py'),remote+'/guest-managed-keys.py')
vm.close()
vm_args=[str(ssh),'-o','BatchMode=yes','-o','StrictHostKeyChecking=yes','-o','IdentitiesOnly=yes','-o','UserKnownHostsFile='+str(lab/'known_hosts'),'-i',str(lab/'private/client.key'),'-p','55228','root@127.0.0.1','/root/portal_bot/venv/bin/python -B -u '+remote+'/guest-managed-keys.py']
...
try:
 ...
 try:
  vm=paramiko.SSHClient();vm.load_host_keys(str(lab/'known_hosts'))
  vm.connect('127.0.0.1',port=55228,username='root',key_filename=str(lab/'private/client.key'),allow_agent=False,look_for_keys=False,timeout=10)
  with vm.open_sftp() as f:f.get(remote+'/http-'+profile+'-result.json',str(root/('http-'+profile+'-result.json')))
  vm.close()
