...
names=[n for n in names if Path(n).suffix in ('.py','.json','.txt') and
       (n.startswith('shared/') or '/' not in n[len('portal_bot/'):] or n.startswith('portal_bot/admin_v2/'))]
names=sorted(set(names+['portal_bot/awg_lab_key_binding.py']))
inventory=[]
with tarfile.open(archive,'w') as tar:
 for name in names:
  path=platform/name;raw=path.read_bytes()
...
try:
 ...
 with s.open_sftp() as f:
  for name in ('source.tar','source-manifest.json','synthetic-endpoints.json','guest-pg-keys.py'):
...
print(json.dumps({'source_files':len(inventory),'archive_sha256':doc['archive_sha256'],'guest_root':remote,'synthetic_only':True}))
