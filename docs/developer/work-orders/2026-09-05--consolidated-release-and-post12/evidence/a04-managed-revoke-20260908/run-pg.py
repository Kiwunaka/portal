...
try:
    i,o,e=s.exec_command('/root/portal_bot/venv/bin/python -B '+remote+'/guest-pg-race.py',timeout=180)
    ...
    (root/'pg-execution.json').write_bytes((json.dumps({'exit_code':code,'stdout_sha256':hashlib.sha256(out).hexdigest(),'stderr_sha256':hashlib.sha256(err).hexdigest()},indent=2)+'\n').encode())
finally:s.close()
