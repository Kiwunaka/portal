...
created=subprocess.run(['runuser','-u','postgres','--','createdb','-O',dsn.username,DATABASE],capture_output=True)
assert created.returncode==0,'Isolated database creation failed'
restricted=subprocess.run(['runuser','-u','postgres','--','psql','-v','ON_ERROR_STOP=1','-d','postgres','-c','REVOKE ALL ON DATABASE '+DATABASE+' FROM PUBLIC'],capture_output=True)
