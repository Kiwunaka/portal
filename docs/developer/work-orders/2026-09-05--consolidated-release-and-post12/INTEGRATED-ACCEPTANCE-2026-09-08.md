...
| A02/A03, owned AWG | A02 и [A03](EXECUTION-A03-ACCEPTANCE-2026-09-08.md) verified/I4 для declared Core/server/Pi lab scope; текущие validator tests и server/static identity совпали с шестью retained MTU cases | D02 whole-device/path MTU/packet matrix и N03/N08 не закрыты; final candidate gates открыты |
| A01, AWG baseline/scope | [Verified/I4](EXECUTION-A01-ACCEPTANCE-2026-09-08.md): точный pin и отдельные contracts/consumers; действующие gates закрыты пустыми identity allowlists, общих lab rules/cohorts нет | Public scope decision и соответствующая runtime/origin matrix остаются отдельными условиями |
| A04, server peer lifecycle | [Прямой отзыв и замена](EXECUTION-A04-PEER-LIFECYCLE-2026-09-08.md): два протокола, 10 ожидаемых результатов на отдельных временных peers, exact Core `02a091c`; cleanup и исходные server hashes PASS | Ручной `awg` scope; автоматические API/device revoke/expiry и полный A04 остаются открытыми |
