...
| `webapp/src/app/(admin)/admin/layout.tsx` | confirmed | Admin UI checks `user.is_admin` client-side before showing admin routes. | Client-side checks are UX only; security depends on backend guards. |
