...
7. **Hysteria2 is a closed owner lab.** The owner reopened a bounded, default-off managed UDP/QUIC selector/fallback on 2026-08-28. Local client/server source and immutable-artifact/default-off proof is `I3`; it is not the sole bootstrap and has no deployed server, live-handshake, performance, RU-origin or release claim. Gecko, Mimic and port hopping remain monitor-only.
8. **Smart DNS is selective and non-recursive.** The 1.2.0 owner lab may route only selected AI/gaming-service DNS to an exact HTTPS `/dns-query` endpoint and those application connections through the existing direct outbound. The owned server returns only a synthetic allowlisted `A`, forwards opaque TLS by visible allowlisted SNI, refuses recursion and stays default-off. It is not custom cryptography, a general proxy, gameplay support or proof of access.
9. **Marketing is legally and operationally gated.** No broad RF advertising, external campaign launch or spend is authorized without current legal qualification, explicit owner authorization, capacity guard and campaign-to-revenue evidence. The first recommended test remains a bounded winback pilot.
10. **Public platform truth stays narrow.** Android and Windows are public; Apple remains readiness-only unless its canonical evidence changes. Linux remains beta-only if Phase 03 is actually proved.
11. **Manual gates stay manual.** Windows trusted signing, physical-device, clean-host TUN/DNS, RU-origin, provider-dashboard and production-origin checks cannot be promoted by local automation or documentation.
