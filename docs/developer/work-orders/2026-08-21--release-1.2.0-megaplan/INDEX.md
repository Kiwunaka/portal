...
Wave status: `PHASE_11_CANDIDATE33_GATES_A_E_LOCAL_ROLLBACK_PASS_GATE_F_BLOCKED`
Release candidate: `POKROV_1_2_0_CANDIDATE33_PRIVATE_SIGNED_SOURCE_GATES_BLOCKED`
...
WO-013GU is the current exact source Gates A–E, isolated rollback and Gate F
authority. Private signed
...
Candidate.32 remains immutable `NO_GO` history. No deploy, public asset, Store
object, stable pointer, Gate G or production mutation occurs.
...
| 01 | Release manifest, version/provenance contract, CI and stop-ship controls | Candidate.33 is the current private signed supply. Fresh merged-source builds, candidate-specific CycloneDX/SLSA/handoff metadata, all six application artifacts and all eleven Windows runtime files pass strict replay. The exact platform/client/Core tuple and release-index source are signature-bound; Android production signing passes, while Windows remains an explicit unsigned direct-beta owner exception. Exact `WIN-001` passes at `I4`; candidate.33 local STOP-SHIP anchors pass `7/7`, while the aggregate Gate A remains `BLOCKED/I1` on hosted, connected-device and signing boundaries. Release-index source-contract/signer/receipt jobs pass with real steps, while required platform/client jobs remain `BLOCKED_BY_ACCESS_GITHUB_BILLING`. Gate F is `BLOCKED 2/17/0`; `DEP-001` and `WIN-001` are `I4`, and no tag, public asset, deploy, Store object, stable pointer or promotion exists. Candidate.32 remains immutable `NO_GO` history | `WO-002`, `WO-003`, `WO-003B`–`WO-003J`, `WO-013J`–`WO-013P`, `WO-013CX`, `WO-013DC`–`WO-013DE`, `WO-013EA`, `WO-013EB`, `WO-013EH`, `WO-013EI`, `WO-013EV`–`WO-013GU` |
| 02 | Typed connection state, proof-driven green state, core ABI and migrations | Exact candidate.33 Gate B source replay passes platform `70/70`, focused client `111/111` and full client widgets `413/413`; the aggregate remains `BLOCKED/I3` on connected Windows recovery and physical-device false-green boundaries. Its exact Windows update reaches `11/11`, retains the Auto LocalSystem service and keeps the disconnected network contour unchanged. Candidate.32 disconnected status/initialize, clean-install and public-package-migration slices remain predecessor evidence and do not transfer automatically. Exact candidate.33 managed default/AWG3.1/AWG2/Smart-DNS, connected recovery and physical Android false-green/device matrices remain open. ABI v3 stays deferred until after 1.2.0 | `WO-004`, `WO-004A2`, `WO-004B2`, `WO-004D`, `WO-006I`, `WO-013CX`, `WO-013DD`–`WO-013DF`, `WO-013EN`, `WO-013EV`, `WO-013EW`, `WO-013FB`–`WO-013GU` |
| 03 | Windows/Android runtime boundaries; conditional Linux beta foundation | Gates A and C stay `BLOCKED` below candidate proof after exact candidate.33 replay with zero source failures. Exact candidate.33 update reaches `11/11`, service state passes and plain/typed singleton forwarding plus foreground focus pass under `WIN-001`, advancing that row to `I4`. Candidate.32 clean install, ordinary UI/IPC, disconnected initialization/recovery and public-package migration remain predecessor slices. Saved account/profile migration, launch-at-login timing, default/AWG/Smart-DNS connected recovery, connected uninstall, Windows 10, SmartScreen UI, sleep/resume, IPv6 and Android device matrices remain open. Candidate.33 Android has fresh production-signed `5/5` static artifacts but no LDPlayer or physical-device runtime credit. Linux native runtime/packaging remains open and deliberately unshipped | `WO-005`, `WO-005C4`, `WO-005D4`, `WO-005G`, `WO-013CC`–`WO-013CO`, `WO-013CX`, `WO-013DD`–`WO-013DF`, `WO-013DJ`, `WO-013DL`, `WO-013DO`, `WO-013EA`, `WO-013EF`, `WO-013EG`, `WO-013EV`–`WO-013GU` |
...
| 08 | App, cabinet and marketing UX/accessibility/performance reconciliation | Locally complete at `I3`. Exact candidate.33 Gate E replay passes local quality `15/15`, client `413/413`, cabinet `69/69`, Admin v2 SDK `75/75` and static budgets `9/9`. Fresh npm registry audit is access-blocked; all three locks are byte-identical to candidate.32's retained zero-finding audit inputs, but no fresh audit PASS is claimed. `FE_PR/PR-10` points to candidate.33 Gate F `BLOCKED 2/17/0` without level change. Exact current/Brain origin, authenticated journeys, physical accessibility/OEM/scaling, comparable artifact/device/browser performance, support, general RU-origin and post-promotion evidence remain `NOT_RUN` or manual. Gate E stays `BLOCKED` below I4 | `WO-010`, `WO-013AN`, `WO-013CI`, `WO-013CO`, `WO-013DA`, `WO-013DD`, `WO-013DE`, `WO-013DQ`, `WO-013DS`, `WO-013EL`, `WO-013EN`, `WO-013EO`, `WO-013FD`–`WO-013FH`, `WO-013FV`, `WO-013FX`, `WO-013GA`, `WO-013GF`, `WO-013GG`, `WO-013GI`, `WO-013GT`, `WO-013GU` |
...
| 10 | FRKN-derived rules and isolated AWG2/AWG3.1/HY2/Smart-DNS owner labs | Candidate.33 binds the same exact Core commit whose direct-RU Raspberry Pi source-level predecessor evidence passes AWG3.1 then AWG2 and the default-off Smart DNS policy/TLS contour. Candidate.33's own SBOM/provenance is now the current `W1-03` authority, but no packaged candidate.33 AWG or Smart-DNS runtime or authenticated-client egress is credited. Phase 10 remains `I3`, not `I4`: packaged Android/Windows, multi-ASN, UDP/IPv6/MTU, leak/privacy/load/lifecycle and authenticated-session evidence remain open. Candidate.29 failures remain immutable predecessor evidence. HY2 remains undeployed; Gecko, Mimic and port hopping remain monitor-only | `WO-012`, `WO-013AO`, `WO-013AR`, `WO-013AS`, `WO-013AU`, `WO-013AX`–`WO-013BL`, `WO-013BP`, `WO-013BQ`, `WO-013BV`–`WO-013BX`, `WO-013CE`, `WO-013CG`, `WO-013CK`, `WO-013DG`, `WO-013DP`, `WO-013DU`, `WO-013EJ`, `WO-013EY`, `WO-013EZ`, `WO-013FB`–`WO-013FW`, `WO-013GF`, `WO-013GI`, `WO-013GT` |
| 11 | Exact-candidate RC matrix, immutable promotion, rollback and go/no-go | Candidate.33 is the current private signed candidate. Its freshly built six-file supply, candidate-specific SBOM/provenance/handoff, signature chain, exact Windows update and exact plain/typed singleton-forwarding/focus pass. Exact source Gates A–E and isolated rollback have now been executed: source/local slices pass, all five aggregates remain blocked on live/manual boundaries, and stable restoration is byte-identical. Gate F is `BLOCKED 2 PASS / 17 non-PASS / 0 FAIL` with zero validation errors. Physical Android, LDPlayer, saved-state migration, Windows default/AWG/Smart-DNS connected lifecycle/uninstall, exact current/Brain/RU origins, provider/Operator/legal, runtime rollback, comparable performance and final attestations remain non-PASS. Candidate.32 stays immutable `NO_GO` predecessor history. No public `v1.2.0`, Store object, stable switch, deploy or promotion occurred | `WO-013`, `WO-013BK`–`WO-013GU` |
...
source Gates A–E, isolated rollback and Gate F authority. Candidate.33 has
exact signed supply, Windows singleton/typed-forwarding/focus PASS and local
byte-identical rollback, while all five source-plan gates and Gate F remain
blocked (`2/17/0`). Candidate.32 is immutable predecessor history;
...
Candidate.33 saved-state/connected device, origins, approvals, runtime
rollback and final-attestation rows remain explicit non-PASS. WO-013FT/013FV
