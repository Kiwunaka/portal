...
| 01 | Release manifest, version/provenance contract, CI and stop-ship controls | Release-v2, client adoption, release-bound CI, reproducible dependencies, shared catalog and cross-repository truth are proved; the public-index source/trust root is on public main, PR-00 is merged, and `OWNER_SOLO_EXCEPTION` explicitly replaces the unavailable second reviewer without claiming independent review. WO-013CX adds exact candidate.13's validated six-artifact supply chain and genuinely passing public-index signer while private platform/client zero-step jobs remain non-PASS. WO-013DC finds affected Python locks in candidate.13, produces a locally clean successor lock set and makes candidate.13 explicitly non-promotable. The bounded signed supply slice remains retained, but a successor candidate is required and the aggregate release controls remain below promotion readiness | `WO-002`, `WO-003`, `WO-003B`, `WO-003C`, `WO-003D`, `WO-003E`, `WO-003F`, `WO-003G`, `WO-003H`, `WO-003I`, `WO-003J`, `WO-013J`, `WO-013K`, `WO-013L`, `WO-013M`, `WO-013N`, `WO-013O`, `WO-013P`, `WO-013CX`, `WO-013DC` |
...
| 08 | App, cabinet and marketing UX/accessibility/performance reconciliation | Locally complete at `I3`; historical candidate.10 quality/static evidence remains retained. WO-013DA binds exact candidate.13 source to direct current-origin health/catalog p95 `39.7521/40.4278 ms` PASS. Gate E stays `BLOCKED` below I4 on authenticated journeys, physical accessibility/OEM/scaling, comparable artifact/device and browser-lab performance, support, RU-origin and post-promotion evidence | `WO-010`, `WO-013AN`, `WO-013CI`, `WO-013CO`, `WO-013DA` |
...
| 11 | Exact-candidate RC matrix, immutable promotion, rollback and go/no-go | Candidates 5–12 remain immutable history. WO-013CX binds exact platform `7d983c0...`, client `ce2581d...` and Core `cd8f0f4...` to signed private candidate.13: supply-chain validation passes, the hosted public-index signer passes, and exact LDPlayer x86_64 AWG 3.1 -> AWG2 -> default/Auto passes in one application process with clean lab/VPN teardown. WO-013CY revalidates those bytes against current promotion heads and reconciles the stale ledger without transferring older evidence. WO-013CZ passes the exact signed candidate.13 portal/client rollback locally while leaving the real runtime drill open. WO-013DA passes source-bound current-origin health/catalog and exact Brain source/readiness; RU remains manual without runtime material, and isolated Windows is blocked by the available environment. The bounded supply/runtime slices reach `I4`; Gate F is not generated because its required exact ARM64 physical-install binding is absent. Clean Windows, RU origin, provider/Operator/legal, runtime rollback and broader performance remain non-PASS. Hosted private platform/client zero-step checks remain non-PASS under the no-purchase solo policy. No tag, public `v1.2.0`, Store object, stable switch or promotion occurred | `WO-013`, `WO-013BK`–`WO-013DA` |
...
| `WO-013` | Prove and promote the exact RC | Cross-repo/release | Signed candidate.13 is current. Supply and bounded LDPlayer lifecycle are `I4`; Gate F is `NOT_RUN_MISSING_EXACT_ARM64_INSTALL_BINDING`, and Gate G/public/stable promotion remains unauthorized | All release-bound WOs |
...
candidate.13 reaches Gate F `GO` and the owner separately authorizes
...
clean default/no-lab/no-VPN state. WO-013CZ also passes the exact signed
candidate.13 portal/client rollback in an isolated fixture and restores stable
...
WO-013CX/013CY supersede that active state with candidate.13. Its signed supply
and bounded LDPlayer AWG 3.1 -> AWG2 -> default lifecycle pass. WO-013CZ adds
the isolated local rollback, and WO-013DA separately passes current/Brain
origins while leaving RU manual and Windows blocked by environment. Gate F is
not run because the exact ARM64 physical-install binding is missing. Older
candidate.8/10 physical, Windows, Brain and RU results remain history and are
not transferred.
...
Windows live network/recovery, candidate.13 origin proof, provider/PostgreSQL/outbox,
