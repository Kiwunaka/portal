...
| `WO-013AS` | Add a default-off owned selective Smart DNS and opaque TLS relay lab without a second VPN core or access claim | Platform/client network-lab source and pre-candidate evidence | Canonical non-recursive allowlist policy, bounded Go DoH/SNI server, deterministic bundle builder, exact platform/client parity guard and selective client DNS materialization exist at `I2`; focused server/platform/client checks pass, while clean exact commits, immutable bundle, full client regression, APK/device proof, guarded installer, dedicated node, live DNS/SNI/access/leak/rollback and candidate remain open | `WO-013AO`, `WO-013AR`, current platform/client source, returned physical phone and LDPlayer |
