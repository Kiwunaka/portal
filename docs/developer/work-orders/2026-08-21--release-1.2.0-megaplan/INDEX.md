...
| 11 | Exact-candidate RC matrix, immutable promotion, rollback and go/no-go | Candidate.32 is the current private signed candidate. Its six-file supply/signature, Android static identity and Windows non-elevated boundary pass. WO-013GG adds exact Gates A–E source replay with zero source failures and an isolated byte-identical portal/client rollback; WO-013GH independently validates `4/4` APKs and the AAB without assigning device or Store credit; WO-013GI reconciles seven stale current-authority rows; WO-013GJ retains predecessor evidence while retargeting all fourteen residual future blockers to candidate.32. Neither reconciliation changes evidence levels or Gate F. Gate F remains `BLOCKED 2/17/0` with zero validation errors. Physical Android, LDPlayer, managed Windows default/AWG/Smart-DNS, exact current/Brain/RU origins, provider/Operator/legal, runtime rollback, comparable performance and final attestations remain non-PASS. No public `v1.2.0`, Store object, stable switch or promotion occurred | `WO-013`, `WO-013BK`–`WO-013GJ` |
...
the bounded historical-proof/current-blocker reconciliation.
Candidate.32 exact supply/signature, source aggregate, isolated rollback and
Android static identity pass, while Gate F is `BLOCKED 2/17/0` with runtime,
origin, approval, runtime-rollback and final-attestation rows explicit
...
exact candidate.32 and a suitable isolated Windows device, first perform the
elevated install/upgrade without screen automation, then run managed default
