...
Wave status: `PHASE_11_CANDIDATE18_NO_GO_CANDIDATE19_SIGNED_WINDOWS_FOUNDATION_ROLLBACK_BRAIN_CURRENT_API_LDPLAYER_INSTALL_PASS_PHYSICAL_CONNECTED_GATES_OPEN_GATE_F_NOT_RUN`
Release candidate: `POKROV_1_2_0_CANDIDATE19_SIGNED_PRIVATE_ACTIONS_ARTIFACT_ONLY_PROMOTION_NOT_AUTHORIZED`
...
| 11 | Exact-candidate RC matrix, immutable promotion, rollback and go/no-go | Candidates 5–18 remain immutable history. Candidate 18 is `NO_GO`: its ordinary Windows UI cannot authenticate the correct LocalSystem service through process-token access, closes IPC and the service exits `5`. WO-013DZ binds the SCM-record correction into signed private candidate.19 at platform `d6898e6...`, client `10f5516...` and Core `cd8f0f4...`. Six-artifact supply, 352-component SBOM, six-subject provenance, 11-file Windows manifest, hosted signer/receipt and exact-source replay pass. The isolated Windows 11 VM proves non-elevated IPC, protected journal, uninstall and public-1.1.6 migration. Exact local rollback passes. Brain source/readiness pass; two retained management-command samples are non-PASS before three final `7/7` delivery samples. Source-bound current API budgets pass, while aggregate STOP-SHIP stays blocked. LDPlayer contributes exact install/launch/byte identity only; its host-tunnel network result is excluded and no nested emulator is used. Candidate.19 physical Android Wi-Fi/Beeline, connected Windows, authenticated current/RU origins, provider/Operator/legal, runtime rollback, comparable device performance and final no-open-P0/false-green/privacy attestation remain non-PASS. Gate F is not regenerated. No tag, public `v1.2.0`, Store object, stable switch or promotion occurred | `WO-013`, `WO-013BK`–`WO-013DZ` |

Active Phase 10/11 supersession: WO-013DZ is the current signed-candidate and
evidence authority. Candidates 13–18 remain immutable history. Candidate 17 is
rejected for incomplete Windows runtime packaging. Candidate 18 is separately
rejected for the exact non-elevated Windows UI/service authentication failure;
its earlier supply, rollback and origin results remain history and cannot
authorize publication. Candidate 19 contains the merged SCM-record correction,
passes signed supply, hosted exact-source replay, Windows foundation, isolated
rollback, Brain/current public API and LDPlayer install/launch slices. The
emulator contributes no network, DNS or egress credit because host Windows uses
another tunnel. No nested emulator is used. Physical ARM64 Wi-Fi/Beeline and
connected Windows network runtime remain open.
