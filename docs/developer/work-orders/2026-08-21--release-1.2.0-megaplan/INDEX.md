...
| 02 | Typed connection state, proof-driven green state, core ABI and migrations | Aggregate Gate B remains `BLOCKED/I3`. Exact candidate.32 source passes platform state/diagnostics `70/70`, focused client `111/111` and full widgets `413/413`. Its installed Windows service additionally accepts trusted ordinary-user `status` and disconnected `initialize`: Core becomes ready while `can_connect`, running tunnel, DNS and egress stay false without a profile, and the network contour remains unchanged. Exact candidate.32 managed default/AWG3.1/AWG2/Smart-DNS, connected recovery and physical Android false-green/device matrices remain open. ABI v3 stays deferred until after 1.2.0 | `WO-004`, `WO-004A2`, `WO-004B2`, `WO-004D`, `WO-006I`, `WO-013CX`, `WO-013DD`–`WO-013DF`, `WO-013EN`, `WO-013EV`, `WO-013EW`, `WO-013FB`–`WO-013GK` |
| 03 | Windows/Android runtime boundaries; conditional Linux beta foundation | Gate C stays `BLOCKED/I3`. Candidate.32 exact bytes and signed identity pass inside the dedicated Windows 11 VM. Its authorized elevated in-place install replaces the retained `8/11` mixed state with exact `11/11`; ordinary UI/trusted IPC, disconnected initialization and managed reboot pass. Forced service termination recovers automatically through SCM in `5,252 ms`. WO-013GM then restores a zero-POKROV snapshot and proves exact clean-host install: setup exit `0`, `11/11`, version `1.2.0+4053`, Auto LocalSystem service, correct pre-initialize clean state and unchanged route/DNS. Public-1.1.6 migration, clean-state UI/IPC/reboot, AWG/Smart-DNS connected recovery, connected uninstall, Windows 10, SmartScreen UI, sleep/resume, IPv6 and Android device matrices remain open. The Android production set is `5/5` exact signed static proof: `4/4` APKs pass signature/alignment/package/version/ABI and the AAB passes bundletool. Direct-only install permission/provider and Store exclusion pass against the signed files plus `10/10` source-contract tests. ADB currently exposes no device, so no install, update or runtime credit follows. Linux native runtime/packaging remains open and deliberately unshipped | `WO-005`, `WO-005C4`, `WO-005D4`, `WO-005G`, `WO-013CC`–`WO-013CO`, `WO-013CX`, `WO-013DD`–`WO-013DF`, `WO-013DJ`, `WO-013DL`, `WO-013DO`, `WO-013EA`, `WO-013EF`, `WO-013EG`, `WO-013EV`–`WO-013GM` |
...
| 11 | Exact-candidate RC matrix, immutable promotion, rollback and go/no-go | Candidate.32 is the current private signed candidate. Its six-file supply/signature and Android static identity pass. WO-013GK adds exact Windows in-place replacement, ordinary UI/trusted IPC, disconnected initialization and managed reboot; WO-013GL adds automatic SCM recovery after disconnected forced service termination; WO-013GM adds exact zero-state clean-host installation with a retained return snapshot. None assigns migration or connected-network credit. WO-013GG adds exact Gates A–E source replay with zero source failures and an isolated byte-identical portal/client rollback; WO-013GH independently validates `4/4` APKs and the AAB without assigning device or Store credit; WO-013GI/GJ reconcile current authority and future blockers without level changes. Gate F remains `BLOCKED 2/17/0` with zero validation errors. Physical Android, LDPlayer, Windows migration/clean-state UI-reboot/default/AWG/Smart-DNS/connected lifecycle/uninstall, exact current/Brain/RU origins, provider/Operator/legal, runtime rollback, comparable performance and final attestations remain non-PASS. No public `v1.2.0`, Store object, stable switch or promotion occurred | `WO-013`, `WO-013BK`–`WO-013GM` |
...
WO-013GM is the current exact zero-state clean-install authority.
