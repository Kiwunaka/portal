...
| 11 | Exact-candidate RC matrix, immutable promotion, rollback and go/no-go | Candidates 5–17 remain immutable history. Candidate 17 is rejected because its eight-file Windows setup omits the three required VC143 app-local DLLs and can return false success after service-start failure. WO-013DV binds exact platform `d6898e6...`, client `820ca10...` and Core `cd8f0f4...` to signed private candidate.18. Six-artifact supply, 352-component SBOM, provenance, 11-file Windows manifest, real hosted signer/receipt and exact-source replay pass. The isolated Windows 11 VM proves clean-app-state install, service, authenticated IPC, restart, uninstall, idle-network restoration and public-1.1.6 migration. LDPlayer contributes install/launch/byte identity only; its host-tunnel network result is excluded. Candidate.18 physical Android Wi-Fi/Beeline, live connected Windows, current/Brain/RU, provider/Operator/legal, rollback, comparable performance and no-open-P0 aggregate proof remain non-PASS. Gate F is not regenerated for candidate.18. No tag, public `v1.2.0`, Store object, stable switch or promotion occurred | `WO-013`, `WO-013BK`–`WO-013DV` |

Active Phase 10/11 supersession: WO-013DV is the current signed-candidate
authority. Candidates 13–16 remain immutable history. Candidate 17 is retained
