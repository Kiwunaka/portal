...
Wave status: `PHASE_11_CANDIDATE19_NO_GO_CANDIDATE20_SIGNED_WINDOWS11_DEFAULT_SOURCE_STATIC_PRIVACY_PASS_ANDROID_ORIGIN_ROLLBACK_AGGREGATE_GATES_OPEN_GATE_F_NOT_RUN`
Release candidate: `POKROV_1_2_0_CANDIDATE20_SIGNED_PRIVATE_ACTIONS_ARTIFACT_ONLY_PROMOTION_NOT_AUTHORIZED`
...
| 01 | Release manifest, version/provenance contract, CI and stop-ship controls | Release-v2, client adoption, release-bound CI, reproducible dependencies, shared catalog and cross-repository truth are proved; the public-index source/trust root is on public main, PR-00 is merged, and `OWNER_SOLO_EXCEPTION` replaces the unavailable second reviewer without claiming independent review. WO-013EA binds the current corrected platform, client and Core tuple to signed artifact-only candidate.20. Six-artifact validation, 352-component SBOM, six-subject provenance and public-index signer/receipt pass; Windows trusted signing remains owner-skipped direct-beta only. WO-013EB repeats the exact candidate.20 STOP-SHIP registry `7/7` and P0 query `0/0`; branch protection and final live aggregate remain non-PASS. `DEP-001` retains `I4`; aggregate release controls remain below promotion readiness | `WO-002`, `WO-003`, `WO-003B`, `WO-003C`, `WO-003D`, `WO-003E`, `WO-003F`, `WO-003G`, `WO-003H`, `WO-003I`, `WO-003J`, `WO-013J`, `WO-013K`, `WO-013L`, `WO-013M`, `WO-013N`, `WO-013O`, `WO-013P`, `WO-013CX`, `WO-013DC`, `WO-013DD`, `WO-013DE`, `WO-013EA`, `WO-013EB` |
...
| 11 | Exact-candidate RC matrix, immutable promotion, rollback and go/no-go | Candidates 5–19 remain immutable history. Candidate 19 is `NO_GO`: its managed profile retained ordinary-user AppData paths for local binary rule sets, Core returned `CORE-005` and the service rolled back without connected state. WO-013EA binds the service-owned rule-set correction into signed private candidate.20 at platform `d6898e6...`, client `8ab9815...` and Core `cd8f0f4...`. Six-artifact supply, 352-component SBOM, six-subject provenance, 11-file Windows manifest, hosted signer/receipt and exact post-build source replay run `33511744299` pass. The isolated Windows 11 VM proves ordinary UI/LocalSystem service, four protected rule sets, default TUN, managed DNS, authenticated DE egress, disconnect rollback, clean uninstall and public-1.1.6 migration; `WIN-003` and `DOD-04` reach `I4` for that exact default-path slice. WO-013EB adds exact source `7/7` STOP-SHIP, P0 `0/0` and privacy `75 + 145+4 + 29 + 15` PASS. WO-013EC adds six-artifact static binding plus zero definite findings across five Android archives, the raw Windows installer and the manifest-bound Windows staging tree. WO-013ED replays exact source Gates A–E: Gate B `70/70 + 111/111`, Gate D `196/196 + 12` and `25/25`, local quality `15/15`, client `413/413`, cabinet `69/69` and static performance `9/9` all pass on Node `22.14.0`; all five gates remain blocked on live/manual boundaries. WO-013EE reconciles all active low-level instructions to candidate.20 and proves that PB-14 fails closed exactly at the missing physical installed-build identity. Candidate.20 LDPlayer/physical Android, Windows 10/non-default/recovery, named origins, provider/Operator/legal, candidate rollback, comparable device performance and final no-open-P0/false-green/privacy attestation remain non-PASS. Gate F is not regenerated. No tag, public `v1.2.0`, Store object, stable switch or promotion occurred | `WO-013`, `WO-013BK`–`WO-013EE` |

Active Phase 10/11 supersession: WO-013EA is the current signed-candidate and
Windows evidence authority; WO-013EB is the exact offline source-gate
authority; WO-013EC is the exact static artifact privacy authority; WO-013ED
is the exact offline Gates A-E authority; and WO-013EE is the active-ledger and
PB-14 physical-boundary authority.
...
Candidate 20 contains the merged service-owned rule-set correction, passes
signed supply and the exact Windows 11 default TUN/DNS/authenticated-egress/
rollback/migration slice. Its six immutable distribution files, five Android
archives, raw Windows installer and manifest-bound Windows staging tree pass
the bounded static privacy slice. The VM retained authorized user-level app
state while prior machine installation/service state was absent; this is not
called a fresh user profile. Candidate 20 LDPlayer install/launch, physical ARM64
Wi-Fi/Beeline, Windows 10/non-default/recovery, named origins, candidate
rollback and aggregate Gate F evidence remain open. Host-
tunneled LDPlayer network output will remain excluded and no nested emulator is
used.
