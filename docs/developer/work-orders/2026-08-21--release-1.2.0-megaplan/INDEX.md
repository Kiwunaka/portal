...
| 03 | Windows/Android runtime boundaries; conditional Linux beta foundation | Gate C stays `BLOCKED/I3`. Candidate.32 exact bytes and signed identity pass inside the dedicated Windows 11 VM. Its authorized elevated in-place install replaces the retained `8/11` mixed state with exact `11/11`; the Auto LocalSystem service, ordinary UI/trusted IPC, disconnected initialization and managed reboot pass with clean recovery and unchanged route/DNS. A subsequent forced service termination recovers automatically through SCM in `5,252 ms`, without operator remediation, network mutation or spontaneous UI. This is not a clean install, public-1.1.6 migration, saved-state rollback or connected-network result. Exact candidate.32 clean install/migration, AWG/Smart-DNS connected recovery, connected uninstall, Windows 10, SmartScreen UI, sleep/resume, IPv6 and Android device matrices remain open. The Android production set is `5/5` exact signed static proof: `4/4` APKs pass signature/alignment/package/version/ABI and the AAB passes bundletool. Direct-only install permission/provider and Store exclusion pass against the signed files plus `10/10` source-contract tests. ADB currently exposes no device, so no install, update or runtime credit follows. Linux native runtime/packaging remains open and deliberately unshipped | `WO-005`, `WO-005C4`, `WO-005D4`, `WO-005G`, `WO-013CC`–`WO-013CO`, `WO-013CX`, `WO-013DD`–`WO-013DF`, `WO-013DJ`, `WO-013DL`, `WO-013DO`, `WO-013EA`, `WO-013EF`, `WO-013EG`, `WO-013EV`–`WO-013GL` |
...
| 11 | Exact-candidate RC matrix, immutable promotion, rollback and go/no-go | Candidate.32 is the current private signed candidate. Its six-file supply/signature and Android static identity pass. WO-013GK adds exact Windows in-place replacement, ordinary UI/trusted IPC, disconnected initialization and managed reboot proof; WO-013GL adds automatic SCM recovery after disconnected forced service termination. Neither assigns clean-install or connected-network credit. WO-013GG adds exact Gates A–E source replay with zero source failures and an isolated byte-identical portal/client rollback; WO-013GH independently validates `4/4` APKs and the AAB without assigning device or Store credit; WO-013GI reconciles seven stale current-authority rows; WO-013GJ retains predecessor evidence while retargeting all fourteen residual future blockers to candidate.32. These ledger slices change no evidence levels and Gate F remains `BLOCKED 2/17/0` with zero validation errors. Physical Android, LDPlayer, Windows clean/migration/default/AWG/Smart-DNS/connected lifecycle/uninstall, exact current/Brain/RU origins, provider/Operator/legal, runtime rollback, comparable performance and final attestations remain non-PASS. No public `v1.2.0`, Store object, stable switch or promotion occurred | `WO-013`, `WO-013BK`–`WO-013GL` |
...
Android static identity plus bounded Windows in-place runtime pass, while Gate F
is `BLOCKED 2/17/0` with clean/connected device, origin, approval,
