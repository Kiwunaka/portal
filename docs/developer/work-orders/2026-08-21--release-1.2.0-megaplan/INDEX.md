...
| 03 | Windows/Android runtime boundaries; conditional Linux beta foundation | WO-013CI advances Gate C `I2 -> I3` on exact candidate.8; WO-013CL then closes the named physical inverse excluded-app traffic/bypass subcheck. Physical Android and LDPlayer ordinary/AWG2/AWG3.1, bounded WARP/per-app/network/lifecycle/Private-DNS, Windows current-host install/service/IPC/restart/uninstall/idle restore and exact Core CI pass. Gate C remains `BLOCKED` below `I4` on connected clean-VM Windows, Android external leak/UDP53/MTU/OEM/endurance/accessibility and Store delivery. Linux is deliberately not shipped in candidate.8 | `WO-005`, `WO-005C4`, `WO-005D4`, `WO-005G`, `WO-013CC`–`WO-013CL` |
...
traffic/bypass subcheck without changing the aggregate Gate F count. Active
WARP, external IPv6/leak, UDP53/MTU, multi-OEM and
...
the next honest slice requires explicit public DoH-name authorization, trusted
certificate and root-only runtime material before separate server/route APPLY
and actual ChatGPT/Gemini/Xbox session access, resolver/DNS-SNI attribution,
...
Windows live network/recovery, exact RU-origin, provider/PostgreSQL/outbox,
