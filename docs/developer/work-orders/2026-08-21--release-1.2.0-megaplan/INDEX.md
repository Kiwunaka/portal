...
Wave status: `PHASE_11_CANDIDATE33_WINDOWS_LIFECYCLE_STARTUP_SAVED_STATE_COLD_START_COMBINED_IDLE_CPU_ARTIFACT_BASELINES_LDPLAYER_UI_MARKETING_BROWSER_PASS_CABINET_BROWSER_BLOCKED_AWG_PLAN_READY_GATE_F_BLOCKED`
...
budget authority; WO-013HJ is the current exact candidate.33 combined Windows
UI/service idle CPU budget and working-set baseline authority. Private signed
...
and strict handoff. Strict validation passes `6/6` application artifacts and
`11/11` Windows runtime files; release-index source-contract, signer and
receipt runs execute real steps and pass. Three platform/client required jobs
instead terminate with `steps=[]` and remain
`BLOCKED_BY_ACCESS_GITHUB_BILLING`.

The exact candidate.33 installer updates candidate.32 in the dedicated
headless Windows 11 VM with exit `0` and `11/11` installed identity. Plain
and typed second launches each retain one process, forward activation and
return foreground focus, so `REL/WIN-001` advances to exact-candidate `I4`.
Candidate.33 source preflight is `READY_LOCAL_FREEZE` with zero blockers. Gate
B passes `70/70 + 111/111`, Gate D passes `196/196 + 12` and `25/25`, local
quality passes `15/15`, and isolated stable -> candidate -> stable rollback is
byte-identical. All five Gates A–E remain `BLOCKED` on live/manual boundaries
with zero source failures. Fresh npm audit is not claimed because the canonical
registry request timed out; all three locks are byte-identical to candidate.32's
retained zero-finding audit inputs. Gate F validates all `19/19` pointers and
is `BLOCKED 2 PASS / 17 non-PASS / 0 FAIL` with zero validation errors.

The exact installed Windows service/Core now passes direct and Smart DNS
synthetic secret-free TUN/DNS connect-disconnect-cleanup in the dedicated
headless VM. The connected direct slice additionally passes forced
LocalSystem termination: SCM restarts automatically in `5449` ms, the durable
journal returns from `committed` to `clean`, TUN disappears and exact
route/DNS fingerprints return without operator remediation. Ordinary IPC then
reports `initialized`, `running=false`, `failure=none`. A connected guest
reboot also returns the same exact install safely disconnected with no TUN,
pre-reboot route/DNS restored, trusted `artifact_ready` IPC, DNS `4/4` and API
health `200`; a fresh post-boot connect/disconnect then restores the boot
baseline and final cleanup retains `11/11`. Connected uninstall with the exact
interactive UI then removes the UI, service, all `11` product files, registry
and TUN, restores exact route/DNS, and a clean exact setup reinstalls `11/11`
with the Automatic LocalSystem service. A real guest login then starts one
exact `--startup` process hidden without UAC, TUN or route/DNS drift; ordinary
activation exposes a window on the same PID, and final cleanup removes the
temporary Run/task/UI state. The owning Profile → Windows preference then
creates and removes that exact Run value with the same ordinary UI/service
state, zero TUN and unchanged route/DNS fingerprints; temporary first-launch
and guest fixtures are removed and the guest link is restored. A dedicated
temporary Windows user then proves synthetic v0 saved-state migration to schema
v1 secure storage during offline login startup. The same hidden PID remains
disconnected when Ethernet appears, with unchanged state/secure-store/experience
fingerprints and zero Core/TUN. SYSTEM cleanup removes the fixture user,
profile, ProgramData and task and restores `pokrovtest`. The exact universal
...
Fresh exact-source production static exports then pass bounded headless
localhost browser collection for all four marketing budgets: home LCP p75
`136 ms`, checkout LCP p75 `96 ms`, CLS p75 `0` and TBT p75 `8 ms`, each from
20 retained samples after three warmups. The cabinet export renders its login
screen, but without an integrated API/auth fixture it cannot produce
authenticated dashboard route-content evidence. That metric is
...
process does not prove physical/comparable Windows or the remaining client
performance matrix; Gate F and all ledger levels stay unchanged.

The exact candidate.33 Windows UI and automatic service then run together in
the same disconnected VM for 30 discarded and 60 retained one-second samples.
Combined idle CPU p95 is `0.749734%` and passes the `1.0%` target and stop.
Combined working-set p95 is `108158976` bytes and becomes a new
`BASELINE_RECORDED`; it is not compared with the earlier UI-only boundary.
The UI remains responding, the service Running, crash events zero, adapter Up
zero and route/DNS unchanged. Guest tooling/task and the UI are removed after
capture. This still does not prove physical/comparable Windows, memory
regression or the remaining client performance matrix; Gate F and all ledger
...
WO-013FY adds a fresh exact-client/Core headless CLI build and a separate
candidate.31 VM recheck. The full bounded build/test contour and installed
candidate IPC `32/32` pass without host input, but the fresh setup is not
byte-identical to candidate.31: `8/11` required runtime files match exactly and
the other three retain exact code sections with different build metadata. No
same-byte or new-candidate credit is assigned; Gate F remains `NO_GO 2/17/1`.

WO-013FZ closes that reproducibility defect in successor client source and
merges it to `POKROV-app/main`. Two independent correction-commit worktrees and
a third merged-main worktree produce the staged Windows bundle `302/302`
byte-identically through the canonical CLI path. Full bounded tests, Windows
analyze, native Release CTest `7/7` and an exact Core DLL 100-cycle backtest
pass. The resulting unsigned local installer is not launched or installed.
This is `PRE_CANDIDATE_LOCAL` proof, not candidate credit: candidate.31 and
Gate F remain immutable `NO_GO 2/17/1`.

WO-013GA then replays the exact clean current tuple after that merge. The
platform/client/Core local quality aggregate passes `15/15`, including cabinet
E2E `69/69` and static performance `9/9`. A separately requested headless
Windows CLI rebuild again matches the retained staged bundle at `302/302` and
passes Release CTest `7/7`. The unsigned installer is retained but not run.
This remains `PRE_CANDIDATE_LOCAL` with `candidate_proven=false`; no candidate,
Gate F or execution-ledger level changes.

WO-013GB binds that clean tuple to published release-index `main` in the
read-only candidate preflight. It returns `READY_LOCAL_FREEZE` with zero
blockers and zero pre-freeze rows below `I3`. The result is not candidate proof:
`candidate_created=false`, `candidate_proven=false`,
`promotion_authorized=false`, and all `51` candidate/deferred/external rows
below `I3` remain open under their declared stage policy.

WO-013GC then removes the manual market-AAB side path from the canonical client
production helper and merges it as client PR 74. Exact merged `main`
`2d6adfc...` produces four production-signed APKs and one market-only AAB in a
single fail-closed CLI run; two successive runs yield the same five hashes,
and the AAB certificate, JAR signature and three Core ABI entries pass
independent replay. The retained Windows setup crosses only the exact
Android-tooling/docs/test delta and is explicitly recorded as reused bytes.
This remains pre-candidate evidence: Store submission is `NOT_REQUESTED`,
candidate creation/signing/promotion are false, and no index level changes.

WO-013GD then performs a fresh canonical Windows CLI build from that exact
merged client `main`. The full seed/contracts/test/analyze/Gradle/Release
contour passes, native Release CTest passes `7/7`, and the complete staged
product bundle matches the preceding successor rebuild at `302/302` with zero
differing lines. The installer remains unsigned under the owner exception and
is not launched. This is still pre-candidate evidence: candidate creation,
deploy and publication remain false, with no index level changes.

WO-013GE then repeats the read-only freeze preflight against the complete
current tuple after both precursor slices. Clean platform/ledger `9d92889...`,
client `2d6adfc...`, Core `cd8f0f4...` and published release-index `main`
return `READY_LOCAL_FREEZE` with zero blockers and zero pre-freeze rows below
`I3`. The remaining `22` candidate, `16` deferred and `13` external rows stay
open; candidate creation/proof/promotion remain false.

WO-013GF supersedes that pre-candidate boundary with private signed
`pokrov-1.2.0-candidate.32`. The exact successor tuple is bound to six
application artifacts, fresh candidate-specific SBOM/provenance/handoff and a
hosted Ed25519 manifest signature. Strict replay passes `6/6` artifacts and
`11/11` Windows runtime files. A dedicated Windows 11 VM additionally proves
the exact candidate bytes and non-elevated fail-closed installer behavior
without changing the pre-existing install, service, routes or DNS. It does not
claim elevated install or managed-network credit. Candidate.32 Gate F is
`BLOCKED 2/17/0`; publication, deployment and Gate G remain unauthorized.

WO-013GG then replays the source-owned Gates A–E slices against that exact
signed tuple. Gate B passes `70/70 + 111/111`, Gate D passes `196/196 + 12`
subtests plus `25/25`, the full local-quality aggregate passes `15/15`, and
fresh WebApp/AdminApp/Marketing audits report `0/0/0`. The isolated portal and
client rollback returns byte-identically from candidate.32 to retained
`1.1.6`. All five gates remain `BLOCKED` on live/manual boundaries, and Gate F
remains `BLOCKED 2/17/0` with zero validation errors.

WO-013GK adds the first exact candidate.32 installed Windows runtime slice. In
the dedicated Windows 11 VM, the elevated installer replaces the retained
`8/11` predecessor mix with exact `11/11`, an ordinary user reaches trusted
IPC `status` and disconnected `initialize`, the exact UI opens without UAC,
and a managed reboot retains the exact install, clean recovery journal and
unchanged route/DNS contour. This is bounded in-place and disconnected proof,
not clean-install, connected default/AWG/Smart-DNS or uninstall credit. Gate F
therefore remains `BLOCKED 2/17/0`.

WO-013GL then forcibly terminates the exact disconnected service. SCM starts a
new process automatically after `5,252 ms`, without operator remediation. The
exact `11/11` identity, clean recovery journal and route/DNS contour remain
unchanged, while no POKROV adapter or UI appears. This is not connected
tunnel/AWG/Smart-DNS, sleep/resume or endurance credit, so Gate F remains
`BLOCKED 2/17/0`.

WO-013GM restores the retained clean Windows 11 baseline after preserving the
prior candidate.32 state in a return snapshot. Zero POKROV state is proved
before installation; the exact signed setup then exits `0` and independently
reconciles to `11/11`, version `1.2.0+4053`, Auto LocalSystem service and an
unchanged disconnected network. The exact-source recovery contract confirms
that no journal before first initialization is the clean initial state. This
does not prove migration, connected runtime or clean-state UI/IPC/reboot, so
Gate F remains `BLOCKED 2/17/0`.

WO-013GN boots that retained clean-installed snapshot and observes the exact
service Running before any console login. From the ordinary owner session,
trusted status starts at `artifact_ready`, initialize reaches `initialized`
without enabling connect/tunnel/DNS/egress, and the exact UI exposes a main
window without UAC. Network counts remain unchanged and all diagnostic
fixtures are removed. Launch-at-login timing, migration and connected runtime
remain open; Gate F stays `BLOCKED 2/17/0`.

WO-013GO then restores the zero-POKROV baseline, installs the exact retained
public `1.1.6+29` package as an ordinary per-user app and migrates it with the
exact candidate.32 installer. Setup exits `0`, the candidate reconciles to
`11/11`, the Auto LocalSystem service runs, and the legacy directory plus HKCU
uninstall record are absent only after the machine install succeeds. The
route/DNS contour remains unchanged and guest staging is removed. This proves
public-package migration only: saved account/profile state, post-migration
ordinary UI/IPC, Android, rollback and connected runtime remain open. Gate F
stays `BLOCKED 2/17/0`.

WO-013GP resumes that migrated state and first observes the service
Running/Automatic before console login. In the ordinary owner session, exact
co-located CLI status is trusted at `artifact_ready`, disconnected initialize
reaches `initialized`, and the exact UI opens a same-owner/session main window
without UAC. `can_connect`, tunnel, DNS and egress stay false; route/DNS counts
do not change. Two harness errors and one expected `server_untrusted`
fail-closed result from invalid CLI placement remain explicit evidence. The
validated diagnostic CLI and all guest staging are removed. Saved-state,
connected, Android and rollback matrices remain open; Gate F stays
`BLOCKED 2/17/0`.

WO-013GQ then tests candidate.32's exact second-launch boundary. Plain and
typed launches each exit `0`, retain one original process and forward
activation, but the existing window does not regain foreground focus.
`REL/WIN-001` is therefore an exact STOP-SHIP failure and candidate.32 becomes
immutable `NO_GO`. Gate F regenerates as `NO_GO 2 PASS / 17 non-PASS / 2
FAIL` with zero validation errors. Client commit `76abed9...` transfers
foreground permission before `WM_COPYDATA`; focused tests, Release build,
native `7/7` and bounded VM plain/typed focus replay pass only as
`PRE_CANDIDATE_LOCAL`. No new candidate, signature, deploy or publication is
created. Seven current status rows and all `57` future-action fields are
reconciled without changing any completion index.

## Execution order

| Phase | Architectural outcome | State | Primary WO |
|---|---|---|---|
| 00 | Baseline, authority map, complete execution ledger | Locally proved (`I3`); WO-013GE is the retained exact-source preflight with zero blockers and zero pre-freeze rows below `I3`. WO-013GF then creates the exact successor candidate without changing the baseline authority map | `WO-001`, `WO-013GE`, `WO-013GF` |
| 01 | Release manifest, version/provenance contract, CI and stop-ship controls | Candidate.33 is the current private signed supply. Fresh merged-source builds, candidate-specific CycloneDX/SLSA/handoff metadata, all six application artifacts and all eleven Windows runtime files pass strict replay. The exact platform/client/Core tuple and release-index source are signature-bound; Android production signing passes, while Windows remains an explicit unsigned direct-beta owner exception. Exact `WIN-001` passes at `I4`; candidate.33 local STOP-SHIP anchors pass `7/7`, while the aggregate Gate A remains `BLOCKED/I1` on hosted, complete connected-device and signing boundaries. Release-index source-contract/signer/receipt jobs pass with real steps, while required platform/client jobs remain `BLOCKED_BY_ACCESS_GITHUB_BILLING`. Gate F is `BLOCKED 2/17/0`; `DEP-001` and `WIN-001` are `I4`, and no tag, public asset, deploy, Store object, stable pointer or promotion exists. Candidate.32 remains immutable `NO_GO` history | `WO-002`, `WO-003`, `WO-003B`–`WO-003J`, `WO-013J`–`WO-013P`, `WO-013CX`, `WO-013DC`–`WO-013DE`, `WO-013EA`, `WO-013EB`, `WO-013EH`, `WO-013EI`, `WO-013EV`–`WO-013GY` |
| 02 | Typed connection state, proof-driven green state, core ABI and migrations | Exact candidate.33 Gate B source replay passes platform `70/70`, focused client `111/111` and full client widgets `413/413`. Exact Windows direct and Smart DNS synthetic TUN/DNS lifecycles cleanly restore baseline; connected forced-service termination, connected guest reboot and connected uninstall each return the exact network contour to a safe disconnected state, with a final clean `11/11` reinstall. Physical Android ordinary Wi-Fi plus fail-closed Beeline slices provide bounded runtime proof. The aggregate remains `BLOCKED/I3`: managed default/AWG recovery, complete false-green/leak/device matrices and LDPlayer parity remain open. Candidate.32 disconnected status/initialize, clean-install and public-package-migration slices remain predecessor evidence and do not transfer automatically. ABI v3 stays deferred until after 1.2.0 | `WO-004`, `WO-004A2`, `WO-004B2`, `WO-004D`, `WO-006I`, `WO-013CX`, `WO-013DD`–`WO-013DF`, `WO-013EN`, `WO-013EV`, `WO-013EW`, `WO-013FB`–`WO-013GY` |
| 03 | Windows/Android runtime boundaries; conditional Linux beta foundation | Gates A and C stay `BLOCKED` below candidate proof after exact candidate.33 replay with zero source failures. Exact candidate.33 update reaches `11/11`, service state and `WIN-001` focus pass, installed service/Core direct and Smart DNS synthetic lifecycles pass, and connected forced-service termination plus connected guest reboot each restore the durable journal and exact route/DNS baseline in the headless VM. Connected uninstall terminates the interactive UI, removes every product file/service/registry/TUN, restores exact route/DNS and cleanly reinstalls `11/11`. Exact real-login `--startup` remains hidden without UAC/network mutation and ordinary activation exposes the same PID; the owning product preference creates/removes that exact Run value. Synthetic v0 saved state migrates to schema v1 secure storage during offline startup and the same hidden PID remains disconnected when Ethernet appears; complete SYSTEM cleanup restores `pokrovtest`. The exact universal APK installs in place on physical Android, passes Wi-Fi validated DNS `4/4`, and supplies a bounded Beeline slice. Valid saved-account/managed-profile refresh and managed auto-connect/default/AWG recovery, Windows 10, SmartScreen UI, sleep/resume, IPv6, LDPlayer and the complete Android device matrix remain open. Linux native runtime/packaging remains open and deliberately unshipped | `WO-005`, `WO-005C4`, `WO-005D4`, `WO-005G`, `WO-013CC`–`WO-013CO`, `WO-013CX`, `WO-013DD`–`WO-013DF`, `WO-013DJ`, `WO-013DL`, `WO-013DO`, `WO-013EA`, `WO-013EF`, `WO-013EG`, `WO-013EV`–`WO-013HC` |
| 04 | Observability, error catalog, redacted bundle and support pipeline | Locally complete (`I3`) for shipped Android/Windows scope. Candidate.33's fresh SBOM/provenance and strict supply validation pass without raw egress or connection material. Candidate.32 and candidate.31 remain predecessor supply history; candidate.30's stale metadata remains immutable rejected history. Deployed runtime/RBAC/audit, connected managed-device evidence and final live privacy attestation remain open | `WO-006`, `WO-006I`, `WO-006J`, `WO-006K`, `WO-006L`, `WO-013S`, `WO-013DJ`, `WO-013DQ`, `WO-013DR`, `WO-013EP`, `WO-013EQ`, `WO-013FB`, `WO-013FH`, `WO-013FJ`, `WO-013FN`, `WO-013FO`, `WO-013FS`, `WO-013FT`, `WO-013GF`, `WO-013GT` |
| 05 | Portal bounded contexts, payments, HTTP/DB/outbox reliability | Locally complete (`GATE-D` and `ARCH-002` at `I3`); 007G–007I split admin, public/client and Action Intent domain/runtime owners. Exact candidate.33 Gate D source replay passes payment/provider/callback/HTTP/DB/outbox/module `196/196 + 12 subtests` and Action Intent/policy `25/25`. Candidate.33 has no exact deployed Brain/current-origin or provider runtime proof. Gate D stays `BLOCKED` below I4 until production provider/PostgreSQL/outbox/reconciliation/operator/runtime-rollback evidence exists | `WO-007`, `WO-007G`, `WO-007H`, `WO-007I`, `WO-013CI`, `WO-013CO`, `WO-013EL`, `WO-013EN`, `WO-013FD`, `WO-013FE`, `WO-013FH`, `WO-013FV`, `WO-013FX`, `WO-013GF`, `WO-013GG`, `WO-013GT`, `WO-013GU` |
...
| 11 | Exact-candidate RC matrix, immutable promotion, rollback and go/no-go | Candidate.33 is the current private signed candidate. Its freshly built six-file supply, candidate-specific SBOM/provenance/handoff, signature chain, exact Windows update and exact plain/typed singleton-forwarding/focus pass. Exact source Gates A–E and isolated rollback have been executed; exact Windows synthetic direct/Smart DNS lifecycle, connected forced-service rollback, connected reboot recovery, connected uninstall/clean reinstall, product startup-preference, real-login hidden/same-PID activation, synthetic saved-state offline-startup/network-appearance and physical Android ordinary Wi-Fi plus bounded Beeline slices are now retained. Read-only Windows AWG3.1/AWG2 plans are ready but receive no runtime credit without separately authorized APPLY. Gate F remains `BLOCKED 2 PASS / 17 non-PASS / 0 FAIL` with zero validation errors; the two device rows are `MANUAL_OWNER_TEST`, not PASS. LDPlayer, valid saved-account/managed-profile refresh and managed auto-connect timing, managed Windows default/AWG recovery, Android AWG/leak/lifecycle matrices, exact current/Brain/RU origins, provider/Operator/legal, runtime rollback, comparable performance and final attestations remain non-PASS. Candidate.32 stays immutable `NO_GO` predecessor history. No public `v1.2.0`, Store object, stable switch, deploy or promotion occurred | `WO-013`, `WO-013BK`–`WO-013HC` |
...
source Gates A–E and isolated rollback authority; WO-013GV is the current
...
current read-only Windows AWG3.1/AWG2 PLAN-readiness authority; WO-013HA is the
current exact Windows login-startup authority; WO-013HB is the current exact
Windows product startup-preference authority; WO-013HC is the current exact
Windows synthetic saved-state offline-startup/network-appearance authority.
Candidate.33 has exact signed
supply, Windows singleton/typed-forwarding/focus PASS, local byte-identical
rollback, product preference plus real-login hidden/same-PID activation,
synthetic saved-state migration/no-autoconnect, synthetic Windows TUN/DNS cleanup, connected
forced-service/reboot/uninstall cleanup plus bounded physical Android
Wi-Fi/Beeline evidence, while all five source-plan gates and
Gate F remain blocked (`2/17/0`). Candidate.32 is immutable predecessor history;
WO-013GQ remains its exact focus `NO_GO` authority. WO-013GF through
WO-013GP retain candidate.32 supply, source, static Android, rollback and
bounded Windows install/service/migration history. WO-013GR/013GS retain the
successor truth and
preflight boundaries before candidate.33.
Candidate.33 managed AWG/valid saved-account auto-connect/complete device, origins, approvals,
runtime rollback and final-attestation rows remain explicit non-PASS.
WO-013FT/013FV
retain candidate.31 signed-supply and exact Brain
`NO_GO 2/17/1` history; WO-013FX is its append-only source-diagnosis
correction, and WO-013FY is historical exact-source diagnostic evidence.
WO-013FZ is the successor-main reproducibility authority, WO-013GA is the
exact successor-source local-quality authority, WO-013GB/013GE are retained
preflight history, WO-013GC is the exact merged-main Android production
precursor authority, and WO-013GD is the exact merged-main Windows CLI rebuild
authority. WO-013FS
...
WO-013FY proves a fresh exact-client/Core CLI build and full bounded test pass,
then rechecks candidate.31 installed identity and IPC headlessly. The new build
is not fully byte-reproducible and is not a candidate; Gate F stays unchanged.
Its two required hosted jobs execute zero steps and are explicitly
`BLOCKED_BY_ACCESS_GITHUB_BILLING`, while local replacement checks pass.
WO-013FZ proves the correction in merged client main with three clean builds
and a `302/302` exact staged bundle. Full CLI verification and the exact Core
DLL backtest pass, while the local installer remains unsigned and uninstalled.
No candidate is created, so Gate F remains unchanged.
...
| `WO-013FT` | Create and sign candidate.31 with corrected candidate-bound supply metadata, then regenerate Gate F | Six same-byte application artifacts, fresh SBOM/provenance/handoff, strict supply PASS, hosted Ed25519 signature/receipt, candidate.30 append-only digest correction and all `19/19` Gate F pointers | Candidate.31 became current at this slice and is now predecessor history. Strict validation passes `6/6` artifacts and `11/11` Windows files; its initial Gate F is `BLOCKED 2 PASS / 17 non-PASS / 0 FAIL`. ADB sees no device; managed Windows/Android/origin/approval rows remain open. Three zero-step hosted jobs remain `BLOCKED_BY_ACCESS_GITHUB_BILLING`; no deploy or promotion occurs | Retain immutable candidate.31 history; current execution moves to candidate.32 under WO-013GF |
...
| `WO-013FY` | Rebuild candidate.31's exact client/Core source headlessly and recheck the installed signed candidate | Full CLI build/test contour, `8/11` same-byte runtime comparison, exact code-section hashes, fresh source-built IPC probe and isolated Windows 11 VM | Functional build and tests pass, but the setup and three runtime files are not byte-identical, so no same-byte or new-candidate credit is assigned. Installed candidate.31 remains `11/11`; IPC passes `32/32`; VM ends powered off. Gate F stays `NO_GO 2/17/1` | Make PE/ELF metadata deterministic before claiming full reproducibility; independently continue exact managed Windows and physical Android matrices |
| `WO-013FZ` | Correct Windows build nondeterminism, merge it to client main and prove the merged tree headlessly | Two independent correction-commit builds, a third merged-main CLI build, `302/302` exact bundle comparison, complete bounded tests, Release CTest and exact Core DLL backtest | Successor client main has same-byte staged Windows reproducibility and complete local validation. The installer is unsigned under the owner exception and not installed. This is `PRE_CANDIDATE_LOCAL`; candidate.31 Gate F remains `NO_GO 2/17/1` | Assemble/sign a newly numbered candidate from exact merged main before assigning release credit; independently continue the existing candidate.31 managed Windows, physical Android, origin and rollback rows |
| `WO-013GA` | Replay the exact current platform/client/Core local quality contour and perform one additional headless Windows CLI rebuild | Clean-source `15/15` aggregate, cabinet `69/69`, static budgets `9/9`, full client gate, Android Gradle, Windows Release CTest `7/7` and `302/302` bundle comparison | All local slices pass. Missing dependencies and canonical-path setup misses are retained as environment evidence and corrected without product changes. The setup is unsigned and uninstalled; `candidate_proven=false`, candidate.32 does not exist and Gate F remains `NO_GO 2/17/1` | Create a newly numbered exact signed candidate only under separate release authorization, then execute the remaining managed-device, origin, rollback and final Gate F rows |
| `WO-013GB` | Run the read-only exact-source candidate preflight against authoritative release-index `main` | Clean four-repository identities, release-index contract, exact Core artifacts, stage policy and all `378` ledger rows | `READY_LOCAL_FREEZE`: zero blockers and zero pre-freeze rows below `I3`. Candidate creation/proof/promotion remain false; `22` candidate, `16` deferred and `13` external rows stay open. Candidate.31 Gate F remains `NO_GO 2/17/1` | Obtain separate authorization before any candidate.32 assembly; after creation, execute its candidate-stage and applicable external/manual evidence without promoting deferred rows |
| `WO-013GC` | Make the canonical production helper build and verify all five Android release files, merge it to client main and retain exact pre-candidate bytes | Client PR 74, two complete helper runs, four APK signature checks, AAB JAR/certificate/Core-ABI checks, exact hashes and independent copied-artifact replay | Exact merged client `main` produces Android `5/5` with the production certificate and identical hashes across two runs. Store is `NOT_REQUESTED`; Windows is reused only across a four-file Android-tooling/docs/test delta. Candidate creation/signing/promotion remain false; hosted zero-step CI is `BLOCKED_BY_ACCESS_GITHUB_BILLING`; no levels change | Obtain separate authority for candidate.32, then generate fresh candidate-bound SBOM/provenance/handoff, validate all six files before signing and run the exact device/origin/rollback rows |
| `WO-013GD` | Rebuild Windows headlessly from exact merged client main and close WO-013GC's reuse-only boundary | Canonical CLI release helper, full seed/contracts/tests/analyze/Gradle/Release contour, native CTest `7/7`, complete bundle hashes and independent comparison | Fresh exact-main compilation passes and the staged product bundle matches `302/302` with zero differing lines. The separately hashed installer is unsigned and unlaunched. Candidate creation/deploy/publication remain false; no levels change | Obtain separate authority for candidate.32, then generate and strictly validate its fresh six-file candidate-bound supply before any device or promotion credit |
| `WO-013GE` | Re-run the read-only freeze preflight against the complete current successor tuple after Android and Windows precursor closure | Clean platform/client/Core/release-index identities, release contract, exact Core artifacts, stage policy and all `378` ledger rows | `READY_LOCAL_FREEZE`: zero blockers and zero pre-freeze rows below `I3`. Candidate creation/proof/promotion remain false; `22` candidate, `16` deferred and `13` external rows stay open. Candidate.31 Gate F remains `NO_GO 2/17/1`; no levels change | Obtain separate authority before candidate.32 assembly; then bind and strictly validate the exact six-file candidate supply before executing candidate/external rows |
| `WO-013GF` | Create and sign candidate.32, bind exact successor supply and test the Windows non-elevated VM boundary | Six exact artifacts, fresh SBOM/provenance/handoff, release-index signer/receipt, ten hosted jobs, dedicated Windows 11 VM and all `19/19` Gate F pointers | Supply/signature pass `6/6` and `11/11`. Exact VM identity and non-elevated fail-closed behavior pass without changing the pre-existing install or network; elevated install and managed runtime are not credited. Gate F is `BLOCKED 2 PASS / 17 non-PASS / 0 FAIL`; three zero-step jobs remain access-blocked. No deploy, public release or promotion occurs | Run exact candidate.32 elevated install/upgrade and managed Windows default/AWG/Smart-DNS in isolation, then physical Android and exact origins/rollback/approval/final rows |
| `WO-013GG` | Replay candidate.32 source-owned Gates A–E and its isolated portal/client rollback, then refresh Gate F | Exact source tuple, STOP-SHIP, Gate B/D suites, local quality, dependency audits, signed manifest/handoff and disposable stable-pointer fixture | Gate B passes `70/70 + 111/111`, Gate D `196/196 + 12` plus `25/25`, local quality `15/15`, audits `0/0/0`, and rollback restores byte-identically. All five gates remain `BLOCKED` on live/manual boundaries; Gate F remains `BLOCKED 2/17/0` with zero validation errors. No index level, runtime pointer, deploy or public state changes | Run elevated managed Windows and exact Android device matrices, then named origins/provider/Operator/legal/runtime rollback/performance/final attestations before another Gate F |
| `WO-013GH` | Independently validate candidate.32 Android static signing, alignment, identity, App Bundle and direct/store separation | Four exact APKs, exact market AAB, signed release index, Android build-tools 36.1.0, official bundletool 1.18.1, direct/store manifest dumps, `10/10` flavor-contract tests and retained JAR warnings | `4/4` APKs pass signature/alignment/package/version/ABI checks; AAB passes bundletool and certificate identity. The signed direct APK alone has the install permission/private update provider, while Store excludes both. Strict JAR exit `4` warnings are retained, Store is `NOT_REQUESTED`, five stale candidate.21 ledger rows now reference candidate.32 without level promotion, and no device, host network, deploy or publication occurs | Install the exact ABI-matched APK on an isolated Android target and retain installed identity/runtime, direct upgrade, AWG/Smart-DNS, network/lifecycle/leak and clean-restore proof before any I3/I4 credit |
| `WO-013GI` | Reconcile seven ledger rows that still treated candidate.21 as current signed/index/quality/SBOM authority | Candidate.32 GF/GG/GH evidence hashes, exact seven-row before/after map and full 378-row invariant | REPO-001, DOD-23, PB-14, P12-023, PR-09, PR-10 and W1-03 now name candidate.32 as current authority. All seven retain `I3`; Gate F remains `BLOCKED 2/17/0`. Candidate.21 runtime/static history is not relabelled, and no runtime, deploy or publication occurs | Review remaining candidate.21 mentions separately; update only blockers/current labels while preserving historical evidence until exact candidate.32 device/runtime replacements exist |
| `WO-013GJ` | Reconcile fourteen residual rows whose strongest proof is predecessor history but whose future blocker must target candidate.32 | Exact fourteen-row before/after map, zero predecessor-targeting `next_action` values and the full 378-row invariant | Candidate.20/21/16 evidence stays historical while every affected future action now names exact candidate.32. Ten `I3` and four `I1` rows retain their levels; Gate F remains `BLOCKED 2/17/0`. No runtime, device, deploy or publication occurs | Stop wording-only reconciliation and execute candidate.32 managed Windows, physical Android, packaged AWG/Smart-DNS, network/lifecycle/leak/restore, origin and final approval matrices |
| `WO-013GK` | Prove exact candidate.32 Windows in-place replacement, ordinary UI/trusted IPC and disconnected reboot | Exact signed installer/manifest, elevated install, `11/11` installed identity, exact-source ordinary-user CLI, ordinary UI, clean recovery and pre/post-reboot route/DNS contour | In-place replacement passes from `8/11` to `11/11`; service/UI/IPC/disconnected initialization and reboot pass without a tunnel or DNS mutation. Three collector/harness errors remain retained and reconciled without reinstalling. Clean install, public-1.1.6 migration, connected default/AWG/Smart-DNS, lifecycle and uninstall remain open; Gate F stays `BLOCKED 2/17/0` | Continue disconnected forced-service-stop recovery, then prove clean/migration and connected default/AWG/Smart-DNS/lifecycle/uninstall boundaries separately |
| `WO-013GL` | Prove exact candidate.32 disconnected forced service termination and automatic SCM recovery | Exact `11/11` identity, elevated forced termination, old/new process observation, clean recovery journal, route/DNS contour and bounded fixture cleanup | SCM automatically restores a new service process in `5,252 ms` without operator remediation. Exact files and disconnected network remain unchanged; task and guest fixtures are removed. Connected tunnel/AWG/Smart-DNS recovery, sleep/resume, network change, endurance and uninstall remain open; Gate F stays `BLOCKED 2/17/0` | Repeat service recovery while connected under default/AWG/Smart-DNS, then prove sleep/resume, network change, endurance and connected uninstall separately |
| `WO-013GM` | Prove exact candidate.32 clean-host Windows installation from a retained zero-POKROV snapshot | Reversible snapshot transition, five exact signed-supply files, clean pre-state, setup exit `0`, `11/11` installed identity, exact source recovery contract and independent elevated post-check | Clean install passes with version `1.2.0+4053`, Auto LocalSystem service, no spontaneous UI/adapter and unchanged route/DNS. Raw collector misses remain retained without repeating the successful installer. Clean-state UI/IPC/reboot, public-1.1.6 migration and connected matrices remain open; Gate F stays `BLOCKED 2/17/0` | Resume the clean-installed snapshot for UI/IPC/initialize/reboot, then preserve it and test public-1.1.6 migration plus connected default/AWG/Smart-DNS/lifecycle/uninstall separately |
| `WO-013GN` | Prove exact candidate.32 clean boot-before-login and ordinary UI/trusted IPC/disconnected initialize | Retained clean-installed snapshot, pre-login service observation, exact-source CLI, medium-integrity owner session, UI window/owner/session checks, safe network contour and bounded fixture removal | Auto LocalSystem service runs before console login; status transitions from `artifact_ready` to disconnected `initialized`; ordinary UI opens without UAC and retains trusted IPC; network is unchanged and fixtures are removed. Launch-at-login timing, migration and connected matrices remain open; Gate F stays `BLOCKED 2/17/0` | Preserve the clean initialized state, then test public-1.1.6 migration, managed default/AWG/Smart-DNS, connected recovery/reboot/uninstall and explicit launch-at-login timing |
| `WO-013GO` | Prove exact public 1.1.6 per-user package migration to candidate.32's machine/service layout | Canonical public handoff, exact public setup/executable, zero-state baseline, ordinary per-user install, exact candidate supply, elevated migration, `11/11` identity, legacy cleanup and network contour | Public `1.1.6+29` installs as an ordinary per-user app, then candidate.32 setup exits `0`, produces exact `11/11`, runs the Auto LocalSystem service and removes the legacy directory/HKCU uninstall state only after success. Saved account/profile state, post-migration ordinary UI/IPC, Android, rollback and connected matrices remain open; Gate F stays `BLOCKED 2/17/0` | Prove ordinary UI/IPC on the migrated state, then saved-state fixtures, managed default/AWG/Smart-DNS, connected recovery/uninstall, Android migration and guarded runtime rollback separately |
| `WO-013GP` | Prove ordinary UI/trusted IPC/disconnected initialize after exact public-1.1.6 package migration | Retained migrated VM state, pre-login service observation, exact co-located CLI, medium-integrity owner session, UI owner/session/window, network contour, fail-closed placement control and complete fixture cleanup | Service is Running/Automatic before login; trusted status transitions `artifact_ready` to `initialized`; exact UI opens without UAC; connect/tunnel/DNS/egress stay false and network counts do not change. Two harness errors and expected invalid-placement `server_untrusted` remain explicit; diagnostic CLI and staging are removed. Saved-state, connected, Android and rollback matrices remain open; Gate F stays `BLOCKED 2/17/0` | Run saved-state fixtures, managed default/AWG/Smart-DNS, connected recovery/uninstall, Android migration and guarded runtime rollback separately |
| `WO-013GQ` | Prove candidate.32 exact second-launch singleton/typed forwarding/focus and retain the corrected successor source boundary | Exact installed candidate.32 UI, independent foreground control, plain/typed launches, client source correction, focused tests/build/native CTest, bounded VM replay and complete cleanup | Exact candidate.32 retains one process and forwards both activations but fails foreground focus, making `WIN-001`, Gate A and Gate F `NO_GO 2/17/2`. Client `76abed9...` plus VM replay pass as `PRE_CANDIDATE_LOCAL`; all levels remain unchanged and no candidate/public/deploy state changes | Keep candidate.32 immutable; obtain separate authority for a newly numbered corrected candidate, then repeat exact `WIN-001` and all applicable release gates |
| `WO-013GR` | Reconcile active client/platform cutover and publishing truth to the exact candidate.32 verdict | Candidate.32 signed identity and Gate F, client cutover seed/docs, platform publishing owner, successor commits and focused contract/seed/link/context checks | All current owners now name candidate.32 as immutable `NO_GO 2/17/2` and client `76abed9...` as `PRE_CANDIDATE_LOCAL`. Generic checks reject inconsistent Gate F/NO_GO state or successor proof relabelled as candidate credit. No index or mutation occurs | Keep candidate.32 immutable; promote the validated source fix through normal branches, then obtain separate authority for a newly numbered candidate and repeat exact gates |
| `WO-013GS` | Run the read-only local freeze preflight on the exact focus-corrected successor tuple | Clean platform/ledger/client/Core/release-index identities, release contracts, version parity, exact Core artifacts, stage policy and all 378 ledger rows | `READY_LOCAL_FREEZE`: zero blockers and zero pre-freeze rows below `I3`. The 51 open rows remain 22 candidate, 16 deferred and 13 external. Candidate.32 stays immutable `NO_GO`; candidate creation/proof/promotion remain false | Promote the exact source tuple through normal branches, then obtain separate authority for a newly numbered candidate before signing or runtime credit |
| `WO-013GT` | Sign candidate.33 and prove exact Windows focus | Exact signed supply and VM update | Supply and WIN-001 pass; Gate F remains BLOCKED 2/17/0; no promotion | Replay the remaining exact gates |
| `WO-013GU` | Replay candidate.33 source-owned Gates A–E and isolated portal/client rollback, then refresh Gate F | Exact source tuple, STOP-SHIP, Gate B/D suites, declared-toolchain local quality, signed handoff and disposable stable-pointer fixture | Gate B passes `70/70 + 111/111`, Gate D `196/196 + 12` plus `25/25`, local quality `15/15`, and isolated rollback restores stable byte-identically. Fresh npm audit is registry-timeout blocked and no PASS is claimed. All five gates remain `BLOCKED` on live/manual boundaries; Gate F remains `BLOCKED 2/17/0` with zero validation errors. No index level, runtime pointer, deploy or public state changes | Run exact candidate.33 connected Windows default/AWG/Smart-DNS and Android device matrices, then named origins/provider/Operator/legal/runtime rollback/performance/final attestations before another Gate F |
| `WO-013GV` | Bind bounded candidate.33 Windows and physical Android runtime, merge client truth and refresh Gate F | Exact installed Windows service/Core, dedicated headless VM, exact universal APK and physical Android 12 device | Direct and Smart DNS synthetic TUN/DNS lifecycles cleanly restore baseline; physical Wi-Fi validated DNS `4/4` and bounded Beeline slices are retained. Windows live-network and Android physical rows become `MANUAL_OWNER_TEST`; Gate F remains `BLOCKED 2/17/0`; no index level or promotion state changes | Run managed default/AWG, LDPlayer, leak/lifecycle and remaining origin/provider/Operator/legal/performance/final gates |
...
| `WO-013HA` | Prove exact candidate.33 Windows real-login hidden startup and ordinary same-process activation | Exact installed UI, real interactive user hive, guest reboot/login, route/DNS/TUN baseline, guest-only screenshots and temporary interactive-token activation | Exact `--startup` creates one responsive hidden session-1 process without UAC or network drift; ordinary activation exposes the same PID; temporary Run/task/UI state is removed. WO-013HB/013HC subsequently close product preference and synthetic saved-state/network-appearance slices; `WIN-005` stays `I3` and Gate F stays `2/17/0` | Retain as exact login-startup evidence; use WO-013HC limitations for the remaining valid-account managed auto-connect work |
...
| `WO-013HJ` | Prove exact candidate.33 combined Windows UI/service idle CPU and retain its working-set baseline | Dedicated headless VM, exact installed UI/service identities, limited interactive-session harness, 30 discarded warmups, 60 retained one-second samples, strict normalizer/validator and network/crash cleanup contour | Combined CPU p95 `0.749734% <= 1.0%` passes; combined working-set p95 `108158976` bytes is `BASELINE_RECORDED`, not regression PASS. Two pre-sample harness failures remain no-product-result. No level, Gate F, deploy or publication changes | Repeat on comparable physical Windows, compare a later matching combined-memory run, then close Android cold start, Windows cold OS boot, connect/frame/battery/thermal and authenticated cabinet performance rows |
...
- `WO-013FY-candidate31-cli-recheck.md`
- `evidence/013FY-candidate31-cli-recheck/`
- `WO-013FZ-windows-main-reproducibility.md`
- `evidence/013FZ-windows-main-reproducibility/`
- `WO-013GA-successor-main-local-quality.md`
- `evidence/013GA-successor-main-local-quality/`
- `WO-013GB-successor-main-freeze-preflight.md`
- `evidence/013GB-successor-main-freeze-preflight/`
- `WO-013GC-successor-android-production-precursor.md`
- `evidence/013GC-successor-android-production-precursor/`
- `WO-013GD-successor-windows-exact-main-cli-rebuild.md`
- `evidence/013GD-successor-windows-exact-main-cli-rebuild/`
- `WO-013GE-successor-current-freeze-preflight.md`
- `evidence/013GE-successor-current-freeze-preflight/`
- `WO-013GF-candidate32-signed-supply-windows-non-elevated.md`
- `evidence/013GF-candidate32-signed-supply-windows-non-elevated/`
- `WO-013GG-candidate32-gates-a-e-local-rollback.md`
- `evidence/013GG-candidate32-gates-a-e-local-rollback/`
- `WO-013GH-candidate32-android-static-signing.md`
- `evidence/013GH-candidate32-android-static-signing/`
- `WO-013GI-candidate32-ledger-authority-reconciliation.md`
- `evidence/013GI-candidate32-ledger-authority-reconciliation/`
- `WO-013GJ-candidate32-historical-blocker-reconciliation.md`
- `evidence/013GJ-candidate32-historical-blocker-reconciliation/`
- `WO-013GK-candidate32-windows-inplace-ipc-reboot.md`
- `evidence/013GK-candidate32-windows-inplace-ipc-reboot/`
- `WO-013GL-candidate32-windows-forced-service-recovery.md`
- `evidence/013GL-candidate32-windows-forced-service-recovery/`
- `WO-013GM-candidate32-windows-clean-install.md`
- `evidence/013GM-candidate32-windows-clean-install/`
- `WO-013GN-candidate32-windows-clean-runtime.md`
- `evidence/013GN-candidate32-windows-clean-runtime/`
- `WO-013GO-candidate32-windows-public116-migration.md`
- `evidence/013GO-candidate32-windows-public116-migration/`
- `WO-013GP-candidate32-windows-public116-postmigration-runtime.md`
- `evidence/013GP-candidate32-windows-public116-postmigration-runtime/`
- `WO-013GQ-candidate32-windows-singleton-focus-no-go.md`
- `evidence/013GQ-candidate32-windows-singleton-focus-no-go/`
- `WO-013GT-candidate33-signed-supply-windows-focus.md`
- `evidence/013GT-candidate33-signed-supply-windows-focus/`
- `WO-013GU-candidate33-gates-a-e-local-rollback.md`
- `evidence/013GU-candidate33-gates-a-e-local-rollback/`
- `WO-013GV-candidate33-bounded-windows-android-runtime.md`
- `evidence/013GV-candidate33-bounded-windows-android-runtime/`
- `WO-013GW-candidate33-windows-connected-crash-recovery.md`
- `evidence/013GW-candidate33-windows-connected-crash-recovery/`
- `WO-013GX-candidate33-windows-connected-reboot-recovery.md`
...
- `WO-013HA-candidate33-windows-login-startup.md`
- `evidence/013HA-candidate33-windows-login-startup/`
- `WO-013HB-candidate33-windows-product-preference.md`
- `evidence/013HB-candidate33-windows-product-preference/`
- `WO-013HC-candidate33-windows-saved-state-startup.md`
- `evidence/013HC-candidate33-windows-saved-state-startup/`
...
is exact `BLOCKED 2/17/0`; candidate.31 becomes the release boundary at that
historical slice and is later superseded by WO-013GF.
...
WO-013FY then proves the exact client/Core source still completes the full
headless build/test contour and that installed candidate.31 remains `11/11`
with fresh-probe IPC `32/32`. The rebuilt setup is only `8/11` same-byte; its
three different runtime files retain exact code sections but different build
metadata. It receives no release credit and Gate F remains `NO_GO 2/17/1`.
WO-013FZ then closes that source/tooling defect in merged successor client
main. Three clean CLI builds share the same source tree and the staged Windows
bundle matches `302/302` byte for byte; full bounded tests, native Release
CTest and the exact Core DLL 100-cycle backtest pass. No new candidate is
assembled, so candidate.31 and Gate F remain unchanged.
WO-013GA then repeats the clean current-source aggregate at `15/15 PASS` and
performs an additional headless Windows CLI build. Its staged bundle again
matches `302/302` and native Release CTest passes `7/7`; no candidate or Gate F
credit is assigned.
WO-013GB then runs the read-only exact-source preflight against clean published
release-index `main`. It returns `READY_LOCAL_FREEZE` with zero pre-freeze
blockers while retaining all `51` candidate/deferred/external rows below `I3`;
candidate creation, proof and promotion remain false.
WO-013GC then merges the single-path Android production helper to client
`main` and retains the exact successor Android supply set. Four APKs and the
market-only AAB pass production-signature checks and reproduce `5/5` across
two helper runs. No candidate number or candidate-bound supply metadata is
created, so this receives no `I4` or Gate F credit.
WO-013GD then compiles Windows again from that exact merged client `main` using
the canonical CLI helper. The full local contour and native CTest `7/7` pass,
and the staged bundle matches the preceding successor rebuild at `302/302`.
The unsigned installer is not launched and no candidate or Gate F credit is
assigned.
WO-013GE then repeats the read-only exact-source preflight after both precursor
slices. The current clean four-repository tuple returns `READY_LOCAL_FREEZE`
with zero blockers and zero pre-freeze rows below `I3`; all `51`
candidate/deferred/external rows remain open and no candidate credit is
assigned.

WO-013GF then creates private signed candidate.32 from that exact successor
tuple. Strict candidate supply and signature validation pass, and a dedicated
Windows 11 VM proves exact candidate bytes plus non-elevated fail-closed
installer behavior without changing its pre-existing install, service, routes
or DNS. The elevated install and managed runtime remain open. Candidate.32
Gate F is `BLOCKED 2/17/0`; candidate.31 becomes immutable predecessor history.

WO-013GG binds the exact candidate.32 source replay and disposable rollback:
Gate B passes `70/70 + 111/111`, Gate D passes `196/196 + 12` plus `25/25`,
local quality passes `15/15`, and the stable portal/client fixture restores
byte-identically. Gate F remains `BLOCKED 2/17/0`; the local rollback does not
replace an authorized runtime pointer/kill drill or origin readback.

WO-013GH independently validates the exact candidate.32 Android archives.
All `4/4` APKs pass signature, alignment, package/version and ABI checks; the
market AAB passes official bundletool validation and contains the expected
certificate and three Core ABIs. The exact direct APK alone contains the
installer permission and private update provider; the Store AAB excludes both,
and focused flavor contracts pass `10/10`. Strict JAR warnings remain explicit,
Store is `NOT_REQUESTED`, and no install or device credit is assigned. Five
stale candidate.21 ledger rows are refreshed to candidate.32 without changing
their levels or Gate F counts.

WO-013GI then reconciles seven additional current-authority rows: signed index,
PB-14 boundary, public trust surface, local quality, release hardening and
SBOM/provenance now point to candidate.32. Every row keeps its prior index;
Gate F stays `BLOCKED 2/17/0`. Candidate.21 device/runtime/static-privacy
results remain historical and are not transferred to candidate.32.

WO-013GJ completes the residual historical/current split for fourteen rows.
Candidate.20/21/16 evidence stays attached to the candidate that produced it,
while all affected future actions now target exact candidate.32. Ten `I3` and
four `I1` rows keep their levels, no ledger `next_action` points to
candidate.21, and Gate F stays `BLOCKED 2/17/0`. This is reconciliation only,
not new runtime or device proof.

WO-013GK then advances the exact candidate.32 Windows boundary in the isolated
Windows 11 VM. One elevated in-place install replaces the retained `8/11`
mixed state with exact `11/11`; ordinary-user status/initialize IPC, the exact
UI and a managed disconnected reboot pass with an Auto LocalSystem service,
clean recovery journal and unchanged route/DNS contour. Raw collector and ACL
failures remain retained and are reconciled without repeating the successful
install. This does not prove clean install, public-1.1.6 migration, connected
default/AWG/Smart-DNS, lifecycle or uninstall, so Gate F remains
`BLOCKED 2/17/0`.

WO-013GL then terminates that exact disconnected service process forcibly.
The old process disappears and SCM restores a new process automatically after
`5,252 ms`, without operator remediation. Exact `11/11` files, the clean
recovery journal and the safe route/DNS contour remain unchanged; no adapter,
UI or diagnostic fixture appears. This does not transfer credit to connected
default/AWG/Smart-DNS recovery, sleep/resume, network change, endurance or
uninstall, and Gate F remains `BLOCKED 2/17/0`.

WO-013GM then preserves that state in a return snapshot and restores the
dedicated zero-POKROV Windows 11 baseline. The exact candidate.32 installer
exits `0`; raw clean pre-state plus an independent post-check prove `11/11`,
version `1.2.0+4053`, Auto LocalSystem service and unchanged disconnected
route/DNS. Exact source confirms that the absent journal before first
initialization is the clean initial recovery state. Collector misses are kept
without repeating the successful install, and a powered-off clean-installed
snapshot is retained. Migration, clean-state UI/IPC/reboot and connected
matrices remain open, so Gate F stays `BLOCKED 2/17/0`.

WO-013GN boots that exact clean-installed snapshot and observes the Auto
LocalSystem service Running before console login. Exact-source CLI status from
the ordinary owner session begins at `artifact_ready`; initialize reaches
`initialized` while connect, tunnel, egress and DNS stay false. The exact UI
opens a same-owner/session main window without UAC and preserves trusted IPC.
The network contour is unchanged, the UI stops, and all diagnostic fixtures
are removed. Launch-at-login timing, migration and connected matrices remain
open, so Gate F stays `BLOCKED 2/17/0`.

WO-013GO restores the zero-POKROV baseline and installs the exact retained
public `1.1.6+29` package as an ordinary per-user application. The exact
candidate.32 setup then exits `0`, produces `11/11`, starts the Auto
LocalSystem service and removes the legacy directory plus HKCU uninstall state
only after success. Route/DNS counts stay unchanged, all bounded guest staging
is removed, and the exact service is left running without the UI. This proves
public-package migration only: saved account/profile state, post-migration
ordinary UI/IPC, Android upgrade/downgrade, guarded runtime rollback and every
connected matrix remain open. Gate F stays `BLOCKED 2/17/0`.

WO-013GP resumes that migrated VM and first observes the service
Running/Automatic before console login. The ordinary owner then reaches
trusted `artifact_ready`, disconnected `initialized` and an exact
same-owner/session UI main window without UAC. Connect, tunnel, DNS and egress
stay false and the safe network contour does not change. Two pre-product
harness errors remain retained. A third invalid-placement control returns
`server_untrusted` exactly because source binds the expected SCM service path
to the IPC client's executable directory; the subsequent co-located exact CLI
passes and is removed. Guest staging is removed and the VM is powered off.
Saved-state, connected, Android and rollback matrices remain open, so Gate F
stays `BLOCKED 2/17/0`.

WO-013GQ supersedes the prior candidate.32 execution direction. Do not spend
additional release-gate time attempting to rehabilitate its immutable bytes:
exact `WIN-001` focus already makes the candidate `NO_GO`. Its remaining
Windows, Android, origin and rollback gaps stay historical non-PASS rows.

After separate candidate-creation authorization, assemble a newly numbered
exact candidate from client `76abed9...`, the reviewed platform/Core tuple and
fresh candidate-bound supply metadata. Prove exact Windows plain/typed
singleton and focus first. Only after that passes should the expensive managed
default/AWG3.1/AWG2/Smart-DNS, saved-state, connected recovery/uninstall,
Android physical/LDPlayer, origin, rollback and approval matrices resume.

Reconcile Brain to exact committed source only under separate production
authorization. Use an isolated emulator only when its network path is not
inherited from the host tunnel; otherwise wait for the physical phone and
retain separate Wi-Fi and Beeline results. No trial/account or entitlement is
created without action-time owner confirmation. Do not use host input while
the owner is using the workstation.
