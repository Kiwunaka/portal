...
Wave status: `PHASE_11_CANDIDATE33_WINDOWS_LIFECYCLE_STARTUP_PREFERENCE_PASS_AWG_PLAN_READY_GATE_F_BLOCKED`
...
startup-preference authority. Private signed
...
and guest fixtures are removed and the guest link is restored. The exact universal
...
saved-state/managed auto-connect/delayed-network timing, sleep, leak/lifecycle
...
| 03 | Windows/Android runtime boundaries; conditional Linux beta foundation | Gates A and C stay `BLOCKED` below candidate proof after exact candidate.33 replay with zero source failures. Exact candidate.33 update reaches `11/11`, service state and `WIN-001` focus pass, installed service/Core direct and Smart DNS synthetic lifecycles pass, and connected forced-service termination plus connected guest reboot each restore the durable journal and exact route/DNS baseline in the headless VM. Connected uninstall terminates the interactive UI, removes every product file/service/registry/TUN, restores exact route/DNS and cleanly reinstalls `11/11`. Exact real-login `--startup` remains hidden without UAC/network mutation and ordinary activation exposes the same PID; the owning product preference creates and removes that exact Run value without premature TUN or route/DNS change. The exact universal APK installs in place on physical Android, passes Wi-Fi validated DNS `4/4`, and supplies a bounded Beeline slice. Saved account/profile migration, managed auto-connect/delayed-network/default/AWG recovery, Windows 10, SmartScreen UI, sleep/resume, IPv6, LDPlayer and the complete Android device matrix remain open. Linux native runtime/packaging remains open and deliberately unshipped | `WO-005`, `WO-005C4`, `WO-005D4`, `WO-005G`, `WO-013CC`–`WO-013CO`, `WO-013CX`, `WO-013DD`–`WO-013DF`, `WO-013DJ`, `WO-013DL`, `WO-013DO`, `WO-013EA`, `WO-013EF`, `WO-013EG`, `WO-013EV`–`WO-013HB` |
...
Windows product startup-preference authority. Candidate.33 has exact signed
...
synthetic Windows TUN/DNS cleanup, connected
...
Candidate.33 managed AWG/saved-state/complete device, origins, approvals,
...
| `WO-013HA` | Prove exact candidate.33 Windows real-login hidden startup and ordinary same-process activation | Exact installed UI, real interactive user hive, guest reboot/login, route/DNS/TUN baseline, guest-only screenshots and temporary interactive-token activation | Exact `--startup` creates one responsive hidden session-1 process without UAC or network drift; ordinary activation exposes the same PID; temporary Run/task/UI state is removed. Product-preference, saved-state and managed auto-connect timing remain open; `WIN-005` stays `I3` and Gate F stays `2/17/0` | Enable launchAtLogin through the product preference with sanitized saved state, then repeat delayed-network/service-readiness timing before I4 |
| `WO-013HB` | Prove exact candidate.33 Windows product startup preference on/off without premature auto-connect | Exact installed UI, guest-only keyboard path, temporary first-launch marker, product-generated Run value, service/TUN/route/DNS contour and complete guest cleanup | The Profile → Windows switch creates and removes the exact quoted UI plus `--startup` Run value while the same UI/service state, zero TUN and exact route/DNS fingerprints remain. Marker, UI and guest fixtures are removed and the bridged link is restored. WO-013HA separately proves real-login behavior for the same value; saved-state, managed auto-connect and delayed-network timing remain open. `WIN-005` stays `I3`; Gate F stays `2/17/0` | Use a sanitized saved-state fixture and repeat managed auto-connect plus delayed-network/service-readiness timing before I4 |
