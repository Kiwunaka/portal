...
has a later exact Gate F snapshot. WO-013FP first binds all `19/19` pointers at
...
AWG3.1, AWG2 and the ordinary fallback all stage and fail the authenticated
egress probe with clean rollback. Exact-Core AWG3.1 and AWG2 independently
return `failed_no_outer_response` from the owned direct-RU Pi. The frozen Gate
F snapshot remains `NO_GO 2/17/1`; its subsequent Windows live-network and
authenticated-egress rows are now explicit FAIL, equivalent to `2/17/3` if
regenerated after the shared live path is restored. No candidate source defect
is isolated, so candidate.29 remains active and immutable. Physical Android,
Smart DNS, provider/Operator/legal, rollback and final aggregate rows stay
...
| 01 | Release manifest, version/provenance contract, CI and stop-ship controls | Candidate.29 is the active private signed supply: six rebuilt artifacts, strict-v2 handoff, SBOM/provenance, offline validation and main-only Ed25519 signature pass. `OWNER_SOLO_EXCEPTION` remains explicit and Windows trusted signing remains `SKIPPED_BY_OWNER` for direct beta only. Exact Gate F is `NO_GO 2/17/1`; later Windows/authenticated-egress evidence adds two explicit FAIL rows without regenerating the frozen snapshot. Branch protection, final live aggregate and public promotion remain non-PASS. `DEP-001` retains `I4`; no public asset or promotion exists | `WO-002`, `WO-003`, `WO-003B`–`WO-003J`, `WO-013J`–`WO-013P`, `WO-013CX`, `WO-013DC`–`WO-013DE`, `WO-013EA`, `WO-013EB`, `WO-013EH`, `WO-013EI`, `WO-013EV`–`WO-013FR` |
| 02 | Typed connection state, proof-driven green state, core ABI and migrations | Aggregate Gate B remains `BLOCKED/I3`. Candidate.29 exact Windows passes authenticated IPC, direct TUN/DNS connect/disconnect and connected-reboot recovery, but real AWG3.1, AWG2 and ordinary-fallback managed attempts all stop at `core_egress_probe_failed` and roll back cleanly. Physical Android false-green/device matrices remain open. ABI v3 stays deferred until after 1.2.0 | `WO-004`, `WO-004A2`, `WO-004B2`, `WO-004D`, `WO-006I`, `WO-013CX`, `WO-013DD`–`WO-013DF`, `WO-013EN`, `WO-013EV`, `WO-013EW`, `WO-013FB`–`WO-013FR` |
| 03 | Windows/Android runtime boundaries; conditional Linux beta foundation | Gate C stays `BLOCKED/I3`. Candidate.29 exact setup passes first-attempt precursor upgrade, `11/11` identity, ordinary-user UI, LocalSystem service, direct TUN/DNS and connected reboot. Exact managed Windows live network and authenticated egress now FAIL across AWG3.1, AWG2 and the ordinary fallback; cleanup passes. Smart DNS, connected uninstall, Windows 10, interactive SmartScreen, sleep/crash and IPv6 remain open or environment-blocked. Exact Android runtime is unrun and host-tunneled emulator output stays excluded. Linux native runtime/packaging remains open and deliberately unshipped | `WO-005`, `WO-005C4`, `WO-005D4`, `WO-005G`, `WO-013CC`–`WO-013CO`, `WO-013CX`, `WO-013DD`–`WO-013DF`, `WO-013DJ`, `WO-013DL`, `WO-013DO`, `WO-013EA`, `WO-013EF`, `WO-013EG`, `WO-013EV`–`WO-013FR` |
...
| 10 | FRKN-derived rules and isolated AWG2/AWG3.1/HY2/Smart-DNS owner labs | AWG2/AWG3.1 and bounded HY2/Smart-DNS source labs remain locally proved and default-off. Candidate.29 packages the current client/Core contracts and passes direct-only Windows TUN/DNS. Its packaged Windows AWG3.1/AWG2 and exact-Core direct-RU Pi runs now fail: Windows reaches the common managed egress gate, while both Pi variants receive no outer response. The ordinary Windows fallback fails identically, so no AWG-specific candidate defect is isolated. Phase 10 remains `I3`, not `I4`, pending live-path restoration, exact repeats, physical Android, multi-ASN, UDP/IPv6/MTU, leak/privacy/load/lifecycle and authenticated-session evidence. HY2 remains undeployed; Gecko, Mimic and port hopping remain monitor-only | `WO-012`, `WO-013AO`, `WO-013AR`, `WO-013AS`, `WO-013AU`, `WO-013AX`–`WO-013BL`, `WO-013BP`, `WO-013BQ`, `WO-013BV`–`WO-013BX`, `WO-013CE`, `WO-013CG`, `WO-013CK`, `WO-013DG`, `WO-013DP`, `WO-013DU`, `WO-013EJ`, `WO-013EY`, `WO-013EZ`, `WO-013FB`–`WO-013FR` |
| 11 | Exact-candidate RC matrix, immutable promotion, rollback and go/no-go | Candidate.29 is the active private signed successor with exact six-artifact supply. Gate F first validates at `BLOCKED 2/17/0`, then current/Brain evidence makes it `NO_GO 2/17/1`. Subsequent exact Windows managed live-network and authenticated-egress failures would make a regenerated snapshot `2/17/3`; the shared live path is repaired and repeated first. Physical Android, Smart DNS, general-RU origin, provider/Operator/legal, runtime rollback, comparable performance and final rows remain open. No public `v1.2.0`, Store object, stable switch or promotion occurred | `WO-013`, `WO-013BK`–`WO-013FR` |

Active Phase 10/11 supersession: WO-013FO is the private signed-supply and
bounded direct-Windows authority for candidate.29. WO-013FP/013FQ are its exact
Gate F authorities, ending at `NO_GO 2/17/1`. WO-013FR is the later managed
Windows and direct-RU Pi authority: Windows live network and authenticated
egress are explicit FAIL, and both AWG variants receive no outer response on
the Pi. A regenerated aggregate would be `2/17/3`, but waits for live-path
restoration. WO-013FN remains the source-fix and precursor authority:
...
| `WO-013FR` | Exercise exact candidate.29 managed Windows egress and independent exact-Core AWG from the direct-RU Pi | Fresh Windows first launch, AWG3.1/AWG2/ordinary managed attempts, safe service probe, exact Core ARM64 runners and cleanup readback | All three Windows profiles fail `core_egress_probe_failed` with clean rollback; both Pi AWG variants fail `failed_no_outer_response`. The frozen Gate F stays `NO_GO 2/17/1`, with two subsequent FAIL rows equivalent to `2/17/3` if regenerated. No candidate byte defect is isolated and no level changes | Restore the shared live delivery path, rerun the same immutable candidate on Windows and Pi, then continue Smart DNS and physical Android matrices |
