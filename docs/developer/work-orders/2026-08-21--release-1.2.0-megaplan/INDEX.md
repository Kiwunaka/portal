...
|---|---|---|---|
| 00 | Baseline, authority map, complete execution ledger | Locally proved (`I3`); WO-013GE is the latest exact-source preflight and reports zero blockers, zero pre-freeze rows below `I3` and `READY_LOCAL_FREEZE` without creating a candidate | `WO-001`, `WO-013GE` |
| 01 | Release manifest, version/provenance contract, CI and stop-ship controls | Candidate.31 is the current private signed supply. Strict replay passes its candidate-bound CycloneDX root/property, SLSA invocation/internal references, six artifacts and eleven Windows runtime files. WO-013FY's `8/11` exact-source diagnostic remains historical; successor client main now reproduces the staged Windows bundle `302/302` byte-identically across three clean CLI builds, and WO-013GA replays the exact current tuple at local `15/15` plus one additional `302/302` CLI build. WO-013GC adds the missing canonical market-AAB build/verification path and retains exact merged-main Android `5/5` production-signed precursor bytes. WO-013GD freshly rebuilds Windows from that exact merged main and again matches the staged bundle at `302/302`, with Release CTest `7/7`. WO-013GE binds the resulting complete clean tuple to published release-index `main` and returns `READY_LOCAL_FREEZE` with zero blockers. All remain pre-candidate proof: candidate creation/proof/promotion are false. Exact Gate F is `NO_GO 2/17/1` because live Brain matches only `196/197` bound platform files; the sole mismatch is exactly known predecessor `1207b63…`. Candidate.30 remains immutable `NO_GO` history. `OWNER_SOLO_EXCEPTION` and the direct-beta Windows signing exception remain explicit. Hosted jobs that execute zero steps remain `BLOCKED_BY_ACCESS_GITHUB_BILLING`; final live aggregate and public promotion remain non-PASS. `DEP-001` retains `I4`; no public asset or promotion exists | `WO-002`, `WO-003`, `WO-003B`–`WO-003J`, `WO-013J`–`WO-013P`, `WO-013CX`, `WO-013DC`–`WO-013DE`, `WO-013EA`, `WO-013EB`, `WO-013EH`, `WO-013EI`, `WO-013EV`–`WO-013GE` |
| 02 | Typed connection state, proof-driven green state, core ABI and migrations | Aggregate Gate B remains `BLOCKED/I3`. Candidate.31 supply, exact same-byte installed identity and corrected local IPC contention pass, including a fresh exact-source probe recheck, but exact managed network runtime has not been rerun. Candidate.30 direct TUN/DNS/reboot evidence is retained only as same-byte predecessor evidence; candidate.29 AWG failures remain immutable history and are not transferred. Physical Android false-green/device matrices remain open. ABI v3 stays deferred until after 1.2.0 | `WO-004`, `WO-004A2`, `WO-004B2`, `WO-004D`, `WO-006I`, `WO-013CX`, `WO-013DD`–`WO-013DF`, `WO-013EN`, `WO-013EV`, `WO-013EW`, `WO-013FB`–`WO-013FY` |
| 03 | Windows/Android runtime boundaries; conditional Linux beta foundation | Gate C stays `BLOCKED/I3`. Candidate.31 exact same-byte installed identity passes `11/11`; the automatic LocalSystem service and corrected local IPC contention pass `32/32`, including a fresh exact-source probe recheck. Successor client main passes the full CLI contour and reproduces its staged Windows bundle `302/302`; WO-013GA adds another full headless build, `302/302` comparison and Release CTest `7/7`. WO-013GC produces the successor Android production set `5/5`, including a fail-closed verified market AAB, and WO-013GD freshly recompiles Windows from that exact merged main with another `302/302` comparison and Release CTest `7/7`; neither is installed or assigned device/candidate credit. No new candidate is assembled and candidate.31 receives no transferred credit. Managed AWG/Smart-DNS, connected uninstall, Windows 10, interactive SmartScreen, sleep/crash and IPv6 remain unrun or environment-blocked. Exact successor Android runtime is `NOT_RUN`; host-tunneled emulator output stays excluded. Linux native runtime/packaging remains open and deliberately unshipped | `WO-005`, `WO-005C4`, `WO-005D4`, `WO-005G`, `WO-013CC`–`WO-013CO`, `WO-013CX`, `WO-013DD`–`WO-013DF`, `WO-013DJ`, `WO-013DL`, `WO-013DO`, `WO-013EA`, `WO-013EF`, `WO-013EG`, `WO-013EV`–`WO-013GD` |
...
| 08 | App, cabinet and marketing UX/accessibility/performance reconciliation | Locally complete at `I3`. Candidate.25's retained source audits remain history; WO-013GA is the fresh exact-current-source `15/15` local-quality authority, including cabinet `69/69` and static budgets `9/9`, with `candidate_proven=false`. Exact candidate.31 current-origin measurement is `BLOCKED_BY_ACCESS` because the owner's active tunnel is left untouched, and Brain has one exact-source mismatch now identified as predecessor `1207b63…`. Gate E stays `BLOCKED` below I4 on source reconciliation, authenticated journeys, physical accessibility/OEM/scaling, comparable artifact/device/browser performance, support, general RU-origin and post-promotion evidence | `WO-010`, `WO-013AN`, `WO-013CI`, `WO-013CO`, `WO-013DA`, `WO-013DD`, `WO-013DE`, `WO-013DQ`, `WO-013DS`, `WO-013EL`, `WO-013EN`, `WO-013EO`, `WO-013FD`–`WO-013FH`, `WO-013FV`, `WO-013FX`, `WO-013GA` |
...
| 11 | Exact-candidate RC matrix, immutable promotion, rollback and go/no-go | Candidate.31 is the current private signed candidate. Its exact supply, signature, same-byte installed Windows identity, local IPC and direct-RU exact-Core AWG pass, but live Brain exact-source identity fails `196/197`; the sole mismatch exactly matches known predecessor `1207b63…`. Successor client main has `302/302` same-byte staged Windows reproducibility, WO-013GA adds current-source quality `15/15` plus another exact CLI rebuild, WO-013GC retains the complete Android production precursor `5/5`, WO-013GD freshly rebuilds exact merged-main Windows at `302/302` with Release CTest `7/7`, and WO-013GE binds the resulting clean tuple at `READY_LOCAL_FREEZE`; all remain pre-candidate evidence and do not replace or alter the signed candidate. Gate F is `NO_GO 2/17/1` with zero validation errors. The Pi evidence is not packaged-client or general-RU Gate F proof. Candidate.30 and candidate.29 remain immutable rejected history. Physical Android, managed Windows AWG/Smart DNS, general-RU origin, provider/Operator/legal, runtime rollback, comparable performance and final rows remain open. No public `v1.2.0`, Store object, stable switch or promotion occurred | `WO-013`, `WO-013BK`–`WO-013GE` |

Active Phase 10/11 supersession: WO-013FT is the current signed-candidate
authority, WO-013FV is the current Gate F authority, WO-013FX is its append-only
source-diagnosis correction, WO-013FY is historical exact-source diagnostic
evidence, WO-013FZ is the successor-main reproducibility authority, and
WO-013GA is the exact successor-source local-quality authority. WO-013GB is
retained preflight history; WO-013GE is the latest read-only successor-source
local-freeze preflight authority. WO-013GC is the exact merged-main Android
production precursor authority, and WO-013GD is the exact merged-main Windows
CLI rebuild authority.
Candidate.31's signature and regenerated
...
After separately authorized candidate.32 creation, prioritize its exact
Android install and packaged AWG3.1 then AWG2. Candidate.31 remains immutable
history and does not receive the successor precursor's artifact credit.
