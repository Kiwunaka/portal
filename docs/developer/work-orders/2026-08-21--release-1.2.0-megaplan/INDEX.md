...
Wave status: `PHASE_11_CANDIDATE16_SIGNED_STATIC_ARTIFACT_PRIVACY_PASS_LDPLAYER_IDLE_BASELINE_ARM64_INSTALL_PASS_WINDOWS_IDLE_PASS_AWG_PROVIDER_IDENTITY_OPEN_GATE_F_NO_GO`
...
| 01 | Release manifest, version/provenance contract, CI and stop-ship controls | Release-v2, client adoption, release-bound CI, reproducible dependencies, shared catalog and cross-repository truth are proved; the public-index source/trust root is on public main, PR-00 is merged, and `OWNER_SOLO_EXCEPTION` replaces the unavailable second reviewer without claiming independent review. WO-013DE binds the current corrected platform, client and Core tuple to signed artifact-only candidate.16. Six-artifact validation, SBOM/provenance, public-index hosted signer/receipt, support pin and dependency checks pass. `DEP-001` retains `I4`; aggregate release controls remain below promotion readiness | `WO-002`, `WO-003`, `WO-003B`, `WO-003C`, `WO-003D`, `WO-003E`, `WO-003F`, `WO-003G`, `WO-003H`, `WO-003I`, `WO-003J`, `WO-013J`, `WO-013K`, `WO-013L`, `WO-013M`, `WO-013N`, `WO-013O`, `WO-013P`, `WO-013CX`, `WO-013DC`, `WO-013DD`, `WO-013DE` |
| 02 | Typed connection state, proof-driven green state, core ABI and migrations | Aggregate Gate B remains `BLOCKED/I3`. Candidate.13's warm AWG lifecycle and candidate.14's cached-default lab failure are retained history. WO-013DE corrects false managed-profile readiness and proves the ordinary candidate.16 lifecycle. WO-013DF independently proves exact `awg31_lab` and `awg2_lab` activation, tunnel, DNS and routes, but both authenticated-egress checks fail and the app stays non-green before fail-closed disconnect. Windows connected recovery and exact physical-device boundaries remain open; ABI v3 stays deferred until after 1.2.0 | `WO-004`, `WO-004A2`, `WO-004B2`, `WO-004D`, `WO-006I`, `WO-013CX`, `WO-013DD`, `WO-013DE`, `WO-013DF` |
| 03 | Windows/Android runtime boundaries; conditional Linux beta foundation | Gate C stays `BLOCKED/I3`. Older physical, candidate.13 emulator and candidate.14 lab evidence remains immutable history and is not transferred. Exact candidate.16 x86_64 install/default traffic passes on LDPlayer; both current AWG profiles activate exactly but fail authenticated egress after tunnel/DNS/routes form. WO-013DL proves exact candidate.16 Windows install, `8/8` files, service/owner binding, authenticated IPC, SCM restart, uninstall and idle route/DNS restoration on the owner current host. WO-013DO proves the exact candidate.16 ARM64 physical install identity without launching the app; physical runtime, connected clean-VM Windows, leak/UDP53/MTU/OEM/endurance/accessibility and Store delivery remain open. WO-013DJ proves the successor Linux journald socket/fallback/readback lane, but Linux remains deliberately unshipped | `WO-005`, `WO-005C4`, `WO-005D4`, `WO-005G`, `WO-013CC`–`WO-013CO`, `WO-013CX`, `WO-013DD`, `WO-013DE`, `WO-013DF`, `WO-013DJ`, `WO-013DL`, `WO-013DO` |
| 04 | Observability, error catalog, redacted bundle and support pipeline | Locally complete (`I3`) for shipped Android/Windows scope, including four-class safe transport failures, active operational producers, Android count-only routing, no-upload short code, signed temporary support mode, encrypted-only manual export and a current 7/7 MSVC native proof for the bounded Windows service journal; 013S proves the active public pin and hosted source-control custody. WO-013DQ repeats exact candidate.16 source privacy gates: platform `53/53`, release logging over 145 production files plus four negative fixtures, observability `29/29` and support bundle `15/15`. WO-013DR then binds all six signed artifacts and adds a bounded static payload scan with zero definite findings while preserving the Windows extractor and live-runtime limits. WO-013DJ separately advances Linux `OBS-043` to `I3`; deployed runtime/RBAC/audit and exact-candidate device evidence remain open | `WO-006`, `WO-006I`, `WO-006J`, `WO-006K`, `WO-006L`, `WO-013S`, `WO-013DJ`, `WO-013DQ`, `WO-013DR` |
...
| 08 | App, cabinet and marketing UX/accessibility/performance reconciliation | Locally complete at `I3`. WO-013DE runs the complete exact candidate.16 source gate under Node `22.14.0` with Playwright/Chromium and passes `15/15`, including `413` client widgets, cabinet `69/69` and static performance `9/9`. WO-013DQ adds clean source-bound current-origin health/catalog p95 PASS across 50 samples each. WO-013DS binds the signed x86_64 install, records a stable 56.35-second LDPlayer idle baseline and passes separate direct-emulator persistent health/catalog p95 budgets; no client-session or physical-device credit transfers. Gate E stays `BLOCKED` below I4 on authenticated journeys, physical accessibility/OEM/scaling, comparable artifact/device performance, support, RU-origin and post-promotion evidence | `WO-010`, `WO-013AN`, `WO-013CI`, `WO-013CO`, `WO-013DA`, `WO-013DD`, `WO-013DE`, `WO-013DQ`, `WO-013DS` |
...
| 11 | Exact-candidate RC matrix, immutable promotion, rollback and go/no-go | Candidates 5–15 remain immutable history. WO-013DE binds exact platform `719e23d...`, client `75ba7e7...` and Core `cd8f0f4...` to signed private candidate.16. Six-artifact supply, SBOM/provenance, hosted public-index signer/receipt, exact dependency locks and complete local quality pass. Exact LDPlayer default traffic passes. WO-013DF proves both current AWG profiles activate, create tunnel/DNS/routes and then fail authenticated egress without false green. WO-013DL adds the bounded exact Windows current-host idle slice; connected network and clean-VM proof stay open. WO-013DM records Datalix payment restoration without accepting the changed host identity. WO-013DN adds the exact candidate.16 RU bundle and owned-Pi no-mutation PLAN while leaving the incomplete RU environment manual. WO-013DO closes exact physical ARM64 install identity and runs Gate F, which validates all `19/19` pointers and returns `NO_GO 2/17/2` with zero validation errors. WO-013DQ adds current-origin public API performance plus exact STOP-SHIP `7/7` local proof. WO-013DR verifies all six signed artifact sizes/hashes and records a zero-definite-finding bounded static payload privacy scan. WO-013DS binds the exact installed x86_64 APK and adds bounded idle/direct-emulator performance without converting it into client-session or physical-device proof. WO-013DT retains the pre-recovery `6/7` candidate.16 Brain slice. WO-013DU restores current-runtime DE service and proves both delivery addresses, but deploys platform `a68280f...` after candidate.16; it therefore requires a successor signed candidate instead of rewriting the old Gate F result. Physical runtime, complete successor current/Brain/RU, connected Windows, provider/Operator/legal, rollback, comparable device performance and no-open-P0 aggregate proof remain non-PASS. No tag, public `v1.2.0`, Store object, stable switch or promotion occurred | `WO-013`, `WO-013BK`–`WO-013DU` |

Active Phase 10/11 supersession: WO-013DE is the current signed-candidate
authority. Candidate.13's corrected warm AWG lifecycle and candidate.14's
cached-default activation failure remain immutable history; neither transfers
to candidate.16. The current candidate includes the merged readiness
correction, passes exact signed supply and the complete local quality gate, and
installs byte-exactly on LDPlayer. A fresh ordinary default connection passes
tunnel, managed DNS and authenticated egress, then disconnects cleanly.
WO-013DF then proves both requested lab profiles activate exactly and form
tunnel, DNS and IPv4 routes, but neither confirms authenticated egress. The app
stays non-green and removes the VPN. Final default cleanup and reconnection
pass. WO-013DO installs and reads back the exact candidate.16 ARM64 artifact
without launching the app or taking screen control. Physical runtime remains
`MANUAL_OWNER_TEST`.
