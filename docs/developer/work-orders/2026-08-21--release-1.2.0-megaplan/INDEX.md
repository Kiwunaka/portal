...
Wave status: `PHASE_11_CANDIDATE18_SIGNED_WINDOWS_FOUNDATION_ROLLBACK_BRAIN_CURRENT_API_P0_LOCAL_PASS_PHYSICAL_NETWORK_GATES_OPEN_GATE_F_NOT_RUN`
Release candidate: `POKROV_1_2_0_CANDIDATE18_SIGNED_PRIVATE_ACTIONS_ARTIFACT_ONLY_PROMOTION_NOT_AUTHORIZED`
...
| 11 | Exact-candidate RC matrix, immutable promotion, rollback and go/no-go | Candidates 5–17 remain immutable history. Candidate 17 is rejected because its eight-file Windows setup omits the three required VC143 app-local DLLs and can return false success after service-start failure. WO-013DV binds exact platform `d6898e6...`, client `820ca10...` and Core `cd8f0f4...` to signed private candidate.18. Six-artifact supply, 352-component SBOM, provenance, 11-file Windows manifest, real hosted signer/receipt and exact-source replay pass. The isolated Windows 11 VM proves clean-app-state install, service, authenticated IPC, restart, uninstall, idle-network restoration and public-1.1.6 migration. WO-013DW proves exact local pointer rollback; WO-013DX proves Brain source `197/197`, readiness `23/23` and enabled delivery `7/7` across three samples; WO-013DY proves current-origin public API budgets, seven permanent regressions and zero queried P0 issues. LDPlayer contributes install/launch/byte identity only; its host-tunnel network result is excluded. Candidate.18 physical Android Wi-Fi/Beeline, live connected Windows, authenticated current/RU origins, provider/Operator/legal, runtime rollback, comparable device performance and final no-open-P0/false-green/privacy attestation remain non-PASS. Gate F is not regenerated for candidate.18. No tag, public `v1.2.0`, Store object, stable switch or promotion occurred | `WO-013`, `WO-013BK`–`WO-013DY` |

Active Phase 10/11 supersession: WO-013DV is the current signed-candidate
authority and WO-013DW is its current isolated rollback evidence. Candidates
13–16 remain immutable history. Candidate 17 is retained
as a rejected Windows packaging result and cannot be published. Candidate 18
contains the merged Windows correction, passes exact signed supply and hosted
source replay, and installs byte-exactly on LDPlayer. The emulator contributes
only install, launch and ABI evidence because host Windows already uses another
tunnel. No previous default, AWG, Smart-DNS, physical or origin result transfers
to the new Android bytes. Physical ARM64 Wi-Fi/Beeline runtime remains
`MANUAL_OWNER_TEST`.
