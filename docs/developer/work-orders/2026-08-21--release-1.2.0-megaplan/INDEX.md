...
Wave status: `PHASE_11_CANDIDATE33_PRIVATE_SIGNED_WIN_001_PASS_GATE_F_BLOCKED`
Release candidate: `POKROV_1_2_0_CANDIDATE33_PRIVATE_SIGNED_GATE_F_BLOCKED`
...
WO-013GT is the current exact candidate authority. Private signed
...
Gate F validates all `19/19` pointers and is `BLOCKED 2 PASS / 17 non-PASS /
0 FAIL` with zero validation errors. Candidate.32 remains immutable `NO_GO`
history. No deploy, public asset, Store object, stable pointer, Gate G or
...
| 01 | Release manifest, version/provenance contract, CI and stop-ship controls | Candidate.33 is the current private signed supply. Fresh merged-source builds, candidate-specific CycloneDX/SLSA/handoff metadata, all six application artifacts and all eleven Windows runtime files pass strict replay. The exact platform/client/Core tuple and release-index source are signature-bound; Android production signing passes, while Windows remains an explicit unsigned direct-beta owner exception. Exact `WIN-001` passes and advances to `I4`; the aggregate source-owned Gates A–E/STOP-SHIP replay is still `NOT_RUN`. Release-index source-contract/signer/receipt jobs pass with real steps, while three zero-step platform/client jobs remain `BLOCKED_BY_ACCESS_GITHUB_BILLING`. Gate F is `BLOCKED 2/17/0`; `DEP-001` and `WIN-001` are `I4`, and no tag, public asset, deploy, Store object, stable pointer or promotion exists. Candidate.32 remains immutable `NO_GO` history | `WO-002`, `WO-003`, `WO-003B`–`WO-003J`, `WO-013J`–`WO-013P`, `WO-013CX`, `WO-013DC`–`WO-013DE`, `WO-013EA`, `WO-013EB`, `WO-013EH`, `WO-013EI`, `WO-013EV`–`WO-013GT` |
| 02 | Typed connection state, proof-driven green state, core ABI and migrations | Aggregate Gate B remains `BLOCKED/I3` and has not yet been replayed against candidate.33. Its exact Windows update reaches `11/11`, retains the Auto LocalSystem service and keeps the disconnected network contour unchanged; fresh client widgets pass `413/413`. Candidate.32 disconnected status/initialize, clean-install and public-package-migration slices remain predecessor evidence and do not transfer automatically. Exact candidate.33 managed default/AWG3.1/AWG2/Smart-DNS, connected recovery and physical Android false-green/device matrices remain open. ABI v3 stays deferred until after 1.2.0 | `WO-004`, `WO-004A2`, `WO-004B2`, `WO-004D`, `WO-006I`, `WO-013CX`, `WO-013DD`–`WO-013DF`, `WO-013EN`, `WO-013EV`, `WO-013EW`, `WO-013FB`–`WO-013GT` |
| 03 | Windows/Android runtime boundaries; conditional Linux beta foundation | Gates A and C stay `BLOCKED/I3`; their exact candidate.33 aggregates are not yet replayed. Exact candidate.33 update reaches `11/11`, service state passes and plain/typed singleton forwarding plus foreground focus pass under `WIN-001`, advancing that row to `I4`. Candidate.32 clean install, ordinary UI/IPC, disconnected initialization/recovery and public-package migration remain predecessor slices. Saved account/profile migration, launch-at-login timing, default/AWG/Smart-DNS connected recovery, connected uninstall, Windows 10, SmartScreen UI, sleep/resume, IPv6 and Android device matrices remain open. Candidate.33 Android has fresh production-signed `5/5` static artifacts but no LDPlayer or physical-device runtime credit. Linux native runtime/packaging remains open and deliberately unshipped | `WO-005`, `WO-005C4`, `WO-005D4`, `WO-005G`, `WO-013CC`–`WO-013CO`, `WO-013CX`, `WO-013DD`–`WO-013DF`, `WO-013DJ`, `WO-013DL`, `WO-013DO`, `WO-013EA`, `WO-013EF`, `WO-013EG`, `WO-013EV`–`WO-013GT` |
...
| 05 | Portal bounded contexts, payments, HTTP/DB/outbox reliability | Locally complete (`GATE-D` and `ARCH-002` at `I3`); 007G–007I split admin, public/client and Action Intent domain/runtime owners. Candidate.32 source passes payment/provider/callback/HTTP/DB/outbox/module `196/196 + 12 subtests` and Action Intent/policy `25/25` as predecessor evidence; exact candidate.33 Gate D replay is still `NOT_RUN`. Candidate.33 has no exact deployed Brain/current-origin or provider runtime proof. Gate D stays `BLOCKED` below I4 until exact-source replay plus production provider/PostgreSQL/outbox/reconciliation/operator/runtime-rollback evidence exists | `WO-007`, `WO-007G`, `WO-007H`, `WO-007I`, `WO-013CI`, `WO-013CO`, `WO-013EL`, `WO-013EN`, `WO-013FD`, `WO-013FE`, `WO-013FH`, `WO-013FV`, `WO-013FX`, `WO-013GF`, `WO-013GG`, `WO-013GT` |
...
| 08 | App, cabinet and marketing UX/accessibility/performance reconciliation | Locally complete at `I3`, but exact candidate.33 Gate E is not yet replayed. The fresh candidate build passes client `413/413`; candidate.32's source-bound `15/15`, cabinet `69/69`, Admin v2 SDK `75/75`, static budgets `9/9` and audits `0/0/0` remain predecessor evidence until exact-source replay. `FE_PR/PR-10` now points to candidate.33 Gate F `BLOCKED 2/17/0` without level change. Exact current/Brain origin, authenticated journeys, physical accessibility/OEM/scaling, comparable artifact/device/browser performance, support, general RU-origin and post-promotion evidence remain `NOT_RUN` or manual. Gate E stays `BLOCKED` below I4 | `WO-010`, `WO-013AN`, `WO-013CI`, `WO-013CO`, `WO-013DA`, `WO-013DD`, `WO-013DE`, `WO-013DQ`, `WO-013DS`, `WO-013EL`, `WO-013EN`, `WO-013EO`, `WO-013FD`–`WO-013FH`, `WO-013FV`, `WO-013FX`, `WO-013GA`, `WO-013GF`, `WO-013GG`, `WO-013GI`, `WO-013GT` |
...
| 11 | Exact-candidate RC matrix, immutable promotion, rollback and go/no-go | Candidate.33 is the current private signed candidate. Its freshly built six-file supply, candidate-specific SBOM/provenance/handoff, signature chain, exact Windows update and exact plain/typed singleton-forwarding/focus pass. Gate F is `BLOCKED 2 PASS / 17 non-PASS / 0 FAIL` with zero validation errors. Physical Android, LDPlayer, saved-state migration, Windows default/AWG/Smart-DNS connected lifecycle/uninstall, exact current/Brain/RU origins, provider/Operator/legal, runtime rollback, comparable performance, exact Gates A–E and final attestations remain non-PASS. Candidate.32 stays immutable `NO_GO` predecessor history. No public `v1.2.0`, Store object, stable switch, deploy or promotion occurred | `WO-013`, `WO-013BK`–`WO-013GT` |

Active Phase 10/11 supersession: WO-013GT is the current signed-candidate,
exact `WIN-001`, supply-chain and Gate F authority. Candidate.33 has exact
signed supply and Windows singleton/typed-forwarding/focus PASS, while Gate F
remains `BLOCKED 2/17/0`. Candidate.32 is immutable predecessor history;
...
Candidate.33 exact Gates A–E, saved-state/connected device, origins,
approvals, runtime rollback and final-attestation rows remain explicit
