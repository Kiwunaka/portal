...
Wave status: `PHASE_11_CANDIDATE18_SIGNED_WINDOWS_FOUNDATION_ROLLBACK_BRAIN_PASS_PHYSICAL_NETWORK_GATES_OPEN_GATE_F_NOT_RUN`
...
| 11 | Exact-candidate RC matrix, immutable promotion, rollback and go/no-go | Candidates 5–17 remain immutable history. Candidate 17 is rejected because its eight-file Windows setup omits the three required VC143 app-local DLLs and can return false success after service-start failure. WO-013DV binds exact platform `d6898e6...`, client `820ca10...` and Core `cd8f0f4...` to signed private candidate.18. Six-artifact supply, 352-component SBOM, provenance, 11-file Windows manifest, real hosted signer/receipt and exact-source replay pass. The isolated Windows 11 VM proves clean-app-state install, service, authenticated IPC, restart, uninstall, idle-network restoration and public-1.1.6 migration. WO-013DW proves the exact signed candidate.18 portal/client sequence `1.1.6 -> candidate.18 -> 1.1.6` in a disposable fixture with byte-identical restoration. WO-013DX proves exact Brain source `197/197`, readiness `23/23` and enabled delivery `7/7` across three samples. LDPlayer contributes install/launch/byte identity only; its host-tunnel network result is excluded. Candidate.18 physical Android Wi-Fi/Beeline, live connected Windows, current/RU, provider/Operator/legal, runtime rollback, comparable performance and no-open-P0 aggregate proof remain non-PASS. Gate F is not regenerated for candidate.18. No tag, public `v1.2.0`, Store object, stable switch or promotion occurred | `WO-013`, `WO-013BK`–`WO-013DX` |
