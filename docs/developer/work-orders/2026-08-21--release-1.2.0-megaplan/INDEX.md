...
Wave status: `PHASE_11_CANDIDATE8_SIGNED_GATE_F_BLOCKED_5_PASS_14_NONPASS`
...
| 11 | Exact-candidate RC matrix, immutable promotion, rollback and go/no-go | Candidates 5 and 6 remain immutable historical snapshots; candidate.7 remains immutable rejected history after its fail-first physical AWG result. WO-013CC creates signed private candidate.8 from exact platform/client/Core sources, passes the `15/15` local gate, validates six artifacts plus eight Windows runtime files, verifies the public release-index signature, refreshes current-origin and proves exact physical ordinary/AWG2/AWG3.1 authenticated egress with clean restore. WO-013CD adds exact-device WARP fallback/revoke, selected-app traffic/bypass, uplink/lifecycle/Doze/standby and Private DNS PASS while retaining active WARP, external IPv6/leak, UDP53/MTU, excluded-app, multi-OEM and endurance as open. WO-013CF proves exact Brain source/readiness/delivery, direct RU-terminal supporting reachability and the exact Windows current-host install/service/IPC/restart/uninstall slice. Candidate.8 Gate F remains `BLOCKED` at `5 PASS / 14 non-PASS / 0 FAIL`: RU and Windows stay manual because the canonical RU contour and connected clean-VM matrix remain absent. Live Smart DNS, provider, Operator, legal, comparable performance and post-promotion evidence also remain open. Hosted private zero-step billing stops remain non-PASS under the no-purchase solo policy. No tag, public `v1.2.0`, store object, stable switch or promotion occurred | `WO-013`, `WO-013BK`–`WO-013BU`, `WO-013BY`–`WO-013CF` |
...
| `WO-013` | Prove and promote the exact RC | Cross-repo/release | Signed candidate.8 is current; successor digest-bound Gate F is `BLOCKED` at `5/19 PASS`, `14/19 non-PASS`, `0 FAIL`. Gate G/public/stable promotion remains unauthorized | All release-bound WOs |
...
retaining RU and Windows live-network rows as manual. The next release action
is connected Windows TUN/DNS/AWG/recovery proof plus the prioritized AWG/DNS
lane and remaining manual Gate F evidence; canonical RU automation is separate.
