...
Wave status: `PHASE_11_CANDIDATE8_SIGNED_GATE_F_BLOCKED_6_PASS_13_NONPASS`
Release candidate: `POKROV_1_2_0_CANDIDATE8_SIGNED_PRIVATE_ACTIONS_ARTIFACT_ONLY_PUBLIC_RELEASE_NOT_CREATED`
...
| 02 | Typed connection state, proof-driven green state, core ABI and migrations | Aggregate Gate B remains `BLOCKED/I3` after WO-013CI replaces the stale candidate.3 replay with exact candidate.8: platform state/observability/support/handoff passes `70/70`, client state/diagnostics/migration/bootstrap passes `111/111`, exact Core hosted CI passes `5/5`, and physical/LDPlayer evidence preserves proof-driven green or explicit fail-closed behavior. Windows connected recovery and external live-device boundaries remain open; ABI v3 stays deferred until after 1.2.0 | `WO-004`, `WO-004A2`, `WO-004B2`, `WO-004D`, `WO-006I`, `WO-013CI` |
| 03 | Windows/Android runtime boundaries; conditional Linux beta foundation | WO-013CI advances Gate C `I2 -> I3` on exact candidate.8: physical Android and LDPlayer ordinary/AWG2/AWG3.1, bounded WARP/per-app/network/lifecycle/Private-DNS, Windows current-host install/service/IPC/restart/uninstall/idle restore and exact Core CI pass. Gate C remains `BLOCKED` below `I4` on connected clean-VM Windows, Android external leak/UDP53/MTU/excluded-app/OEM/endurance/accessibility and Store delivery. Linux is deliberately not shipped in candidate.8 | `WO-005`, `WO-005C4`, `WO-005D4`, `WO-005G`, `WO-013CC`–`WO-013CI` |
...
| 05 | Portal bounded contexts, payments, HTTP/DB/outbox reliability | Locally complete (`GATE-D` and `ARCH-002` at `I3`); 007G–007I split admin, public/client and Action Intent domain/runtime owners. WO-013CI replays exact candidate.8 source at `196/196 + 12` and `25/25`, plus Brain `197/197`, `23/23`, `7/7`; Gate D remains `BLOCKED` below I4 until production provider/PostgreSQL/outbox/reconciliation/operator/rollback proof exists | `WO-007`, `WO-007G`, `WO-007H`, `WO-007I`, `WO-013CI` |
...
| 08 | App, cabinet and marketing UX/accessibility/performance reconciliation | Locally complete at `I3`; WO-013CI binds exact candidate.8 quality `15/15`, static performance `9/9` and current-origin health/catalog p95 PASS. Gate E remains `BLOCKED` below I4 on authenticated journeys, physical screen-reader/OEM/scaling, comparable-artifact/device, browser-lab, support, RU-origin and post-promotion evidence | `WO-010`, `WO-013AN`, `WO-013CI` |
...
## Current next action — close candidate.8 Windows, origin and live-access gates
...
Active WARP, external IPv6/leak, UDP53/MTU, excluded-app, multi-OEM and
endurance remain open; no candidate.7 result is transferred into candidate.8
`I4`. Do not spend the lane on SPB beyond bounded evidence. HY2 remains
default-off and follows the existing kill-switch/material/install guards.
...
bypass. WO-013CG now closes the no-mutation inventory and guarded first-frontend
source/PLAN for the foreign `it` canary without changing it. The next honest
slice is separately authorized frontend APPLY plus immediate existing-transport
and rollback proof; only then may root-only Smart DNS material, server/route
APPLY and actual ChatGPT/Gemini/Xbox session access, resolver/DNS-SNI
attribution, leak/privacy, lifecycle and origin evidence proceed. Android
system Private DNS stays open because the current server is DoH-only. Windows
live network/recovery, exact RU-origin, provider/PostgreSQL/outbox, Operator,
legal/commercial, accessibility, comparable-device performance and
