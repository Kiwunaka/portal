...
| 01 | Release manifest, version/provenance contract, CI and stop-ship controls | Candidate.32 is the current private signed supply. Strict replay passes candidate-specific CycloneDX root/property, SLSA invocation/internal references, all six application artifacts and all eleven Windows runtime files. The exact successor platform/client/Core tuple and release-index source are signature-bound; Android production signing evidence passes, while Windows remains an explicit unsigned direct-beta owner exception. Seven of ten exact-source hosted jobs pass and three zero-step platform/client jobs remain `BLOCKED_BY_ACCESS_GITHUB_BILLING`. Mandatory stop-ship/DoD and final no-open-P0 attestations remain missing, so Gate F is `BLOCKED 2/17/0`, not release-proven. Candidate.31 and candidate.30 remain immutable predecessor history. `DEP-001` retains `I4`; no tag, public asset, deploy, Store object, stable pointer or promotion exists | `WO-002`, `WO-003`, `WO-003B`–`WO-003J`, `WO-013J`–`WO-013P`, `WO-013CX`, `WO-013DC`–`WO-013DE`, `WO-013EA`, `WO-013EB`, `WO-013EH`, `WO-013EI`, `WO-013EV`–`WO-013GF` |
| 02 | Typed connection state, proof-driven green state, core ABI and migrations | Aggregate Gate B remains `BLOCKED/I3`. Candidate.32 signature and supply pass, and its exact Windows installer fails closed from a non-elevated VM session without mutating the pre-existing install or network. Candidate.31 installed identity/IPC and older direct-network evidence remain predecessor history and are not transferred. Exact candidate.32 managed default/AWG3.1/AWG2/Smart-DNS, recovery and physical Android false-green/device matrices remain open. ABI v3 stays deferred until after 1.2.0 | `WO-004`, `WO-004A2`, `WO-004B2`, `WO-004D`, `WO-006I`, `WO-013CX`, `WO-013DD`–`WO-013DF`, `WO-013EN`, `WO-013EV`, `WO-013EW`, `WO-013FB`–`WO-013GF` |
...
| 05 | Portal bounded contexts, payments, HTTP/DB/outbox reliability | Locally complete (`GATE-D` and `ARCH-002` at `I3`); 007G–007I split admin, public/client and Action Intent domain/runtime owners. Candidate.31 Brain readiness/subscription/delivery and its known one-file predecessor mismatch remain historical runtime truth. Candidate.32 binds later platform source but has no exact deployed Brain/current-origin proof, so those rows return to `NOT_RUN`. Gate D stays `BLOCKED` below I4 until exact source reconciliation and production provider/PostgreSQL/outbox/reconciliation/operator/runtime-rollback proof exist | `WO-007`, `WO-007G`, `WO-007H`, `WO-007I`, `WO-013CI`, `WO-013CO`, `WO-013EL`, `WO-013EN`, `WO-013FD`, `WO-013FE`, `WO-013FH`, `WO-013FV`, `WO-013FX`, `WO-013GF` |
...
| 08 | App, cabinet and marketing UX/accessibility/performance reconciliation | Locally complete at `I3`. WO-013GA remains the exact bound-source `15/15` local-quality authority, including cabinet `69/69` and static budgets `9/9`; candidate.32 now signs that successor tuple. Exact candidate.32 current/Brain origin, authenticated journeys, physical accessibility/OEM/scaling, comparable artifact/device/browser performance, support, general RU-origin and post-promotion evidence remain `NOT_RUN` or manual. Gate E therefore stays `BLOCKED` below I4 | `WO-010`, `WO-013AN`, `WO-013CI`, `WO-013CO`, `WO-013DA`, `WO-013DD`, `WO-013DE`, `WO-013DQ`, `WO-013DS`, `WO-013EL`, `WO-013EN`, `WO-013EO`, `WO-013FD`–`WO-013FH`, `WO-013FV`, `WO-013FX`, `WO-013GA`, `WO-013GF` |
...
| 11 | Exact-candidate RC matrix, immutable promotion, rollback and go/no-go | Candidate.32 is the current private signed candidate. Its exact six-file supply, signature, candidate-specific SBOM/provenance/handoff and eleven-file Windows manifest pass. The dedicated Windows 11 VM proves exact candidate bytes plus non-elevated fail-closed behavior without install or network mutation; elevated install, upgrade and managed network remain open. Gate F is `BLOCKED 2/17/0` with zero validation errors. Candidate.31's installed/IPC/Pi and Brain `NO_GO` evidence remains predecessor history and is not transferred. Physical Android, LDPlayer, managed Windows default/AWG/Smart-DNS, exact current/Brain/RU origins, provider/Operator/legal, runtime rollback, comparable performance and final attestations remain non-PASS. No public `v1.2.0`, Store object, stable switch or promotion occurred | `WO-013`, `WO-013BK`–`WO-013GF` |

Active Phase 10/11 supersession: WO-013GF is the current signed-candidate and
Gate F authority. Candidate.32 exact supply/signature pass and its Windows VM
non-elevated contour fails closed, while Gate F is `BLOCKED 2/17/0` with
runtime, origin, approval, rollback and final-attestation rows explicit
non-PASS. WO-013FT/013FV retain candidate.31 signed-supply and exact Brain
