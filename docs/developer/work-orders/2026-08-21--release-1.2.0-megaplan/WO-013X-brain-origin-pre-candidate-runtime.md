# WO-013X — Brain-origin pre-candidate runtime evidence
...
The runtime app catalogs still expose stable `1.1.6` and no release manifest.
That is the expected rollback-safe state while the 1.2.0 candidate and public
assets do not exist. It is not evidence of a stale backend and must not be
changed before candidate publication and rollback gates are authorized.
...
restart, route change, payment action, stable-pointer change, candidate,
GitHub Release or public asset publication occurred.

`BRAIN_ORIGIN_PRE_CANDIDATE_RUNTIME` is `PASS`. The exact-candidate
`BRAIN_ORIGIN` promotion gate is still `NOT_RUN_NO_CANDIDATE`; the current
stable `1.1.6` catalog cannot prove future 1.2.0 bytes. The closed former-free
MTProto port is a separate operational diagnostic and prohibits a current
Telegram-proxy readiness claim, but it does not rewrite the `7/7` VPN delivery
result or prove a VPN outage.
...
not. Distribution remains `I3=309`, `I2=17`, `I1=37`, `I0=14`; 68 rows remain
below `I3`, split `0/33/14/21` across pre-freeze, candidate, external and
deferred stages.
...
Repeat the full/current quick and Brain-origin slices only after the exact
candidate exists. Keep MTProto disabled/unclaimed until its separate owned
service configuration and reachability are intentionally restored and proved.
