...
- Low-level code function inventory rows: `11660`; parser errors: `0`; token-level test-reference hints: `4589`
- Low-level entrypoint hints: `361` FastAPI route handlers, `1` FastAPI middleware, `179` Telegram handlers, `201` script CLI mains, `157` framework overrides, `1` Next.js page component; `portal_bot/bot.py` FastAPI mislabels: `0`
- Source symbol coverage audit rows: `11660`; expected-behavior notes: `11660`; entrypoint mapping gaps: `2`; story source refs: `4358`; story dependency refs: `3234`; module test refs: `552`; direct token test refs: `1634`; private inventory-only rows: `953`; private inventory public/entrypoint leakage: `0`; client platform manual-gate rows: `75`; client desktop tray manual-gate rows: `5`; platform/tray manual-gate rows with `manual_gate_refs`: `80`; client package public-API review rows: `39`; script CLI manifest review rows: `0`; active script CLI workflow-mapping gaps: `76`; script CLI deprecated rows: `2`; public symbol review rows: `54`
- Private helper coverage matrix rows: `953`; accepted-policy rows: `953`; Q-001 owner-decision rows: `0`; high risk rows: `5`; medium risk rows: `177`; low risk rows: `771`
...
- Source symbol coverage tiers are generated by `scripts/generate_code_function_inventory.py`; `entrypoint_needs_mapping_review` is `2`, `story_dependency_source_file` maps Python and TypeScript/TSX dependencies reachable from story-mapped source including symbol-free barrel/re-export modules, `module_test_ref` separates module-level automated evidence from direct symbol assertions, client platform/desktop tiers separate manual runtime verification scope and carry `manual_gate_refs`, and the current review/gap buckets remain explicit: `client_package_public_api_review = 39`, `public_symbol_review = 54`, `script_cli_manifest_review = 0`, and `script_cli_active_without_workflow_mapping = 76`.
...
- Current generated source-symbol review buckets are `public_symbol_review = 54`, `client_package_public_api_review = 39`, `script_cli_manifest_review = 0`, and `script_cli_active_without_workflow_mapping = 76`; the three nonzero buckets are open local review gaps, separate from manual-gate/platform/provider evidence.
- The dated `SCRIPT-MANIFEST-REVIEW-001` closure remains a historical snapshot. The current generated audit has `script_cli_manifest_review = 0`, so every detected script CLI is manifest-classified; `script_cli_active_without_workflow_mapping = 76` remains a separate current workflow-mapping gap.
...
- The dedicated entrypoint-story ledger currently reports `needs_story_mapping_review = 0`. Separately, the source-symbol audit reports `entrypoint_needs_mapping_review = 2`, `script_cli_manifest_review = 0`, and `script_cli_active_without_workflow_mapping = 76`; the dedicated zero does not close those active source-symbol workflow/mapping gaps or convert local tests into live Telegram/provider/device/RU-origin proof.
...
- `COMPLETION-AUDIT.md` currently classifies the local repo audit as materially advanced but not a full goal completion while current source-symbol review buckets and owner-only gates remain open.
...
- Current source-symbol coverage still has `script_cli_active_without_workflow_mapping = 76`; those local workflow-mapping review gaps remain separate from live SSH/provider/device/RU execution evidence, which is owner-gated and should be recorded separately when performed.
