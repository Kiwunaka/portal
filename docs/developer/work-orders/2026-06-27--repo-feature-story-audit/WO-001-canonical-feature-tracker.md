...
- Low-level code function inventory rows: `5103`; parser errors: `0`; token-level test-reference hints: `1705`
- Low-level entrypoint hints: `201` FastAPI route handlers, `1` FastAPI middleware, `172` Telegram handlers, `127` script CLI mains, `110` framework overrides, `1` Next.js page component; `portal_bot/bot.py` FastAPI mislabels: `0`
- Source symbol coverage audit rows: `5103`; expected-behavior notes: `5103`; entrypoint mapping gaps: `0`; story source refs: `3316`; story dependency refs: `810`; module test refs: `78`; direct token test refs: `266`; private inventory-only rows: `23`; private inventory public/entrypoint leakage: `0`; client platform manual-gate rows: `77`; client desktop tray manual-gate rows: `3`; platform/tray manual-gate rows with `manual_gate_refs`: `80`; client package public-API review rows: `3`; script CLI manifest review rows: `1`; active script CLI workflow-mapping gaps: `1`; script CLI deprecated rows: `2`; public symbol review rows: `12`
- Private helper coverage matrix rows: `23`; Q-001 owner-decision rows: `23`; high risk rows: `0`; medium risk rows: `2`; low risk rows: `21`
...
- The literal helper/class-method inventory now has symbol coverage tiers, and the current private-helper matrix has `21` low-risk and `2` medium-risk private inventory-only rows. Q-001 is answered as `ACCEPT_STORY_AND_SYMBOL_TIERS`; future stricter one-test-per-helper coverage requires a new owner decision.
...
- Current generated source-symbol review buckets are `public_symbol_review = 12`, `client_package_public_api_review = 3`, `script_cli_manifest_review = 1`, and `script_cli_active_without_workflow_mapping = 1`; these are open local review gaps, separate from manual-gate/platform/provider evidence.
- The dated `SCRIPT-MANIFEST-REVIEW-001` closure remains a historical snapshot; the current generated audit has `script_cli_manifest_review = 1`, so it does not support a current claim that every detected script CLI is classified.
...
- The dedicated entrypoint-story ledger currently reports `entrypoint_needs_mapping_review = 0`, but the source-symbol audit still has `script_cli_manifest_review = 1` and `script_cli_active_without_workflow_mapping = 1`; the generic entrypoint count does not close those script CLI review gaps or convert local tests into live Telegram/provider/device/RU-origin proof.
...
- Current source-symbol coverage still has `script_cli_active_without_workflow_mapping = 1`; that local workflow-mapping review gap remains separate from live SSH/provider/device/RU execution evidence, which is owner-gated and should be recorded separately when performed.
