...
| Canonical repo lanes in scope | `portal/master` / `PORTALapp/main` |
...
- [ ] Added the client docs pack when any WO touches `external/client-fork/app/**`.
...
- `client-only`: changes live only under `external/client-fork/app/**` and land on `PORTALapp/main`.
- `mixed`: one logical WO with two write lanes, two git-evidence lanes, and one closure decision by the orchestrator.
...
| `client` | `PORTALapp/main` | `<sha(s)>` | `yes | no` | `<notes>` |
