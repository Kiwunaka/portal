...
build tags. Но обновление требует настоящего API/config port и нашего hardening
patch stack; upstream `v4.1.0` — source baseline, не готовый binary. Наших commits
может быть немного, но наследуемый Hiddify fork крупный: 156 commits и 236
измененных файлов после общего upstream base в exact release submodule.
...
| --- | --- | --- | --- | --- |
| [Hiddify Core v4.1.0](https://github.com/hiddify/hiddify-core/releases/tag/v4.1.0) | Public GPLv3 + restrictions; у POKROV отдельное разрешение `OPERATOR_ATTESTED` | Android/iOS/macOS/Windows libs; WARP/AWG/XHTTP/Naive/Mieru/Psiphon/DNSTT; близко к текущему stack | Medium/high: breaking bindings, config migration, ABI/security patches и крупная inherited fork delta | `PRIMARY_SOURCE_BASELINE` |
...
| [Xray-core v26.3.27](https://github.com/XTLS/Xray-core/releases/tag/v26.3.27) + [libXray](https://github.com/XTLS/libXray) | MPL-2.0 core, MIT wrapper | Native owner VLESS/REALITY/XHTTP; cross-platform wrapper уже есть | High: нестабильный upstream API, свой POKROV adapter/TUN lifecycle; latest-only compatibility; нет Naive/Hysteria2 contour | `FALLBACK_C` |
...
| [shoes](https://github.com/cfal/shoes) | MIT | Rust server: VLESS/REALITY/Vision, Hysteria2, TUIC, AnyTLS, Naive, TUN | Молодой pre-1.0 project; XHTTP/control/multi-user/platform gaps | `SERVER_LAB_ONLY` |
| [Qeli](https://github.com/litvinovtd/qeli) | AGPL core/server; MPL shells | Rust, native TUN, multipath, PQ experiment, detailed postmortems | Custom protocol/TLS, beta, no independent external TLS audit, dominant maintainer | `R_AND_D_ONLY` |
...
4. Исправить C ABI ownership, raw config handling, persistence/logging и лишние
   control surfaces; составить upstream fix manifest до `1.13.14` и backport-ить
   нужные changes по одному. Blanket rebase Hiddify delta запрещен.
...
отвергает такой manifest до вызова core. При этом exact released Hiddify fork уже
содержит XHTTP implementation. Значит сначала нужен `singbox-json` contract и
interop proof с Xray server; второй Xray client engine не является обязательным.
...
Сильнейшая причина выбрать Xray — обязательная reference parity для
VLESS/REALITY/XHTTP. Xray имеет MPL-2.0 и владеет этой protocol family. Официальный
libXray уже дает Android/Apple/Linux/Windows builds, JSON `Invoke`, `CGoFree`,
validate/run/stop и Android socket/DNS hooks. Но для POKROV это все равно не
«поменять DLL»:

- нужен собственный stable adapter поверх нестабильного libXray API;
...
Поэтому Xray стоит добавлять на клиент только после RU experiment, где reference
Xray XHTTP стабильно закрывает измеренный gap, который Hiddify XHTTP/другие
contours не закрывают. На сервере Xray уже остается baseline.
...
полевая база уже. `shoes` — сильный Rust server comparator, но пока не production
control/runtime replacement. Qeli полезен как инженерный postmortem; собственный
TLS/protocol без независимого аудита брать нельзя. Набор отдельных
Naive/Hysteria/AWG cores имеет хорошие licenses, но превращает клиент в
оркестратор нескольких network engines. Для Windows это еще возможно; для
Android/iOS цена и риск выше выгоды.
...
Архитектурный и Rust/server разбор:
