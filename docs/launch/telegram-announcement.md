...
Мы продолжаем проверять релизный пакет: живые ссылки загрузки, оплату Lava.top, доставку ключей доступа на почту и доступность из разных сетей. Android для этой волны отмечен оператором как физически проверенный, а Windows выходит без доверенной подписи и может показывать предупреждение неизвестного издателя.
