...
- `/`: primary CTA should lead to application start/install; payment remains secondary while provider proof is blocked.
- `/checkout/`: keep degraded/payment-unavailable copy explicit when provider is not enabled.
...
- `/subscription/`: trial, renewal, Telegram +10 days, payment-unavailable/degraded state.
- `/subscription/checkout/`: Russian blocked-provider copy and native cabinet continuation.
- `/downloads/` and `/dashboard/downloads/`: honest Android/Windows beta or gated state.
...
Routes: `/admin/`, `/admin/dashboard/`, `/admin/users/`, `/admin/network/`, `/admin/nodes/`, `/admin/tickets/`, `/admin/bonuses/`, `/admin/promos/`, `/admin/referrals/`, `/admin/broadcast/`, `/admin/payments/`.
...
- Error/blocked: “Не удалось обновить данные”, “Повторить”, “Открыть поддержку”, “Оплата временно недоступна”, “Публичная загрузка пока не открыта”.

## Forbidden Direction

Avoid public “VPN-сервис”, unsupported platform release claims, `1.0.0`, payment readiness without provider evidence, informal `ты/твой`, and hype such as “магия скорости” or absolute “без пауз” guarantees.
