...
| Site and install page deployed | `https://pokrov.space/`, `https://pokrov.space/install/`, static release `20260508190122` | PASS | Runtime app links still not live. |
...
| Telegram deprecated/expired login UX | `webapp/e2e/telegram-login-refresh.spec.ts`, `webapp/e2e/oidc-fallback.spec.ts`, backend/session refresh code | PASS | Focused Telegram e2e passed; deprecated OAuth callback tokens now fall forward to one fresh Telegram OIDC attempt. |
...
| Admin release cockpit | `webapp/src/app/(dashboard)/admin/release/page.tsx`, focused release cockpit e2e, live static deploy `20260508195049` | PASS | Shows exact operator blockers, copy buttons for GO/probe commands, and no permanent static runtime blocker. |
...
| GitHub Actions repo guardrails | Guardrails run `25573819924` on portal commit `56f13c7152651b20e0403bdd0ace680854174bed` | PASS_WITH_SCOPE_LIMITS | CI guardrails are green on the forced Node 24 actions runtime, but skip operator-only client/browser gates as `SKIPPED_CI_UNAVAILABLE` and are not public-release authorization. |
| Static/backend deploy | Static release `20260508190122`; backend services active from earlier deploy | PASS | Runtime env link sync intentionally not applied. |
