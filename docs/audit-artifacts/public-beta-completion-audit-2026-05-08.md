...
| Site and install page deployed | `https://pokrov.space/`, `https://pokrov.space/install/`, static release `20260508195049` | PASS | Runtime app links still not live. |
...
| GitHub Actions repo guardrails | Guardrails run `25576322253` on portal commit `ac5ad12f62d7e772e9eba893a685e920603d159e` | PASS_WITH_SCOPE_LIMITS | CI guardrails are green on the forced Node 24 actions runtime, but skip operator-only client/browser gates as `SKIPPED_CI_UNAVAILABLE` and are not public-release authorization. |
| Static/backend deploy | Static release `20260508195049`; backend services active from earlier deploy | PASS | Runtime env link sync intentionally not applied. |
