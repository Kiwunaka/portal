...
| AWG2 current Windows-origin Core interop | `BLOCKED_BY_NETWORK_CURRENT_WINDOWS_ORIGIN_REVERSE_UDP` | Exact Core `e8eb772...41a1` emitted the outer handshake but returned `failed_no_outer_response`. A concurrent address-free server capture counted `34` inbound and `8` outbound packets, including `8` fixed-size initiations and `8` responses, while live handshake age remained `-1`; this distinguishes reverse-UDP loss on this origin from contract/key/server-response failure. Interop report SHA-256: `4a3be826...be2b`; packet report SHA-256: `b09d37cb...cfc5`. |
| AWG3.1 current Windows-origin Core interop | `FAIL_NO_OUTER_RESPONSE_CURRENT_WINDOWS_ORIGIN` | Exact Core `e8eb772...41a1` also emitted an outer packet and returned `failed_no_outer_response`. Alignment remains green, but no concurrent AWG3.1 packet capture was repeated, so direct versus reverse path is not overclassified. Interop report SHA-256: `5b96864a...cc6a`. |
...
| Aggregate-safe AWG deploy-plan replay | `PASS_READ_ONLY_PLAN` | Aggregate head `9bbb7c9` contains AWG operations together with OBS-087, HY2 and Smart-DNS source. The deployer digest-pins the reviewed 193-path live/candidate mapping and excludes four later aggregate-only runtime entries that are absent from both frozen revisions; one-sided path or target drift fails closed. Post-commit Brain PLAN again passed `193/193`, `runtime_mutated=false`. Report `2026-08-28-brain-awg-one-file-plan-aggregate-9bbb7c9.json`, SHA-256 `ff48d40fbe8b6447e26aca95b44bc845af865d4211e492253604ad103534e4b6`. No APPLY, deploy, restart or policy mutation occurred. |
| Huawei 4046 identity reconciliation | `PASS_READ_ONLY_PLAN` | The exact installed package reports `1.2.0+4046`; package history shows an update preserving the existing install, with `16.48` days between first install and last update. POKROV was cold-launched without debug auto-connect, Android exposed no active POKROV VPN service and no tunnel was started. Brain still held one stale `1.0.8` device card whose unique legacy install-user had no direct entitlement. The guarded binder resolved exactly one matching device, one global install-user and exactly one active entitled runtime owner, so PLAN passed with `entitlement_resolution=runtime_admin_owner_fallback`. Post-commit report for `60fa832`: `2026-08-28-huawei-awg2-bind-plan-60fa832.json`, SHA-256 `e59559dc77bbc43e730474688d9bc01d2328afb2277f4bc51f46aec4bfa36a9a`; raw identifiers were absent. This proves only fail-closed control-plane target resolution: it is not profile-fetch, Core, tunnel, candidate or release proof. No APPLY, deploy, restart, binding or policy mutation occurred. |
| LDPlayer 4046 exact-install reconciliation | `PASS_READ_ONLY_PLAN` | The generic `LDPlayer` label first selected a stale `1.0.2` server card whose install digest did not match the current rooted emulator, so it was rejected as a usable target. Binder revision `7c277ed` instead read the installed `1.2.0+4046` app-first identity through local root ADB, verified its operator-confirmed digest and sent the raw identity only through SSH stdin. Brain resolved exactly one current `1.2.0` device card, one global install-user and one directly entitled account component; both local and Brain confirmations matched. Post-commit report: `2026-08-28-ldplayer-exact-local-bind-plan-7c277ed.json`, SHA-256 `8af28b401a687ad3286aebe407d9f3a24d4a76c25a56ee6a6c0a3caeed36976f`; raw identifiers were absent. No APPLY, deploy, restart, binding, profile fetch, Core start or tunnel occurred. |
