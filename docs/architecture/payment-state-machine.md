...
For authenticated cabinet and Telegram-bound orders, fulfillment extends the linked account. The cabinet can show the single `connect.pokrov.space` subscription link and QR after access is active; the bot keeps that link behind an explicit manual-link button after a paid Telegram-bound callback as a beta-stage fallback. Anonymous public orders do not receive links in API responses; they receive one emailed access key after fulfillment.
