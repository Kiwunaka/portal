...
- `subscription_url` is generated by the backend and points to `https://connect.pokrov.space/s8Kx2mP7qR4wT/{token}`, where `{token}` must be the user's high-entropy `sub_token` and never a Telegram `tg_id`
