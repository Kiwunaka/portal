...
- `portal_bot/support_pi/runner.mjs`, `support_pi_bridge.py`, and `support_case_tools.py`
  DeepSeek 4.1 high ticket investigations use pi-agent-core with owner-bound
  read tools and staged internal notes/operator handoff. Python validates and
  atomically persists the response and case operations under the existing
  revision guard; account/payment/access mutations remain operator-owned.
...
- a node-local transport profile may carry up to eight `delivery_endpoints[]` entries with stable `id`, DNS `host`, and consumer `label` values when one physical node and one inbound are reachable through multiple public addresses. Subscription renderers expose each address as a separate choice under the one logical location; provisioning, capacity, health, panel identity, and client synchronization remain attached to the single node/inbound and must not be duplicated into alias node rows
...
- `GET /api/client/locations` preserves the existing country/city catalog and adds a stable safe `variants` list per city for manual route choice: `direct` / `Обычный` is always first, while enabled `ru_bridge_relay` endpoint ids and short labels appear only when that exact node passes endpoint validity, bridge allowlist/exclusion, transport-eligibility, and non-self-hop checks. A bridge whose stable id equals the exact delivery-node code, or whose configured host equals the delivery host, is omitted for that node. The variant projection never exposes bridge hosts, ports, Reality material, hidden selector tags, or raw config, and uses the same availability helper as Hiddify/sing-box rendering so excluded targets such as US stay direct-only.
